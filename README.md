...
Automata Lab provides four connected workspaces for creating and investigating deterministic Moore-machine-inspired state transducers. Automata Lab may be used as a sandbox within which to design, test and experiment with various types of state machine and state transducers.
...
The Console has resizable Time, Severity, Code, Source, and Message columns plus a Goto column.
Destination buttons reveal the affected Editor row when a current entity reference is available; older
messages still open the relevant page. Buttons default to Gray, with optional severity palette matching
in Application Settings. See the [Console guide](https://rohingosling.github.io/automata-lab/docs/user-guide/console-and-diagnostics).
