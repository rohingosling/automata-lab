...
import { CANONICAL_FILE_VERSION as FILE_VERSION } from "../../src/domain/model/limits.js";
...
function createPopulatedDraft (): AuthoringDraft
{
    ...
    return {
        ...
        stateMachine:
        {
            ...
            states:
            [
                { terminalState: false, description: "Waiting.", name: "state_idle" },
                { terminalState: true, description: "Working.", name: "state_active" },
            ],
            actions:
            [
...
{
    it ( "reports the attachment file version and preserves its textual state references", () =>
    {
        const original = createPopulatedDraft ();
        const draft = { ...original, chart: { ...original.chart, draftTransitions:
            original.chart.draftTransitions.map ( transition => ( {
                ...transition, sourceState: "state_idle", targetState: "state_active",
            } ) ),
        } };
        const report = createPrintableReport ( draft, 12, DEFAULT_APPLICATION_PREFERENCES );
        expect ( report.fileVersion ).toBe ( "1.4.0" );
        const chart = report.sections.find ( section => section.kind === "chart" );
        expect ( chart?.kind === "chart" ? chart.draftTransitions [ 0 ] : null ).toMatchObject ( {
            sourceState: "state_idle", targetState: "state_active",
        } );
        expect ( createPrintableReport ( original, 12, DEFAULT_APPLICATION_PREFERENCES ).fileVersion )
            .toBe ( "1.4.0" );
    } );

    it ( "copies indicator attachments and direct connections into the versioned report snapshot", () =>
    {
        const original = createPopulatedDraft ();
        const sourceIndicator = { kind: "initial" as const };
        const targetIndicator = { kind: "terminal" as const, id: 1 };
        const draft = {
            ...original,
            chart: {
                ...original.chart,
                draftTransitions: original.chart.draftTransitions.map ( transition => ( {
                    ...transition, sourceIndicator,
                } ) ),
                indicators: {
                    ...original.chart.indicators,
                    initialStateIndicator: { x: 0, y: 0, state: null },
                    indicatorTransitions: [ { id: 4, source: sourceIndicator, target: targetIndicator } ],
                },
            },
        };
        const report = createPrintableReport ( draft, 13, DEFAULT_APPLICATION_PREFERENCES );
        const chart = report.sections.find ( section => section.kind === "chart" );
        ...
        if ( chart?.kind !== "chart" )
        {
            throw new Error ( "Missing Chart Projection." );
        }
        expect ( chart.draftTransitions [ 0 ]?.sourceIndicator ).toEqual ( sourceIndicator );
        expect ( chart.draftTransitions [ 0 ]?.sourceIndicator ).not.toBe ( sourceIndicator );
        expect ( chart.indicatorTransitions ).toEqual ( draft.chart.indicators.indicatorTransitions );
        expect ( chart.indicatorTransitions?.[ 0 ]?.source ).not.toBe ( sourceIndicator );
        expect ( chart.indicatorTransitions?.[ 0 ]?.target ).not.toBe ( targetIndicator );
        expect ( draft.stateMachine ).toBe ( original.stateMachine );
    } );
    ...
    it ( "preserves duplicate entry and exit actions in declared order", () =>
    {
        ...
        expect ( statesSection?.rows.map ( state => state.terminalState ) ).toEqual ( [ false, true ] );
...
describe ( "AL-PRN-003 immutable printable report capture", () =>
{
    it ( "copies every captured value instead of retaining mutable draft references", () =>
    {
        ...
        const state                = { terminalState: false, description: "Original state", name: "state_original" };
