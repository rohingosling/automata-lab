...
): Promise<void>
{
    ...
    for ( const radius of [ 0, 9, 4 ] )
    {
        await shell.evaluate ( ( element, value ) =>
        {
            ( element as HTMLElement ).style.setProperty ( "--group-box-corner-radius", value + "px" );
        }, radius );
        const geometry = await readGroupGeometry ( container );
        for ( const group of geometry.groups )
        {
            expect ( group.radii, group.title ?? "group" ).toEqual ( Array ( 4 ).fill ( radius + "px" ) );
        }
    }
    ...
    for ( const spacing of [ DEFAULT_GROUP_SPACING, UNEQUAL_GROUP_SPACING ] )
    {
        ...
        for ( const [ index, group ] of current.groups.entries () )
        {
            ...
            expect ( group.radii, context ).toEqual ( Array ( 4 ).fill ( "4px" ) );
...
for ( const viewportWidth of [ 1440, 320 ] )
{
        ...
        async ( { page } ) =>
    {
        ...
        for ( const [ groupName, selector, count ] of [
            [ "General", ".settings-general-groups", 2 ],
            [ "Appearance", ".settings-appearance-groups", 2 ],
            [ "Chart", ".settings-chart-groups", 5 ],
            [ "Server", ".settings-server-groups", 1 ],
            [ "Solver", ".settings-solver-groups", 1 ],
            [ "Simulator", ".settings-simulator-groups", 1 ],
...
for ( const viewportWidth of [ 1440, 320 ] )
{
    test ( `Settings group borders have equal side clearance at ${viewportWidth} CSS pixels`,
        async ( { page } ) =>
    {
        await page.setViewportSize ( { width: viewportWidth, height: 900 } );
        await page.goto ( "./" );
        ...
        await page.getByRole ( "menuitem", { name: "Settings", exact: true } ).click ();
        const settings = page.getByRole ( "dialog", { name: "Application Settings" } );
        const detail   = settings.locator ( ".settings-detail" );

        for ( const inset of [ 4, 12 ] )
        {
            ...
            for ( const overflowing of [ false, true ] )
            {
                ...
                {
                    // Exercise content against both a pane edge and a native scrollbar.

                    element.style.maxHeight = value ? "160px" : "none";
                }, overflowing );

                for ( const groupName of [ "General", "Appearance", "Chart", "Server", "Solver", "Simulator", "Print" ] )
                {
                    await settings.getByRole ( "option", { name: groupName, exact: true } ).click ();
                    const geometry = await detail.evaluate ( element =>
                    {
                        ...
                        return {
                            ungroupedControls: Array.from ( element.querySelectorAll ( "input, select, button" ) )
                                .filter ( control => control.closest ( "fieldset" ) === null ).length,
                            checkboxes: Array.from ( element.querySelectorAll ( "input[type='checkbox']" ),
                                checkbox =>
                                {
                                    const label = checkbox.closest ( ".form-field" )?.querySelector ( "label" );
                                    if ( label === null || label === undefined )
                                    {
                                        throw new Error ( "A Settings checkbox needs a visible label." );
                                    }
                                    const labelBounds = label.getBoundingClientRect ();
                                    const boxBounds   = checkbox.getBoundingClientRect ();
                                    return {
                                        labelCenter: labelBounds.top + labelBounds.height / 2,
                                        boxCenter: boxBounds.top + boxBounds.height / 2,
                                    };
                                } ),
                            overflowing: element.scrollHeight > element.clientHeight,
                            groups: Array.from ( element.querySelectorAll ( "fieldset" ), fieldset =>
                            {
                                const groupBounds = fieldset.getBoundingClientRect ();
                                const style       = getComputedStyle ( fieldset );
                                return {
                                    left: groupBounds.left - left,
                                    right: right - groupBounds.right,
                    ...
                    expect ( geometry.ungroupedControls, groupName ).toBe ( 0 );
                    for ( const checkbox of geometry.checkboxes )
                    {
                        expect ( checkbox.boxCenter, groupName ).toBeCloseTo ( checkbox.labelCenter, 1 );
                    }
                    if ( groupName === "Appearance" )
                    {
                        expect ( geometry.overflowing ).toBe ( overflowing );
                    }
                    for ( const group of geometry.groups )
                    {
                        expect ( group.left, groupName ).toBeCloseTo ( inset + 4, 1 );
                        expect ( group.right, groupName ).toBeCloseTo ( group.left, 1 );
                        expect ( group.paddingLeft, groupName ).toBe ( group.paddingRight );
                    }
                }
            }
        }

        await settings.getByRole ( "option", { name: "Server", exact: true } ).click ();
        const serverGroup = settings.getByRole ( "group", { name: "Built-in Server" } );
        await expect ( serverGroup.getByRole ( "textbox", { name: "URL", exact: true } ) ).toBeVisible ();
        const button      = serverGroup.getByRole ( "button", { name: "Test Server", exact: true } );
        const buttonRight = await button.evaluate ( element => element.getBoundingClientRect ().right );
        const contentRight = await serverGroup.evaluate ( element =>
        {
            const bounds = element.getBoundingClientRect ();
            const style  = getComputedStyle ( element );
            return bounds.right - Number.parseFloat ( style.borderRightWidth ) -
                Number.parseFloat ( style.paddingRight );
        } );
        expect ( buttonRight ).toBeCloseTo ( contentRight, 1 );
        expect ( ( await new AxeBuilder ( { page } ).include ( ".settings-dialog" ).analyze () ).violations )
            .toEqual ( [] );
    } );
}

test ( "Settings discards Cancel and restores applied Solver and Simulator choices after reload",
    async ( { page } ) =>
{
    await page.goto ( "./" );
    const settings = page.getByRole ( "dialog", { name: "Application Settings" } );
    for ( const saveChanges of [ false, true ] )
    {
        await page.getByRole ( "menuitem", { name: "File", exact: true } ).click ();
        await page.getByRole ( "menuitem", { name: "Settings", exact: true } ).click ();
        await settings.getByRole ( "option", { name: "Solver", exact: true } ).click ();
        const defaultTab = settings.getByRole ( "combobox", { name: "Default Results Tab", exact: true } );
        await expect ( defaultTab ).toHaveValue ( "solver-candidate-summary" );
        await defaultTab.selectOption ( "solver-candidate-comparison" );
        await settings.getByRole ( "option", { name: "Simulator", exact: true } ).click ();
        const followTrace = settings.getByRole ( "checkbox", { name: "Follow Trace Tail", exact: true } );
        await expect ( followTrace ).toBeChecked ();
        await followTrace.uncheck ();
        await settings.getByRole ( "option", { name: "General", exact: true } ).click ();
        const followConsole = settings.getByRole ( "checkbox", { name: "Follow Tail", exact: true } );
        await expect ( followConsole ).toBeChecked ();
        await followConsole.uncheck ();
        await settings.getByRole ( "button", { name: saveChanges ? "Apply" : "Cancel", exact: true } ).click ();
        await expect ( settings ).toHaveCount ( 0 );
        await page.reload ();
    }
    await page.getByRole ( "menuitem", { name: "File", exact: true } ).click ();
    await page.getByRole ( "menuitem", { name: "Settings", exact: true } ).click ();
    await expect ( settings.getByRole ( "checkbox", { name: "Follow Tail", exact: true } ) )
        .not.toBeChecked ();
    await settings.getByRole ( "option", { name: "Solver", exact: true } ).click ();
    await expect ( settings.getByRole ( "combobox", { name: "Default Results Tab", exact: true } ) )
        .toHaveValue ( "solver-candidate-comparison" );
    await settings.getByRole ( "option", { name: "Simulator", exact: true } ).click ();
    await expect ( settings.getByRole ( "checkbox", { name: "Follow Trace Tail", exact: true } ) )
        .not.toBeChecked ();
} );
