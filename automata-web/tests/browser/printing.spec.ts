...
{
    test ( `TS5 prints terminal flags and notation before Chart navigation with execution ${enabled}`, async ( { page, browserName } ) =>
    {
        await page.addInitScript ( ( { key, version, enabled } ) =>
        {
            localStorage.setItem ( key, JSON.stringify ( { version, preferences: { simulatorEnableTerminalStates: enabled } } ) );
        }, { key: PREFERENCE_STORAGE_KEY, version: PREFERENCE_FORMAT_VERSION, enabled } );
        await page.reload ();
        const chooser = page.waitForEvent ( "filechooser" );
        await page.locator ( "[data-toolbar-entry='toolbar-open']" ).click ();
        await ( await chooser ).setFiles ( "../examples/state-machine-terminal-states.json" );
        await expect ( page.locator ( ".console-code", { hasText: "FILE_OPENED" } ) ).toHaveCount ( 1 );
        await expect ( page.locator ( ".chart-terminal-indicator" ) ).toHaveCount ( 0 );
        await installPrintChartRasterTracking ( page );
        const snapshot = await printCurrentDocument ( page, "menu" );
        expect ( snapshot.fileVersion ).toBe ( "1.4.0" );
        expect ( snapshot.stateChartImageSource ).toMatch ( /^data:image\/png/u );
        const evidence = await page.evaluate ( html =>
        {
            const report = new DOMParser ().parseFromString ( html, "text/html" );
            const states = report.querySelector ( 'table[aria-label="States"]' );
            const rows = Array.from ( states?.querySelectorAll ( "tbody tr" ) ?? [] ).map ( row =>
                Array.from ( row.children ).map ( cell => cell.textContent ) );
            const printWindow = window as typeof window & {
                readonly __automataLabPrintRasterizedArcs?: readonly { readonly fillStyle: string; readonly radius: number }[];
            };
            return { rows, arcs: printWindow.__automataLabPrintRasterizedArcs ?? [] };
        }, snapshot.reportHtml );
        expect ( evidence.rows.map ( row => row [ 2 ] ) ).toEqual ( [ "No", "No", "No", "Yes" ] );
        expect ( evidence.arcs ).toContainEqual ( { fillStyle: "#000000", radius: 15 } );
        expect ( snapshot.reportHtml ).not.toContain ( "Enable Terminal States" );
        expect ( snapshot.reportHtml ).not.toContain ( "TERMINAL_STATE_REACHED" );
        await page.setViewportSize ( { width: 794, height: 1123 } );
        await page.evaluate ( html =>
        {
            const report = document.createElement ( "article" );
            report.className = "print-report";
            report.setAttribute ( "data-print-style", "academic" );
            report.innerHTML = html;
            report.style.width = "184.6mm";
            report.style.margin = "12.7mm";
            document.body.replaceChildren ( report );
        }, snapshot.reportHtml );
        await page.emulateMedia ( { media: "print" } );
        for ( const printStyle of [ "academic", "industry" ] )
        {
            const column = await page.evaluate ( style =>
            {
                document.querySelector ( ".print-report" )?.setAttribute ( "data-print-style", style );
                const heading = document.querySelector ( 'table[aria-label="States"] thead th:nth-child(3)' );
                if ( heading === null )
                {
                    throw new Error ( "Missing Terminal State heading" );
                }
                ...
                return {
                    ...
                    requiredWidth: range.getBoundingClientRect ().width
                        + parseFloat ( appearance.paddingLeft ) + parseFloat ( appearance.paddingRight )
                        + parseFloat ( appearance.borderLeftWidth ) + parseFloat ( appearance.borderRightWidth ),
                };
            }, printStyle );
            expect ( Math.abs ( column.width - column.requiredWidth ) ).toBeLessThan ( 2 );
            const headings = await page.locator ( ".print-report-table thead th" ).evaluateAll ( elements =>
                elements.map ( heading =>
                {
                    ...
                    return {
                        ...
                        whiteSpace: appearance.whiteSpace,
            ...
            {
                expect ( heading.whiteSpace, heading.name ?? "" ).toBe ( "nowrap" );
                expect ( heading.lineCount, heading.name ?? "" ).toBe ( 1 );
                expect ( heading.availableWidth + 1, heading.name ?? "" ).toBeGreaterThanOrEqual ( heading.textWidth );
            }

            await page.screenshot ( {
                path: "tests/test-results/terminal-states-ts5/report-" + browserName + "-" + enabled + "-" + printStyle + ".png",
            } );
        }

        await page.screenshot ( { path: `tests/test-results/terminal-states-ts5/report-${browserName}-${enabled}.png` } );
    } );
}
