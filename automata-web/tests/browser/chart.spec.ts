...
): Promise<void>
{
    ...
    for ( let attemptIndex = 0; attemptIndex < 3; attemptIndex += 1 )
    {
        ...
        try
        {
            ...
            // Fit the current geometry before the fixture continues with later pointer gestures.
...
test ( "Chart zooms out to ten percent", async ( { page } ) =>
{
    ...
    for ( let step = 0; step < 20 && !( await zoomOutButton.isDisabled () ); step++ )
    {
...
test ( "Opening a model fits its first Chart view once without fitting later placement or navigation", async (
    { browserName, page },
) =>
{
    await openLightSwitchExample ( page );
    ...
    await expect.poll ( readViewport ).not.toEqual ( { x: 0, y: 0, zoom: 1 } );
    await expect.poll ( async () =>
    {
        const canvasBounds = await canvas.boundingBox ();
        const stateBounds = await page.locator ( "[data-chart-state='state_off']" ).boundingBox ();
        return canvasBounds !== null && stateBounds !== null &&
            stateBounds.x >= canvasBounds.x && stateBounds.y >= canvasBounds.y &&
            stateBounds.x + stateBounds.width <= canvasBounds.x + canvasBounds.width &&
            stateBounds.y + stateBounds.height <= canvasBounds.y + canvasBounds.height;
    } ).toBe ( true );

    const fittedViewport = await readViewport ();
    const palette = page.getByRole ( "complementary", { name: "Palette" } );
    const stateCount = await page.locator ( ".react-flow__node-state" ).count ();
    await dragPaletteItemToCanvas ( page, palette.getByRole ( "button", { name: "State", exact: true } ),
        { x: 300, y: 180 }, browserName );
    await expect ( page.locator ( ".react-flow__node-state" ) ).toHaveCount ( stateCount + 1 );
    await expect.poll ( readViewport ).toEqual ( fittedViewport );

    // Navigation retains both the viewport and the consumed first-view fit request.

    await page.locator ( "[data-toolbar-entry='toolbar-editor']" ).click ();
    await page.locator ( "[data-toolbar-entry='toolbar-chart']" ).click ();
    await expect.poll ( readViewport ).toEqual ( fittedViewport );

    // Opening the same file again is a new load and must request a fresh first-view fit.

    const fileChooser = page.waitForEvent ( "filechooser" );
    await page.getByRole ( "button", { name: "Open", exact: true } ).click ();
    await page.getByRole ( "dialog" ).getByRole ( "button", { name: "Discard and Continue", exact: true } ).click ();
...
test ( "Chart starts at 100 percent and preserves its viewport through every palette drop and history edit", async (
    { browserName, page },
) =>
{
    ...
    await expect.poll ( readViewport ).toEqual ( { x: 0, y: 0, zoom: 1 } );
    await page.locator ( ".react-flow__controls-zoomout" ).click ();
    await expect.poll ( async () => ( await readViewport () ).zoom ).toBeCloseTo ( 1 / 1.2, 5 );
    await canvas.focus ();
    await page.keyboard.press ( "ArrowRight" );
    await page.keyboard.press ( "ArrowDown" );
    await expect.poll ( async () => ( await readViewport () ).y ).not.toBe ( 0 );

    const originalViewport = await readViewport ();
    const palette = page.getByRole ( "complementary", { name: "Palette" } );
    const placements = [
        { name: "State", x: 350, y: 180 },
        { name: "Initial Indicator", x: 80, y: 80 },
        { name: "Terminal Indicator", x: 620, y: 100 },
        { name: "Transition", x: 200, y: 350 },
    ];

    for ( const [ index, placement ] of placements.entries () )
    {
        await dragPaletteItemToCanvas ( page, palette.getByRole ( "button", { name: placement.name, exact: true } ),
            { x: placement.x, y: placement.y }, browserName );
        await expect ( page.locator ( ".react-flow__node" ) ).toHaveCount ( index + 1 );
        await expect.poll ( readViewport ).toEqual ( originalViewport );
    }
    ...
    await expect.poll ( readViewport ).toEqual ( originalViewport );
    await page.getByRole ( "button", { name: "Fit Chart", exact: true } ).click ();
    await expect.poll ( readViewport ).not.toEqual ( originalViewport );

    for ( let layoutAttempt = 0; layoutAttempt < 2; layoutAttempt++ )
    {
        const viewportBeforePan = await readViewport ();
        const canvasBounds = await canvas.boundingBox ();
        if ( canvasBounds === null )
        {
            throw new Error ( "The Chart canvas has no rendered bounds." );
        ...
        await page.mouse.move ( canvasBounds.x + 80, canvasBounds.y + 20, { steps: 4 } );
        await page.mouse.up ( { button: "middle" } );
        await expect.poll ( readViewport ).not.toEqual ( viewportBeforePan );
        const viewportBeforeLayout = await readViewport ();
        await page.getByRole ( "button", { name: "Automatic Layout", exact: true } ).click ();
        await expect.poll ( readViewport ).not.toEqual ( viewportBeforeLayout );
        await expect ( page.getByRole ( "button", { name: "Automatic Layout", exact: true } ) ).toBeEnabled ();
...
test ( "Chart preserves its viewport when a state drag commits", async ( { page } ) =>
{
    ...
    // Bring the fixture's offscreen states into view before recording and testing the viewport.
    ...
    // Keep this commit-preservation gesture clear of intentional edge auto-pan.
...
test ( "Phase 5 renders the complete authoring projection and runs layered automatic layout", async ( { page } ) =>
{
    ...
    // Browser-measured handles round in screen pixels, including at low fitted zoom levels.

    const viewportZoom = await page.locator ( ".react-flow__viewport" ).evaluate ( element =>
    ...
    const boundaryTolerance        = 0.05 / viewportZoom;
...
) =>
{
    ...
    await expect ( page.getByRole ( "dialog", { name: /^(Add|Edit) State$/u } ) ).toHaveCount ( 0 );
    await expect ( page.getByRole ( "dialog", { name: /^(Add|Edit) Transition$/u } ) ).toHaveCount ( 0 );
    ...
    await expect ( page.getByRole ( "dialog", { name: /^(Add|Edit) Transition$/u } ) ).toBeHidden ();
...
test ( "Chart drag-drop drafts configure atomically and preserve accessible focus", async ( { browserName, page } ) =>
{
    ...
    // The initial states already do not overlap. Wait for layout and its deferred focus restore
    // before creating or selecting drafts in the resulting scene.

    const automaticLayoutButton = footer.getByRole ( "button", { name: "Automatic Layout", exact: true } );

    await automaticLayoutButton.click ();
    await expect ( automaticLayoutButton ).toBeEnabled ();
    await expect ( automaticLayoutButton ).toBeFocused ();
    await expect.poll ( async () =>
    {
        // Initialize the local values needed by this operation.
    ...
    await expect.poll ( () => page.locator ( ".chart-state-node" ).first ().locator (
        ".react-flow__handle",
    ...
    // Await the creation focus request before deliberately testing later focus loss.
    // Manually focusing the node here could pass before the deferred request has finished.

    await expect ( draftNode ).toBeVisible ();
    await expect ( draftNode ).toBeFocused ();
    await draftPath.click ( { force: true } );
    await expect ( draftNode.locator ( ".chart-draft-transition-node" ) )
        .toHaveClass ( /chart-transition-selected/u );
    await expect ( page.getByText ( "Chart Elements Selected: 1", { exact: true } ) ).toBeVisible ();
    await draftNode.focus ();
    await expect ( draftNode ).toBeFocused ();
    await draftNode.evaluate ( element => ( element as HTMLElement ).blur () );
    ...
    const dragPoint = await draftPath.evaluate ( element =>
    {
        if ( !( element instanceof SVGPathElement ) )
        {
            throw new Error ( "The draft curve must be an SVG path." );
        }
        const transform = element.getScreenCTM ();
        if ( transform === null )
        {
            throw new Error ( "The draft curve has no screen transform." );
        }
        const point = element.getPointAtLength ( element.getTotalLength () / 2 );
        const screenPoint = new DOMPoint ( point.x, point.y ).matrixTransform ( transform );
        return { x: screenPoint.x, y: screenPoint.y };
    } );
    await page.mouse.move ( dragPoint.x, dragPoint.y );
    await page.mouse.down ();
    await page.mouse.move ( dragPoint.x + 70, dragPoint.y + 35, { steps: 8 } );
    await page.mouse.up ();
    ...
    {
        const clippedSource = await draftPath.evaluate ( element =>
        {
            if ( !( element instanceof SVGPathElement ) )
            {
                throw new Error ( "The draft curve must be an SVG path." );
        ...
        expect ( firstSnappedEndpointBounds.x + firstSnappedEndpointBounds.width / 2 )
            .toBeCloseTo ( clippedSource.x, 0 );
        expect ( firstSnappedEndpointBounds.y + firstSnappedEndpointBounds.height / 2 )
            .toBeCloseTo ( clippedSource.y, 0 );
        expect ( Math.hypot (
            clippedSource.x - ( firstStateBounds.x + firstStateBounds.width / 2 ),
            clippedSource.y - ( firstStateBounds.y + firstStateBounds.height / 2 ),
        ) ).toBeGreaterThanOrEqual ( Math.min ( firstStateBounds.width, firstStateBounds.height ) / 2 - 1 );
    }
    ...
    if ( secondSnappedEndpointBounds !== null && secondStateBounds !== null )
    {
        ...
        expect ( secondSnappedEndpointBounds.y + secondSnappedEndpointBounds.height / 2 )
            .toBeCloseTo ( clippedSource.y, 0 );
        expect ( Math.hypot (
            clippedSource.x - ( secondStateBounds.x + secondStateBounds.width / 2 ),
            clippedSource.y - ( secondStateBounds.y + secondStateBounds.height / 2 ),
        ) ).toBeGreaterThanOrEqual ( Math.min ( secondStateBounds.width, secondStateBounds.height ) / 2 - 1 );
    }
...
test ( "Editor semantic deletion focuses its confirmation action", async ( { browserName, page } ) =>
{
    ...
    await page.getByRole ( "button", { name: "Confirm" } ).click ();
    await expect ( statePane.getByRole ( "gridcell", { name: "state_editor_delete" } ) ).toBeVisible ();
    ...
    await expect ( statePane.getByRole ( "gridcell", { name: "state_editor_delete" } ) ).toHaveCount ( 0 );
...
{
    for ( const selfTransition of [ false, true ] )
    {
        test ( `Chart attached drafts complete in either order: target-first=${targetFirst}, self=${selfTransition}`,
            async ( { browserName, page } ) =>
        {
            await createNewDocument ( page );
            await page.getByRole ( "treeitem", { name: "Events", exact: true } ).click ();
            await page.locator ( ".editor-list-page" ).getByRole ( "button", { name: "Add" } ).click ();
            await page.getByRole ( "textbox", { name: "Name" } ).fill ( "event_go" );
            await page.getByRole ( "button", { name: "Confirm" } ).click ();
            await page.getByRole ( "treeitem", { name: "Chart", exact: true } ).click ();
            const palette = page.getByRole ( "complementary", { name: "Palette" } );
            const stateButton = palette.getByRole ( "button", { name: "State", exact: true } );
            await dragPaletteItemToCanvas ( page, stateButton, { x: 240, y: 180 }, browserName );
            await dragPaletteItemToCanvas ( page, stateButton, { x: 640, y: 180 }, browserName );
            await page.getByRole ( "button", { name: "Automatic Layout", exact: true } ).click ();
            await expect ( page.getByRole ( "button", { name: "Automatic Layout", exact: true } ) ).toBeEnabled ();
            const firstState = page.locator ( "[data-chart-state='state_1']" );
            const secondState = page.locator ( "[data-chart-state='state_2']" );

            // The existing ordered shortcut and the repaired same-state shortcut leave Event empty.

            await firstState.click ();
            await ( selfTransition ? firstState : secondState ).click ( { modifiers: [ "Shift" ] } );
            const dialog = page.getByRole ( "dialog", { name: /^(Add|Edit) Transition$/u, exact: true } );
            await expect ( dialog ).toBeVisible ();
            await expect ( dialog.getByRole ( "combobox", { name: "State", exact: true } ) ).toHaveValue ( "state_1" );
            await expect ( dialog.getByRole ( "combobox", { name: "Next State", exact: true } ) )
                .toHaveValue ( selfTransition ? "state_1" : "state_2" );
            await expect ( dialog.getByRole ( "combobox", { name: "Event", exact: true } ) ).toHaveValue ( "" );
            await dialog.getByRole ( "button", { name: "Cancel" } ).click ();

            await dragPaletteItemToCanvas ( page, palette.getByRole ( "button", { name: "Transition", exact: true } ),
                { x: 120, y: 380 }, browserName );
            const order = targetFirst ? [ "target", "source" ] as const : [ "source", "target" ] as const;
            for ( const [ index, endpoint ] of order.entries () )
            {
                const grip = page.getByRole ( "button", { name: `Move draft transition ${endpoint} endpoint 0` } );
                await dragLocatorToLocator ( page, grip, endpoint === "source" || selfTransition ? firstState : secondState );
                if ( index === 0 )
                {
            ...
            await performRedo ( page, browserName );
            await expect ( dialog ).toHaveCount ( 0 );
            const sourceGrip = page.getByRole ( "button", { name: "Move draft transition source endpoint 0" } );
            ...
            await expect ( dialog.getByRole ( "combobox", { name: "Next State", exact: true } ) )
                .toHaveValue ( selfTransition ? "state_1" : "state_2" );
            await dialog.getByRole ( "combobox", { name: "Event", exact: true } ).selectOption ( "event_go" );
            await dialog.getByRole ( "button", { name: "Confirm" } ).click ();
            await expect ( page.locator ( ".react-flow__node-draftTransition" ) ).toHaveCount ( 0 );
            await expect ( page.locator ( ".react-flow__edge" ) ).toHaveCount ( 1 );
            await performUndo ( page, browserName );
            await expect ( page.locator ( ".react-flow__node-draftTransition" ) ).toHaveCount ( 1 );
            await expect ( dialog ).toHaveCount ( 0 );
        } );
    }
}

test ( "Chart remembers pan, zoom, and Fit Chart across workspace navigation and resets for New", async ( { page } ) =>
{
    await openLightSwitchExample ( page );
    await page.getByRole ( "treeitem", { name: "Chart", exact: true } ).click ();
    await page.getByRole ( "treeitem", { name: "Editor", exact: true } ).focus ();
    await page.keyboard.press ( "ArrowRight" );
    const canvas = page.getByLabel ( "State Chart canvas" );
    ...
    for ( const operation of [ "pan", "zoomout", "zoomin", "fit" ] )
    {
        const previousViewport = await readViewport ();
        if ( operation === "pan" )
        {
            await canvas.focus ();
            await page.keyboard.press ( "ArrowRight" );
        ...
        {
            await page.locator ( ".react-flow__controls-" + operation ).click ();
        }
        await expect.poll ( readViewport ).not.toEqual ( previousViewport );
        if ( operation === "zoomin" || operation === "zoomout" )
        {
            const expectedZoom = previousViewport.zoom * ( operation === "zoomin" ? 1.2 : 1 / 1.2 );
            await expect.poll ( async () => ( await readViewport () ).zoom ).toBeCloseTo ( expectedZoom, 5 );
        }
        const retainedViewport = await readViewport ();
        for ( const route of [ "States", "Events", "Actions", "Transition Table", "Solver", "Simulator" ] )
        {
            await page.getByRole ( "treeitem", { name: route, exact: true } ).click ();
            await expect ( canvas ).toHaveCount ( 0 );
            await page.getByRole ( "treeitem", { name: "Chart", exact: true } ).click ();
            await expect.poll ( readViewport ).toEqual ( retainedViewport );
        }
    }
    await page.getByRole ( "button", { name: "New", exact: true } ).click ();
    await page.getByRole ( "treeitem", { name: "Chart", exact: true } ).click ();
