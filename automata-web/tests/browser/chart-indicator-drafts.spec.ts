...
import type { FileDocumentV1, FileDocumentV1_4 } from "../../src/domain/model/contracts.js";
...
//--------------------------------------------------------------------------------------------------
// Function: expectedStateMachine
//
// Description:
//
//   Defines expected flags after each gesture while retaining the legacy input fixture.
//
//--------------------------------------------------------------------------------------------------

function expectedStateMachine (
    document:       FileDocumentV1,
    terminalStates: readonly string[] = [],
): FileDocumentV1_4["state_machine"]
{
    return { ...document.state_machine, states: document.state_machine.states.map ( state => (
        { ...state, terminal_state: terminalStates.includes ( state.name ) }
    ) ) };
}
...
async function openDocument ( page: Page, document: FileDocumentV1 | FileDocumentV1_4 ): Promise<void>
{
...
async function saveDocument ( page: Page ): Promise<FileDocumentV1_4>
{
    ...
    return JSON.parse ( await readFile ( filePath, "utf8" ) ) as FileDocumentV1_4;
}
...
for ( const indicatorKind of [ "initial", "terminal" ] as const )
{
    for ( const firstAttachment of [ "indicator", "state" ] as const )
    {
            ...
            async ( { page } ) =>
        {
            ...
            await expect ( page.locator ( ".chart-draft-transition-node" ) ).toHaveCount ( 1 );
            await expect ( page.getByRole ( "dialog", { name: /^(Add|Edit) Transition$/u, exact: true } ) ).toHaveCount ( 0 );
            await expect.poll ( () => readViewport ( page ) ).toBe ( initialViewport );
            const halfAttached = await saveDocument ( page );

            expect ( halfAttached.state_machine ).toEqual ( expectedStateMachine ( fixture ) );
            ...
            if ( indicatorKind === "initial" && firstAttachment === "indicator" )
            {
                ...
                expect ( moved.file_version ).toBe ( "1.4.0" );
                expect ( moved.chart.draft_transitions?.[ 0 ] ).toMatchObject ( { source_indicator: { kind: "initial" } } );
                expect ( moved.state_machine ).toEqual ( expectedStateMachine ( fixture ) );
                await page.locator ( "[data-toolbar-entry='toolbar-undo']" ).click ();
            ...
            expect ( completed.file_version ).toBe ( "1.4.0" );
            expect ( completed.chart.draft_transitions ).toEqual ( [] );
            expect ( completed.state_machine ).toEqual ( { ...expectedStateMachine (
                fixture, indicatorKind === "terminal" ? [ "B" ] : [] ),
...
test ( "A reversed Palette draft creates a persistent visual-only initial-to-terminal connection", async ( { page } ) =>
{
    ...
    expect ( completed.file_version ).toBe ( "1.4.0" );
    expect ( completed.state_machine ).toEqual ( expectedStateMachine ( fixture ) );
    expect ( completed.chart.draft_transitions ).toEqual ( [] );
    ...
    expect ( ( await saveDocument ( page ) ).state_machine ).toEqual ( expectedStateMachine ( fixture ) );
    await page.locator ( "[data-toolbar-entry='toolbar-undo']" ).click ();
...
for ( const indicatorAttached of [ false, true ] )
{
    for ( const stateName of [ "A", "B" ] )
    {
            ...
            async ( { page } ) =>
        {
            ...
            expect ( completed.state_machine ).toEqual ( { ...expectedStateMachine ( fixture ), initial_state: stateName } );
...
test ( "Deleting the initial edge preserves Save access and an incomplete orphan through history", async ( { page } ) =>
{
    ...
    const incomplete = JSON.parse ( await readFile ( filePath, "utf8" ) ) as FileDocumentV1_4;
