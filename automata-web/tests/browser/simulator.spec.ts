...
test ( "Phase 8 uses the shared action panel metrics for its page command bar", async ( { page } ) =>
{
    ...
    // Section 6.1: 8-CSS-pixel top separation, a 6-CSS-pixel gap, right alignment, and one non-wrapping
    // row. The shared page container owns the trailing inset, which lines the last button
    ...
    // retains its internal footer spacing until the action-panel rollout.
...
test ( "Phase 8 renders the traces with the shared tabular treatment", async ( { page } ) =>
{
    ...
    expect ( cellStyle ).toEqual (
        { borderTopWidth: "1px", padding: "4px 7px", textAlign: "start", verticalAlign: "top" },
    );
...
test ( "the trace region opens evenly divided and holds a chosen division", async ( { page } ) =>
{
    ...
    async function traceHeights (): Promise<{ readonly action: number; readonly transition: number }>
    {
        ...
        {
            const actionTrace     = document.querySelector ( ".simulator-action-trace" );
            const transitionTrace = document.querySelector ( ".simulator-transition-trace" );

            if ( actionTrace === null || transitionTrace === null )
            {
                throw new Error ( "Both trace panes must be present." );
            }
            return {
                action:     actionTrace.getBoundingClientRect ().height,
                transition: transitionTrace.getBoundingClientRect ().height,
            };
        } );
    }
    ...
    // Read both panes in one layout and wait for the pointer-driven render to settle.

    await expect.poll ( async () =>
    {
        const heights = await traceHeights ();
        return heights.transition > resized.transition + 40 && heights.action < resized.action - 40;
    } ).toBe ( true );
