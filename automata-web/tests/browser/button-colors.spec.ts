...
// Description:
//
//   Samples the rendered fill and text in each interaction state without activating the button.
//
//--------------------------------------------------------------------------------------------------

async function readButtonPalette ( page: Page, button: Locator )
{
    const colors: { background: string; text: string; shadow: string; border: string }[] = [];
    ...
    for ( const interaction of [ "rest", "hover", "pressed" ] )
    {
        ...
        colors.push ( await button.evaluate ( element =>
        {
            const style = getComputedStyle ( element );
            return { background: style.backgroundColor, text: style.color, shadow: style.textShadow,
...
for ( const theme of [ "Light", "Dark" ] as const )
{
    test ( `button colors preserve Console defaults and commit independently in ${theme}`, async ( { page } ) =>
    {
        ...
        const defaultSurface = theme === "Light" ? "rgb(230, 228, 225)" : "rgb(56, 56, 56)";
        ...
        await expect ( consoleButton ).toHaveCSS ( "background-color", defaultSurface );
        ...
        await expect ( consoleButtons ).toBeEnabled ();
        ...
        await expect ( consoleButton ).toHaveCSS ( "color", "rgb(23, 33, 43)" );
    ...
    test ( `button palettes share fills and readable text in ${theme}`, async ( { page } ) =>
    {
        ...
        for ( const color of COLOR_CHOICES )
        {
            ...
            await page.mouse.move ( 0, 0 );
            await page.mouse.up ();
            const applicationPalette = await readButtonPalette ( page, button );
            const consolePalette = await readButtonPalette ( page, page.locator ( ".console-row button" ).first () );
            expect ( consolePalette.map ( colors => colors.border ) )
                .toEqual ( [ "1px solid rgb(0, 0, 0)", "1px solid rgb(0, 0, 0)", "1px solid rgb(0, 0, 0)" ] );
            if ( theme === "Light" && ( color === "Gray" || color === "White" ) )
            {
                const expectedSurfaces = color === "Gray"
                    ? [ "rgb(230, 228, 225)", "rgb(216, 214, 211)", "rgb(202, 200, 197)" ]
                    : [ "rgb(250, 251, 252)", "rgb(238, 242, 245)", "rgb(225, 231, 237)" ];
                expect ( consolePalette.map ( colors => colors.background ), color + " Console shades" )
                    .toEqual ( expectedSurfaces );
                expect ( consolePalette.map ( colors => colors.text ) )
                    .toEqual ( [ "rgb(23, 33, 43)", "rgb(23, 33, 43)", "rgb(23, 33, 43)" ] );
                expect ( consolePalette.map ( colors => colors.shadow ) ).toEqual ( [ "none", "none", "none" ] );
            }
            else
            {
                expect ( consolePalette, theme + " " + color + " Console palette" ).toEqual ( applicationPalette );
            }
...
test ( "system colors override both palettes in forced-colors mode", async ( { page } ) =>
{
    ...
    await expect ( consoleButton ).toHaveCSS (
        "border-color", await applicationButton.evaluate ( element => getComputedStyle ( element ).borderColor ),
    );
...
for ( const theme of [ "Light", "Dark" ] as const )
{
    test ( "Console severity matching is transactional, persistent, and scoped to inline actions in " + theme, async ( { page } ) =>
    {
        ...
        for ( const severity of [ "message", "warning", "error" ] as const )
        {
            ...
            const nearestColor = { message: "Green", warning: "Blue", error: "Red" } [ severity ];
            dialog = await openAppearance ( page );
            await dialog.getByRole ( "combobox", { name: "Application Buttons" } ).selectOption ( nearestColor );
            await dialog.getByRole ( "button", { name: "Apply", exact: true } ).click ();
            const applicationPalette = await readButtonPalette ( page, clear );
            const consolePalette = await readButtonPalette ( page, button );
            expect ( consolePalette, theme + " " + severity + " matches " + nearestColor ).toEqual ( applicationPalette );
