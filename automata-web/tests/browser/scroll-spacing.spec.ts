...
test ( "intrinsic Transition Table and fixed Console columns stay flush with their scrollport", async ( { page } ) =>
{
    await pullExample ( page );
    await openEditorNode ( page );
    ...
    for ( const inset of [ 4, 8 ] )
    {
        ...
        await expect ( consoleTable.locator ( ".console-row:last-child .console-context button" ) )
