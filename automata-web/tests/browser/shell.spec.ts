...
test ( "State Machine Info stacks group boxes and aligns values from the longest label", async ( { page } ) =>
{
    ...
    // The shared container carries the page inset so the Validate button aligns with the
    // content above it.

    expect ( layout.rightAlignmentDifference ).toBeCloseTo ( 0, 3 );
...
test ( "Solver fills its Sequence pane and infers a complete model from human-friendly event/action observations", async ( { page } ) =>
{
    ...
    // The shared container owns the page inset so Solve aligns with the
    // Sequence editor.

    expect ( pageCommandBarAlignment ).toBeCloseTo ( 0, 3 );
...
test ( "Editor catalogs share Name and Description columns and States defaults to 60/40", async ( { page } ) =>
{
    await createNewDocument ( page );

    // Process each target from the catalog pages collection in order.

    for ( const catalogPage of [
        { accessibleName: "States", pageName: "States" },
        { accessibleName: "Events", pageName: "Events" },
        { accessibleName: "Reusable actions", pageName: "Actions" },
    ] as const )
    {
        await page.getByRole ( "treeitem", { name: catalogPage.pageName, exact: true } ).click ();

        const catalog = page.getByRole ( "grid", { name: catalogPage.accessibleName } );

        await expect ( catalog.getByRole ( "columnheader" ) ).toHaveText ( [ "Name", "Description" ] );
    }

    await page.getByRole ( "treeitem", { name: "States", exact: true } ).click ();

    await expect.poll ( async () => await page.locator ( ".states-editor" ).evaluate ( element =>
    {
        // Initialize the local values needed by this operation.

        const stateListWidth   = ( element.querySelector ( ".states-list-pane" ) as HTMLElement )
            .getBoundingClientRect ().width;
        const actionTabsWidth = ( element.querySelector ( ".state-association-pane" ) as HTMLElement )
            .getBoundingClientRect ().width;

        // Return the state-list fraction of the two pane widths.

        return stateListWidth / ( stateListWidth + actionTabsWidth );
    } ) ).toBeCloseTo ( 0.6, 2 );
} );
...
test ( "Phase 3 imports and exports Editor CSV data through the nested File menus", async ( { page } ) =>
{
    ...
    await page.getByRole ( "treeitem", { name: "States", exact: true } ).click ();
    await expect ( page.getByRole ( "gridcell", { name: "state_one" } ) ).toBeVisible ();
    await expect ( page.getByRole ( "gridcell", { name: "state_two" } ) ).toBeVisible ();
...
test ( "Phase 3 rejects an invalid CSV import atomically and reports it in the Console and a modal", async ( { page } ) =>
{
    ...
    await page.getByRole ( "treeitem", { name: "States", exact: true } ).click ();
    await expect ( page.locator ( ".states-list-pane [role='gridcell']" ) ).toHaveCount ( 0 );
} );
...
test ( "AC-001 and AC-020 round-trip JSON with one download per fallback save", async ( { page } ) =>
{
    ...
    const transitionDialog = page.getByRole ( "dialog", { name: /^(Add|Edit) Transition$/u } );
...
test ( "AL-UI-012 aligns every standard action panel with the content above it", async ( { page } ) =>
{
    ...
    // Every standard page panel lines its last button up with its sibling content's
    // right edge.

    const editorPages =
    [
        { content: ".data-grid", tree: "Events" },
        { content: ".data-grid", tree: "Actions" },
    ...
    // The Chart footer shares the page inset and aligns with the Canvas.
...
test ( "AL-UI-013 lets the Chart Palette and Canvas fill their regions", async ( { page } ) =>
{
    ...
    expect ( regions.canvas.right ).toBe ( regions.chartPage.right );
} );
...
test ( "AL-UI-011 aligns the Entry and Exit action lists with their tab control border", async ( { page } ) =>
{
    ...
    // Retain top separation below the tabs; the page owns the footer bottom inset.

    expect ( insets ).toEqual ( { left: 0, right: 0, verticalPadding: "10px/0px" } );
} );
...
test ( "About groups its licences and shipped release notes in accessible tabs", async ( { browserName, page } ) =>
{
    await page.getByRole ( "menuitem", { name: "Help", exact: true } ).click ();
    await page.getByRole ( "menuitem", { name: "About Automata Lab" } ).click ();

    const dialog      = page.getByRole ( "dialog", { name: "About Automata Lab" } );
    const description = dialog.getByText ( "deterministic state transducers", { exact: false } );
    const repository  = dialog.getByRole ( "link", { name: "GitHub - Automata Lab", exact: true } );
    const tabs        = dialog.getByRole ( "tablist", { name: "About content" } );

    await expect ( dialog ).toBeVisible ();
    await expect ( dialog.getByText ( "Version 1.8.0", { exact: true } ) ).toBeVisible ();
    await expect ( description ).toBeVisible ();
    await expect ( repository ).toBeVisible ();
    await expect ( repository ).toHaveAttribute ( "href", "https://github.com/rohingosling/automata-lab" );
    ...
    const repositoryBounds  = await repository.boundingBox ();
    const tabsBounds        = await tabs.boundingBox ();

    if ( descriptionBounds === null || repositoryBounds === null || tabsBounds === null )
    {
        throw new Error ( "The About description, repository link, and tabs require visible bounds." );
    }

    expect ( repositoryBounds.y ).toBeGreaterThanOrEqual ( descriptionBounds.y + descriptionBounds.height );
    expect ( repositoryBounds.y + repositoryBounds.height ).toBeLessThanOrEqual ( tabsBounds.y );
    await expect ( dialog.getByRole ( "button", { name: "Close dialog", exact: true } ) ).toBeFocused ();
    await page.keyboard.press ( "Tab" );

    if ( browserName === "webkit" )
    {
        // This WebKit test engine skips native links with Tab; verify focusability directly.

        const licencesTab = dialog.getByRole ( "tab", { name: "Licences", exact: true } );

        await licencesTab.focus ();
        await expect ( licencesTab ).toBeFocused ();
        await repository.focus ();
    }

    await expect ( repository ).toBeFocused ();
    await page.keyboard.press ( "Tab" );
    await expect ( dialog.getByRole ( "tab", { name: "Licences", exact: true } ) ).toBeFocused ();
    ...
    await page.screenshot ( { path: test.info ().outputPath ( "about-desktop.png" ) } );
    ...
    const releaseNotesLayout = await dialog.evaluate ( element =>
    {
        ...
        const textBox = element.querySelector<HTMLTextAreaElement> ( "#about-release-notes" );
        const context = document.createElement ( "canvas" ).getContext ( "2d" );
        if ( textBox === null || context === null )
        {
            throw new Error ( "The Release Notes textbox and text measurement context are required." );
        }
        const textBounds = textBox.getBoundingClientRect ();
        const style = getComputedStyle ( textBox );
        context.font = style.font;
        const availableWidth = textBox.clientWidth - parseFloat ( style.paddingLeft ) - parseFloat ( style.paddingRight );
        const eightyCharacterWidth = context.measureText ( "0".repeat ( 80 ) ).width;
        ...
        return {
            ...
            availableWidth,
            eightyCharacterWidth,
            horizontalScroll: textBox.scrollWidth > textBox.clientWidth,
        };
    } );

    expect ( releaseNotesLayout.bottomInset ).toBeCloseTo ( 10, 0 );
    expect ( releaseNotesLayout.textHeight ).toBeGreaterThan ( 200 );
    expect ( releaseNotesLayout.availableWidth ).toBeGreaterThanOrEqual ( releaseNotesLayout.eightyCharacterWidth );
    expect ( releaseNotesLayout.horizontalScroll ).toBe ( false );
    await page.screenshot ( { path: test.info ().outputPath ( "about-release-notes.png" ) } );
    await expect ( dialog.getByText ( "Third-Party Runtime Notices" ) ).toHaveCount ( 0 );
    await dialog.getByRole ( "tab", { name: "Licences" } ).click ();

    // Licence terms remain complete in the dialog, below the separate repository link.

    await expect ( dialog.getByRole ( "tabpanel" ).locator ( "a" ) ).toHaveCount ( 0 );
    ...
    for ( const size of [ { height: 450, width: 720 }, { height: 720, width: 320 }, { height: 640, width: 320 } ] )
    {
        ...
        if ( size.width === 320 && size.height === 640 )
        {
            await page.screenshot ( { path: test.info ().outputPath ( "about-narrow.png" ) } );
        }
    }
} );

test ( "retired master hiding restores navigation and the editor workspace at every width", async ( { page } ) =>
{
    await page.evaluate ( () =>
    {
        localStorage.setItem ( "automata-lab.preferences.v1", JSON.stringify (
            { version: 1, preferences: { masterPanelVisible: false, masterPanelWidth: 280, theme: "Dark" } },
        ) );
    } );
    await page.reload ();
    const master = page.locator ( ".master-panel" );
    const detail = page.locator ( ".detail-region" );
    await expect ( master ).toBeVisible ();
    await expect ( master.locator ( ".panel-title-bar button" ) ).toHaveCount ( 0 );
    await expect ( detail ).toBeVisible ();
    expect ( await detail.evaluate ( element => element.getBoundingClientRect ().width ) ).toBeGreaterThan ( 500 );
    await createNewDocument ( page );
    await openEditorNode ( page );
    await page.getByRole ( "treeitem", { name: "States", exact: true } ).click ();
    await expect ( page.locator ( ".states-list-pane" ).getByRole ( "button", { name: "Add", exact: true } ) ).toBeVisible ();
    for ( const width of [ 980, 375 ] )
    {
...
test ( "Console destination buttons and headers stay aligned across viewport sizes", async ( { page } ) =>
{
    await page.locator ( "[data-toolbar-entry='toolbar-pull']" ).click ();
    await expect ( page.locator ( ".console-code", { hasText: "HOSTED_MODEL_PULLED" } ) ).toHaveCount ( 1 );
    await page.locator ( "[data-toolbar-entry='toolbar-simulator']" ).click ();
    ...
    const buttons     = consoleGrid.locator ( ".console-context button" );

    await expect ( buttons.filter ( { hasText: /^Simulator$/ } ).first () ).toBeVisible ();
    await expect ( consoleGrid.getByRole ( "columnheader" ) )
        .toHaveText ( [ "Time", "Severity", "Code", "Source", "Message", "Goto" ] );
    await consoleGrid.getByRole ( "button", { name: "Editor", exact: true } ).first ().click ();
    await expect ( page.getByRole ( "treeitem", { name: "Editor", exact: true } ) )
        .toHaveAttribute ( "aria-selected", "true" );

    const codeGrip = consoleGrid.getByRole ( "separator", { name: "Resize Code column" } );
    await expect ( codeGrip ).toHaveAttribute ( "aria-valuenow", "250" );
    const gripBounds = await codeGrip.boundingBox ();
    if ( gripBounds === null )
    {
        throw new Error ( "Code column grip is not visible" );
    }
    await page.mouse.move ( gripBounds.x + gripBounds.width / 2, gripBounds.y + gripBounds.height / 2 );
    await page.mouse.down ();
    await page.mouse.move ( gripBounds.x + gripBounds.width / 2 + 64, gripBounds.y + gripBounds.height / 2 );
    await page.mouse.up ();
    await expect ( codeGrip ).toHaveAttribute ( "aria-valuenow", "314" );
    await expect ( consoleGrid.locator ( ".console-code" ).first () ).toHaveCSS ( "width", "314px" );
    await codeGrip.press ( "ArrowLeft" );
    await expect ( codeGrip ).toHaveAttribute ( "aria-valuenow", "306" );
    await codeGrip.press ( "Home" );
    await expect ( codeGrip ).toHaveAttribute ( "aria-valuenow", "80" );
    await codeGrip.press ( "End" );
    await expect ( codeGrip ).toHaveAttribute ( "aria-valuenow", "1200" );
    await codeGrip.dblclick ();
    await expect ( codeGrip ).toHaveAttribute ( "aria-valuenow", "250" );
    ...
    for ( const width of [ 1440, 1000, 600 ] )
    {
        ...
        const buttonWidths = await buttons.evaluateAll ( elements =>
            elements.map ( element => element.getBoundingClientRect ().width ) );
        expect ( buttonWidths.length ).toBeGreaterThan ( 1 );
        expect ( buttonWidths.every ( buttonWidth => buttonWidth === 128 ) ).toBe ( true );

        const columnOffsets = await consoleGrid.evaluate ( grid =>
        {
            const headers = Array.from ( grid.querySelectorAll ( "[role='columnheader']" ) );
            return Array.from ( grid.querySelectorAll ( ".console-row" ) ).map ( row =>
                Array.from ( row.children ).map ( ( cell, index ) =>
                    cell.getBoundingClientRect ().left - ( headers [ index ]?.getBoundingClientRect ().left ?? 0 ) ) );
        } );
        expect ( columnOffsets.every ( offsets => offsets.every ( offset => Math.abs ( offset ) < 1 ) ) )
            .toBe ( true );

        await consoleGrid.evaluate ( grid =>
        {
            grid.scrollTop = grid.scrollHeight;
            grid.scrollLeft = grid.scrollWidth;
        } );
        const headerOffset = await consoleGrid.evaluate ( grid =>
            ( grid.querySelector ( ".console-column-header" )?.getBoundingClientRect ().top ?? 0 )
            - grid.getBoundingClientRect ().top );
        expect ( Math.abs ( headerOffset ) ).toBeLessThan ( 2 );
        await expect ( buttons.last () ).toBeVisible ();
    }
} );

test ( "Console entity destinations reveal affected rows and fall back for historical references", async ( { page } ) =>
{
    await page.addInitScript ( () =>
    {
        Object.defineProperty ( window, "showOpenFilePicker", { configurable: true, value: undefined } );
    ...
    const model = {
        ...
        settings: { name: "Console Navigation", description: "", version: "1.0.0" },
        state_machine: {
            initial_state: "state_0",
            states: Array.from ( { length: 130 }, ( _, index ) => ( { name: "state_" + index, description: "" } ) ),
            events: [ { name: "first_event", description: "" }, { name: "last_event", description: "" }, { name: "transition_event", description: "" } ],
            actions: [ { name: "first_action", description: "" }, { name: "last_action", description: "" } ],
            state_actions: { entry: [], exit: [] }, transition_table: [
                { state: "state_0", event: "first_event", state_next: "state_1" },
                { state: "state_0", event: "transition_event", state_next: "state_2" },
            ],
        },
        chart: {
    ...
    const openModel = async (): Promise<void> =>
    {
        ...
        await page.locator ( "[data-toolbar-entry='toolbar-open']" ).click ();
        await ( await chooser ).setFiles ( { name: "console-navigation.json", mimeType: "application/json", buffer: Buffer.from ( JSON.stringify ( model ) ) } );
    };
    await openModel ();
    await page.getByRole ( "menuitem", { name: "File", exact: true } ).click ();
    await page.getByRole ( "menuitem", { name: "Validate State Machine", exact: true } ).click ();
    // Diagnostic batches are bounded; choose a state that is beyond the initial rendered grid batch
    // but still present in the retained diagnostic batch.
    for ( const [ name, button ] of [ [ "state_129", "State" ], [ "last_event", "Event" ], [ "last_action", "Action" ] ] as const )
    {
        const entry = page.locator ( ".console-row" ).filter ( { hasText: "'" + name + "'" } ).last ();
        await entry.getByRole ( "button", { name: button, exact: true } ).click ();
        const selected = page.locator ( ".detail-region [role='row'][aria-selected='true']" ).first ();
        await expect ( selected ).toContainText ( name ?? "" );
        await expect ( selected.getByRole ( "gridcell" ).first () ).toBeFocused ();
        await expect ( selected ).toBeInViewport ();
    }
    await page.getByRole ( "treeitem", { name: "Transition Table", exact: true } ).click ();
    await page.getByRole ( "button", { name: "Open selection list: Event 2", exact: true } ).click ();
    await page.getByRole ( "option", { name: "first_event", exact: true } ).click ();
    await page.getByRole ( "dialog", { name: "Error", exact: true } ).getByRole ( "button", { name: "OK", exact: true } ).click ();
    await page.getByRole ( "checkbox", { name: "Follow Tail", exact: true } ).check ();
    await page.locator ( ".console-row-error" ).last ().getByRole ( "button", { name: "Transition Table", exact: true } ).click ();
    await expect ( page.locator ( ".detail-region [role='row'][aria-selected='true']" ).first () ).toHaveAttribute ( "aria-rowindex", "3" );
    await expect ( page.locator ( ".detail-region [role='row'][aria-selected='true'] [role='gridcell']" ).first () ).toBeFocused ();
    await page.getByRole ( "checkbox", { name: "Follow Tail", exact: true } ).uncheck ();

    const historicalEvent = page.locator ( ".console-row" ).filter ( { hasText: "'last_event'" } ).first ();
    await openModel ();
    await expect ( page.getByRole ( "treeitem", { name: "Editor", exact: true } ) ).toHaveAttribute ( "aria-selected", "true" );
    await historicalEvent.getByRole ( "button", { name: "Event", exact: true } ).click ();
    await expect ( page.locator ( ".detail-region [role='row'][aria-selected='true']" ).first () ).toContainText ( "first_event" );

    for ( const [ theme, color ] of [ [ "Dark", "rgb(64, 64, 64)" ], [ "Light", "rgb(191, 191, 191)" ] ] as const )
    {
        await page.locator ( "[data-toolbar-entry='toolbar-theme']" ).click ();
        await page.getByRole ( "menuitemradio", { name: theme, exact: true } ).click ();
        await expect ( page.getByRole ( "separator", { name: "Resize Code column" } ) ).toHaveCSS ( "background-color", color ?? "" );
    }
} );
