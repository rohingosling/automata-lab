...
test ( "Help opens the documentation home from the combined production artifact", async ( { page } ) =>
{
    await page.goto ( "./" );
    ...
    await expect ( documentationPage ).toHaveURL (
        "http://127.0.0.1:4187/automata-lab/docs/",
    );
    await expect ( documentationPage.getByRole ( "heading", { level: 1, name: "Automata Lab Overview" } ) )
        .toBeVisible ();
...
test ( "Phase 10 serves direct and reloaded guide routes with local runtime assets", async ( { page } ) =>
{
    ...
    const launchApplicationLink = page.getByRole ( "link", { name: "Launch Automata Lab", exact: true } ).first ();

    await expect ( launchApplicationLink ).toHaveAttribute ( "href", "/automata-lab/" );
    await expect ( launchApplicationLink ).not.toHaveAttribute ( "target", "_blank" );
    await launchApplicationLink.click ();
    await expect ( page ).toHaveURL ( "http://127.0.0.1:4187/automata-lab/" );
    await expect ( page.getByRole ( "menubar", { name: "Application menu" } ) ).toBeVisible ();

    // Finish worker startup before navigating away so WebKit does not report its cancellation.
...
test ( "TS5 Help downloads the current terminal example from the combined artifact", async ( { page } ) =>
{
    await page.goto ( "./docs/user-guide/files-and-data-exchange.html" );
    const pending = page.waitForEvent ( "download" );
    await page.getByRole ( "link", { name: "Download the current terminal-state light-switch example" } ).click ();
    const download = await pending;
    expect ( download.suggestedFilename () ).toBe ( "state-machine-terminal-states.json" );
    const path = await download.path ();
    if ( path === null )
    {
        throw new Error ( "The example download must be available." );
    }
    const document = JSON.parse ( await readFile ( path, "utf8" ) );
    expect ( document.file_version ).toBe ( "1.4.0" );
    expect ( document.state_machine.states.map ( ( state: { terminal_state: boolean } ) => state.terminal_state ) )
