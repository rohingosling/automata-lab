...
function createLoopDocumentText (): string
{
    ...
    const document: AutomataDocument = 
    {
        ...
        stateMachine:
        {
            ...
            states:       [ { terminalState: false, name: "s", description: "" } ],
            transitionTable:
            [
...
}
{
    ...
    const draft  = createEmptyAuthoringDraft ();
    const states = Array.from ( { length: MAXIMUM_STATE_COUNT }, ( _value, index ) => ( { terminalState: false,
        name:        `state_${index}`,
        description: "",
    } ) );
...
): ServerRequestEnvelope
{
    ...
    return {
        ...
        sessionId: null,
        payload: operation === "simulation.start" ? { enableTerminalStates: false } : {},
