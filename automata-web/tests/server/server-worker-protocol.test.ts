...
function createRequest ( operation: ServerOperation ): Record<string, unknown>
{
    ...
    if ( operation === "simulation.start" || operation === "simulation.reset" )
    {
        payload = { enableTerminalStates: false };
    }
    return {
...
function createSessionSnapshot (): Record<string, unknown>
{
    ...
    return {
        ...
        executionPolicy: { enableTerminalStates: false },
...
function createResult ( operation: ServerOperation ): unknown
{
    ...
    switch ( operation )
    {
        ...
        case "simulation.step":
            ...
            return {
                session: { ...createSessionSnapshot (), initialEntryActionsPending: false, processedEventCount: 1 },
...
describe ( "terminal /2 exact snapshot contracts", () =>
{
    it.each ( [
        { kind: "active", reason: "terminal_state" },
        { kind: "terminated", reason: "other", state: "state_ready" },
        { kind: "terminated", reason: "terminal_state", state: "" },
        { kind: "terminated", reason: "terminal_state", state: " ", extra: true },
        { kind: "terminated", reason: "terminal_state", state: "x".repeat ( 257 ) },
        { kind: "terminated", reason: "terminal_state", state: "wrong_state" },
        { kind: "terminated", state: "state_ready" },
        null,
    ] ) ( "rejects malformed or inconsistent execution status %#", executionStatus =>
    {
        const response = createSuccessEnvelope ( "simulation.run" );
        response [ "result" ] = {
            session: { ...createSessionSnapshot (), executionPolicy: { enableTerminalStates: true },
                initialEntryActionsPending: false, executionStatus },
            consumedEventCount: 0, emittedActions: [], warnings: [],
        };
        expect ( decodeServerOutboundEnvelope ( response ).isSuccessful ).toBe ( false );
    } );

    it.each ( [ undefined, {}, { enableTerminalStates: "true" },
        { enableTerminalStates: true, extra: true }, null ] ) ( "requires exact boolean policy %#", executionPolicy =>
    {
        const response = createSuccessEnvelope ( "simulation.start" );
        response [ "result" ] = { ...createSessionSnapshot (), executionPolicy };
        expect ( decodeServerOutboundEnvelope ( response ).isSuccessful ).toBe ( false );
    } );

    it.each ( [ "disabled", "pending" ] ) ( "rejects terminal status with %s execution", invalidity =>
    {
        const response = createSuccessEnvelope ( "simulation.run" );
        response [ "result" ] = {
            session: { ...createSessionSnapshot (),
                executionPolicy: { enableTerminalStates: invalidity !== "disabled" },
                initialEntryActionsPending: invalidity === "pending",
                executionStatus: { kind: "terminated", reason: "terminal_state", state: "state_ready" } },
            consumedEventCount: 0, emittedActions: [], warnings: [],
        };
        expect ( decodeServerOutboundEnvelope ( response ).isSuccessful ).toBe ( false );
    } );
