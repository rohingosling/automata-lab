...
describe ( "Automata Lab file codec", () =>
{
    ...
    it ( "round-trips two source states related to the same visual terminal indicator", () =>
    {
        ...
        const document: AutomataDocument = {
            ...sourceDocument.document,
            stateMachine: { ...sourceDocument.document.stateMachine, states:
                sourceDocument.document.stateMachine.states.map ( state => ( { ...state,
                    terminalState: state.name === firstState || state.name === secondState } ) ) },
            chart:
            {
                ...sourceDocument.document.chart,
                indicators:
