...
import
{
    ...
    chartTransitionExportPaint,
    normalizeChartTransitionExportStyles,
...
{
    it ( "preserves wrapped remembered event label text in the sanitized SVG scene", () =>
    {
        const root = document.createElement ( "div" );

        root.innerHTML = '<svg><g class="react-flow__edge-textwrapper"><text x="100" y="120">' +
            '<tspan x="100" dy="-0.5em">event_</tspan><tspan x="100" dy="1em">remembered</tspan>' +
            '</text></g></svg>';
        sanitizeChartSvgClone ( root );

        expect ( root.querySelectorAll ( "tspan" ) ).toHaveLength ( 2 );
        expect ( root.textContent ).toBe ( "event_remembered" );
        expect ( root.querySelector ( "tspan" ) ).toHaveAttribute ( "dy", "-0.5em" );
    } );

    it ( "exports identical normal draft paint after selection and removes the curve hit target", () =>
    {
        const source = document.createElement ( "div" );

        source.innerHTML = '<div class="react-flow__node-draftTransition selected">' +
            '<div class="chart-draft-transition-node chart-transition-selected" ' +
            'style="box-shadow: 0 0 3px blue; outline: 3px solid blue; border: 1px solid blue">' +
            '<svg><defs><marker><polygon style="color: blue; fill: blue" /></marker></defs>' +
            '<path class="chart-draft-transition-path" d="M 200 200 C 210 200 290 200 300 200" ' +
            'transform="translate(-150 -150)" style="stroke: blue; stroke-width: 3; ' +
            '--chart-transition-color: rgb(80, 81, 82); --chart-transition-width: 1.8" />' +
            '<g><path class="chart-draft-transition-hit" /></g></svg></div></div>';
        const path = source.querySelector<SVGPathElement> ( ".chart-draft-transition-path" )!;
        const clone = source.cloneNode ( true ) as HTMLElement;

        expect ( chartTransitionExportPaint ( path ) ).toEqual ( { color: "rgb(80, 81, 82)", width: 1.8 } );
        normalizeChartTransitionExportStyles ( source, clone );

        const exportedPath = clone.querySelector<SVGPathElement> ( ".chart-draft-transition-path" )!;
        const exportedDraft = clone.querySelector<HTMLElement> ( ".chart-draft-transition-node" )!;
        const marker = clone.querySelector<SVGElement> ( "polygon" )!;

        expect ( exportedPath.style.stroke ).toBe ( "rgb(80, 81, 82)" );
        expect ( exportedPath.style.strokeWidth ).toBe ( "1.8" );
        expect ( marker.style.fill ).toBe ( "rgb(80, 81, 82)" );
        expect ( exportedPath ).toHaveAttribute ( "transform", "translate(-150 -150)" );
        expect ( clone.querySelector ( ".chart-draft-transition-hit, .chart-transition-selected, .selected" ) )
            .toBeNull ();
        expect ( exportedDraft.style.boxShadow ).toBe ( "none" );
        expect ( exportedDraft.style.outline ).toBe ( "none" );
        expect ( source.querySelector ( ".chart-transition-selected" ) ).not.toBeNull ();
        expect ( path.style.stroke ).toBe ( "blue" );

        path.style.stroke = "rgb(80, 81, 82)";
        path.style.strokeWidth = "1.8";
        const unselectedClone = source.cloneNode ( true ) as HTMLElement;

        normalizeChartTransitionExportStyles ( source, unselectedClone );
        expect ( unselectedClone.innerHTML ).toBe ( clone.innerHTML );
    } );
