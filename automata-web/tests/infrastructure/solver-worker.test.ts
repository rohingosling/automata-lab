...
describe ( "Solver Worker protocol boundary", () =>
{
    ...
    it ( "preserves attached draft references across the strict worker boundary", () =>
    {
        const message = structuredClone ( resultMessage ( createJob ( "attachments" ) ) );
        if ( message.result.status !== "success" )
        {
            throw new Error ( "The attachment fixture requires a successful candidate." );
        }
        const stateName = message.result.candidate.stateMachine.states [ 0 ]?.name;
        const eventName = message.result.candidate.stateMachine.events [ 0 ]?.name;
        if ( stateName === undefined || eventName === undefined )
        {
            throw new Error ( "The candidate requires a state." );
        }
        Object.assign ( message.result.candidate.chart, {
            draftTransitions: [ { id: 7, source: { x: 0, y: 0 }, target: { x: 100, y: 0 },
                sourceState: stateName, targetState: stateName, rememberedEvent: eventName, rememberedTransitionIndex: 0 } ],
        } );
        const decoded = decodeSolverWorkerMessage ( message );
        expect ( decoded?.kind ).toBe ( "result" );
        if ( decoded?.kind !== "result" || decoded.result.status !== "success" )
        {
            throw new Error ( "The attached candidate must survive decoding." );
        }
        expect ( decoded.result.candidate.chart.draftTransitions ).toEqual (
            message.result.candidate.chart.draftTransitions,
        );
        expect ( Object.isFrozen ( decoded.result.candidate.chart.draftTransitions [ 0 ] ) ).toBe ( true );
        for ( const invalid of [ { sourceState: 42 }, { rememberedEvent: 42 }, { rememberedEvent: "missing" },
            { rememberedTransitionIndex: -1 }, { rememberedTransitionIndex: 50_000 },
            { rememberedEvent: null, rememberedTransitionIndex: 0 } ] )
        {
            const malformed = structuredClone ( message );
            if ( malformed.result.status !== "success" )
            {
                throw new Error ( "The malformed fixture requires a candidate." );
            }
            Object.assign ( malformed.result.candidate.chart.draftTransitions [ 0 ] ?? {}, invalid );
            expect ( decodeSolverWorkerMessage ( malformed ) ).toBeNull ();
        }
    } );

    it ( "preserves indicator draft identities and direct notation through the strict Worker boundary", () =>
    {
        const message = structuredClone ( resultMessage ( createJob ( "indicator-attachments" ) ) );
        if ( message.result.status !== "success" )
        {
            throw new Error ( "The indicator fixture requires a candidate." );
        }
        Object.assign ( message.result.candidate.chart, {
            indicators:
            {
                initialStateIndicator: { x: 20, y: 30, state: null },
                terminalStateIndicators: [ { id: 4, x: 400, y: 300 } ],
                terminalStateTransitions: [],
                indicatorTransitions: [ { id: 3, source: { kind: "initial" }, target: { kind: "terminal", id: 4 } } ],
            },
            draftTransitions: [ { id: 8, source: { x: 20, y: 30 }, target: { x: 100, y: 100 },
                sourceIndicator: { kind: "initial" } } ],
        } );
        const decoded = decodeSolverWorkerMessage ( message );
        expect ( decoded ).not.toBeNull ();
        if ( decoded?.kind !== "result" || decoded.result.status !== "success" )
        {
            throw new Error ( "The indicator candidate must survive decoding." );
        }
        expect ( decoded.result.candidate.chart ).toEqual ( message.result.candidate.chart );
        expect ( Object.isFrozen ( decoded.result.candidate.chart.draftTransitions [ 0 ]?.sourceIndicator ) ).toBe ( true );
        for ( const invalid of [ { kind: "terminal", id: 999 }, { kind: "initial", extra: true }, { kind: "unknown" } ] )
        {
            const malformed = structuredClone ( message );
            if ( malformed.result.status !== "success" )
            {
                throw new Error ( "The fixture requires a candidate." );
            }
            Object.assign ( malformed.result.candidate.chart.draftTransitions [ 0 ]!, { sourceIndicator: invalid } );
            expect ( decodeSolverWorkerMessage ( malformed ) ).toBeNull ();
        }
        for ( const indicatorTransitions of [ null,
            [ { id: 3, source: { kind: "initial" }, target: { kind: "initial" } } ],
            [ { id: 3, source: { kind: "initial" }, target: { kind: "terminal", id: 99 } } ],
            [ { id: 3, source: { kind: "initial" }, target: { kind: "terminal", id: 4 } },
                { id: 4, source: { kind: "initial" }, target: { kind: "terminal", id: 4 } } ],
        ] )
        {
            const malformed = structuredClone ( message );
            if ( malformed.result.status !== "success" )
            ...
            Object.assign ( malformed.result.candidate.chart.indicators, { indicatorTransitions } );
            expect ( decodeSolverWorkerMessage ( malformed ) ).toBeNull ();
        }
    } );
