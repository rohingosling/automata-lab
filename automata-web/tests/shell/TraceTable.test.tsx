...
function renderTrace ( entries: readonly number[], followTail: boolean = true )
{
    return <TraceTable columns={ COLUMNS } emptyMessage="No entries" entries={ entries } followTail={ followTail } labelledBy="trace-heading" />;
}
...
describe ( "trace scroll clearance", () =>
{
    ...
    it ( "keeps the reading position while follow is disabled, including appends and resizes", () =>
    {
        const rendered      = render ( renderTrace ( ENTRIES, false ) );
        const scrollElement = findScrollElement ( rendered.container );

        expect ( scrollElement.scrollTop ).toBe ( 0 );
        expect ( screen.getByRole ( "cell", { name: "row_0" } ) ).toBeInTheDocument ();
        expect ( screen.queryByRole ( "cell", { name: "row_999" } ) ).not.toBeInTheDocument ();
        rendered.rerender ( renderTrace ( [ ...ENTRIES, 1000 ], false ) );
        resizeTrace ( 134, 8 );
        expect ( scrollElement.scrollTop ).toBe ( 0 );
        expect ( screen.getAllByRole ( "row" ).length ).toBeLessThan ( 50 );

        rendered.rerender ( renderTrace ( [ ...ENTRIES, 1000 ], true ) );
        expect ( scrollElement.scrollTop ).toBe ( 0 );

        scrollElement.scrollTop = scrollElement.scrollHeight - viewportHeight;
        fireEvent.scroll ( scrollElement );
        rendered.rerender ( renderTrace ( [ ...ENTRIES, 1000, 1001 ], true ) );
        expect ( scrollElement.scrollTop ).toBe ( scrollElement.scrollHeight - viewportHeight );
        expect ( screen.getByRole ( "cell", { name: "row_1001" } ) ).toBeInTheDocument ();
    } );

    it ( "does not jump on re-enable after rows have arrived while a pinned trace was disabled", () =>
    {
        const rendered      = render ( renderTrace ( ENTRIES ) );
        const scrollElement = findScrollElement ( rendered.container );
        const previousEnd   = scrollElement.scrollTop;
        const nextEntries   = [ ...ENTRIES, 1000 ];

        rendered.rerender ( renderTrace ( ENTRIES, false ) );
        rendered.rerender ( renderTrace ( nextEntries, false ) );
        expect ( scrollElement.scrollTop ).toBe ( previousEnd );
        rendered.rerender ( renderTrace ( nextEntries, true ) );
        expect ( scrollElement.scrollTop ).toBe ( previousEnd );
        resizeTrace ( 134 );
        expect ( scrollElement.scrollTop ).toBe ( previousEnd );
    } );

    it ( "resumes when enabled at the end and pauses again when the reader scrolls away", () =>
    {
        const rendered      = render ( renderTrace ( ENTRIES, false ) );
        const scrollElement = findScrollElement ( rendered.container );
        const nextEntries   = [ ...ENTRIES, 1000 ];
        ...
        scrollElement.scrollTop = scrollElement.scrollHeight - viewportHeight;
        fireEvent.scroll ( scrollElement );
        rendered.rerender ( renderTrace ( ENTRIES, true ) );
        rendered.rerender ( renderTrace ( nextEntries, true ) );
        expect ( scrollElement.scrollTop ).toBe ( scrollElement.scrollHeight - viewportHeight );

        scrollElement.scrollTop = 100;
        fireEvent.scroll ( scrollElement );
        rendered.rerender ( renderTrace ( [ ...nextEntries, 1001 ], true ) );
        expect ( scrollElement.scrollTop ).toBe ( 100 );
    } );

    it.each ( [ false, true ] ) ( "resets the virtual reading position after clearing with follow %s", followTail =>
    {
        const rendered      = render ( renderTrace ( ENTRIES, followTail ) );
        const scrollElement = findScrollElement ( rendered.container );

        scrollElement.scrollTop = 5000;
        fireEvent.scroll ( scrollElement );
        expect ( screen.queryByRole ( "cell", { name: "row_0" } ) ).not.toBeInTheDocument ();

        rendered.rerender ( renderTrace ( [], followTail ) );
        expect ( screen.getByText ( "No entries" ) ).toBeInTheDocument ();
        rendered.rerender ( renderTrace ( ENTRIES.slice ( 0, 20 ), followTail ) );

        const newScrollElement = findScrollElement ( rendered.container );

        expect ( newScrollElement ).not.toBe ( scrollElement );
        expect ( newScrollElement.scrollTop ).toBe ( followTail ? newScrollElement.scrollHeight - viewportHeight : 0 );
        expect ( screen.getByRole ( "cell", { name: followTail ? "row_19" : "row_0" } ) ).toBeInTheDocument ();
        expect ( newScrollElement.querySelector ( "table" ) ).toHaveAttribute ( "aria-rowcount", "21" );
    } );
} );