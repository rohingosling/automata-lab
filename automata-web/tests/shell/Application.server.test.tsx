...
// Server shell tests exercise document revisions; browser Chart tests verify DOM handle measurement.
// Keep the real Chart while omitting measurement that cannot run in JSDOM's layout-free document.
...
function createHostedDocument (): AutomataDocument
{
    ...
    return {
        ...
        stateMachine:
        {
            ...
            states:       [ { terminalState: false, description: "Idle state", name: "state_idle" } ],
        },
    };
}
...
describe ( "Phase 7 application server shell", () =>
{
    ...
    it ( "tests a pending Settings URL in an isolated probe and Cancel preserves the active server", async () =>
    {
        ...
        const serverGroup = screen.getByRole ( "group", { name: "Built-in Server" } );
        const serverUrl   = within ( serverGroup ).getByRole ( "textbox", { name: "URL" } );

        await user.clear ( serverUrl );
        await user.type ( serverUrl, "builtin://alternate" );
        await user.click ( within ( serverGroup ).getByRole ( "button", { name: "Test Server" } ) );
