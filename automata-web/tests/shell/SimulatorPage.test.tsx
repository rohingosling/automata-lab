...
function Harness ( properties: HarnessOverrides & { readonly onSequencesChange?: ( s: readonly SimulatorSequence[] ) => void } )
{
    ...
    return (
        <SimulatorPage
            ...
            followTraceTail   = { properties.followTraceTail ?? COMPILE_TIME_CONFIGURATION.applicationSettings.simulator.simulatorFollowTraceTail }
    ...
    it ( "asks for a document before offering sequence editing", () =>
    {
        render (
            <SimulatorPage
                ...
                draft              = { null }
                followTraceTail    = { COMPILE_TIME_CONFIGURATION.applicationSettings.simulator.simulatorFollowTraceTail }
...
describe ( "Simulator traces", () =>
{
    ...
    it ( "applies the follow preference to both transition and action traces", () =>
    {
        const scrollHeight = vi.spyOn ( HTMLElement.prototype, "scrollHeight", "get" ).mockReturnValue ( 1040 );
        const clientHeight = vi.spyOn ( HTMLElement.prototype, "clientHeight", "get" ).mockReturnValue ( 260 );
        const session = createSession (
            {
                actionTrace: Array.from ( { length: 30 }, ( _, index ) => ( {
                    action: `action_${index}`, phase: "entry" as const, state: "state_on",
                } ) ),
                transitionTrace: Array.from ( { length: 30 }, ( _, index ) => ( {
                    destinationState: "state_on",
                    event:            `event_${index}`,
                    outcome:          "TRANSITION" as const,
                    sourceState:      "state_on",
        ...
        try
        {
            const rendering = render ( <Harness followTraceTail={ false } session={ session } /> );
            const scrollElements = Array.from (
                rendering.container.querySelectorAll<HTMLElement> ( ".simulator-trace-scroll" ),
            );

            expect ( scrollElements ).toHaveLength ( 2 );
            expect ( scrollElements.map ( element => element.scrollTop ) ).toEqual ( [ 0, 0 ] );
            rendering.unmount ();

            const following = render ( <Harness followTraceTail={ true } session={ session } /> );

            expect ( Array.from ( following.container.querySelectorAll<HTMLElement> ( ".simulator-trace-scroll" ) )
                .map ( element => element.scrollTop ) ).toEqual ( [ 780, 780 ] );
        }
        finally
        {
