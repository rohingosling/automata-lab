...
import type { RuntimeExecutionPolicy } from "../../src/domain/runtime/contracts.js";
...
function createHostedDocument (): AutomataDocument
{
    ...
    return {
        ...
        stateMachine:
        {
            ...
            states:
            [
                { terminalState: false, description: "", name: "state_off" },
                { terminalState: false, description: "", name: "state_on" },
            ],
            transitionTable:
            [
...
class SimulatorGateway extends BrowserServerWorkerGateway implements ServerGateway
{
    ...
    public override async startSession ( executionPolicy: RuntimeExecutionPolicy ): Promise<ServerGatewayResult<HostedSessionDto>>
    {
        this.startSessionRequestCount++;
        this.pinnedModel    = this.compiledModel;
        this.pinnedRevision = this.modelRevision;
        this.runtimeSession = resetRuntimeSession ( this.compiledModel, executionPolicy );
    ...
    public override async resetSession ( _sessionId: string, executionPolicy: RuntimeExecutionPolicy ): Promise<ServerGatewayResult<HostedSessionDto>>
    {
        // Initialize the local values needed by this operation.

        const pinnedModel = this.requirePinnedModel ();

        this.runtimeSession = resetRuntimeSession ( pinnedModel, executionPolicy );
    ...
    private snapshot ( consumedEventCount: number ): HostedSessionDto
    {
        ...
        return {
            ...
            executionPolicy:            this.runtimeSession.executionPolicy,
            executionStatus:            this.runtimeSession.executionStatus,
        ...
        gateway.replaceHostedDocument (
            {
                ...
                stateMachine:
                {
                    ...
                    states: [ ...hostedDocument.stateMachine.states, { terminalState: false, description: "", name: "state_extra" } ],
                },
            },
        );
...
class DeferredSimulatorGateway extends SimulatorGateway
{
    public releaseRun: () => void = () => {};
    ...
    {
        const result = await super.runSession ( request );
        await this.runGate;
        return result;
    }
}

describe ( "terminal execution response races", () =>
{
    beforeEach ( () =>
    {
        window.localStorage.clear ();
    ...
    it.each ( [ "sequence", "raw_editor" ] ) ( "retains the current buffer cursor after changing %s", async change =>
    {
        const document = createHostedDocument ();
        const gateway = new DeferredSimulatorGateway ( { ...document, simulator: { sequences: [
            { name: "first", description: "", sequence: [ "event_toggle", "event_toggle", "event_toggle" ] },
            { name: "second", description: "", sequence: [ "event_toggle", "event_toggle" ] },
        ] } } );
        const user = await openSimulatorWithHostedModel ( gateway );
        await user.click ( screen.getByRole ( "button", { name: "Start Session" } ) );
        await user.click ( screen.getByRole ( "button", { name: "Run" } ) );
        ...
        if ( change === "sequence" )
        {
            await user.selectOptions ( screen.getByRole ( "listbox", { name: "Event Sequences" } ), "1" );
        }
        else
        {
            const editor = screen.getByRole ( "textbox", { name: "Editor" } );
        ...
        await act ( async () => gateway.releaseRun () );
        await waitFor ( () => expect ( transitionTraceRows () ).toHaveLength ( 3 ) );
        expect ( screen.getByRole ( "listbox", { name: "Buffer Position" } ) ).toHaveValue ( "0" );
        expect ( screen.getByRole ( "button", { name: "Run" } ) ).toBeEnabled ();
    } );
} );

class CorruptedTerminalGateway extends SimulatorGateway
{
    public override async runSession (
        request: HostedSessionEventRequest,
    ): Promise<ServerGatewayResult<HostedSessionOperationResult>>
    {
        ...
        return result.isSuccessful
            ? successful ( { ...result.value, consumedEventCount: request.eventBuffer.length + 1 } )
            : result;
    }
}

describe ( "terminal notification validation", () =>
{
    beforeEach ( () =>
    {
        window.localStorage.clear ();
    ...
    it ( "does not publish terminal status or a reached message for an invalid execution response", async () =>
    {
        const document = createHostedDocument ();
        const terminalModel = { ...document, stateMachine: { ...document.stateMachine,
            states: document.stateMachine.states.map ( state => ( { ...state, terminalState: true } ) ),
        } };
        const prepared = new AutomataDocumentCodec ().open ( serializeCanonicalDocument ( terminalModel ).text );
        if ( !prepared.isSuccessful )
        {
            throw new Error ( "The terminal fixture must prepare successfully." );
        }
        const user = await openSimulatorWithHostedModel ( new CorruptedTerminalGateway ( prepared.document ) );
        await user.click ( screen.getByRole ( "menuitem", { name: "File" } ) );
        await user.click ( screen.getByRole ( "menuitem", { name: "Settings" } ) );
        await user.click ( screen.getByRole ( "option", { name: "Simulator" } ) );
        await user.click ( screen.getByRole ( "checkbox", { name: "Enable Terminal States" } ) );
        await user.click ( screen.getByRole ( "button", { name: "Apply" } ) );
        await user.click ( screen.getByRole ( "button", { name: "Start Session" } ) );
        await user.click ( screen.getByRole ( "button", { name: "Run" } ) );
        await screen.findAllByText ( "SERVER_RESPONSE_INVALID" );
        expect ( screen.queryByText ( "TERMINAL_STATE_REACHED" ) ).not.toBeInTheDocument ();
        expect ( within ( screen.getByRole ( "contentinfo", { name: "Application status" } ) ).queryByText ( "Terminal state reached" ) )
            .not.toBeInTheDocument ();
        expect ( transitionTraceRows () ).toHaveLength ( 0 );
    } );
} );