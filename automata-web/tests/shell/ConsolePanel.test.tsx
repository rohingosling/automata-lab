...
const ENTRIES: readonly ConsoleEntry[] = [
    ...
    {
        code: "MODEL_NOTE",
        context: { label: "Second message", route: "states" },
...
describe ( "AL-CON-001 structured Console panel", () =>
{
    ...
    it ( "keeps duplicate codes independently selectable and routes contextual entries", async () =>
    {
        ...
        expect ( screen.getAllByRole ( "columnheader" ).map ( header => header.textContent ) )
            .toEqual ( [ "Time", "Severity", "Code", "Source", "Message", "Goto" ] );
        ...
        expect ( navigate ).toHaveBeenCalledWith ( ENTRIES [ 1 ]?.context );
        expect ( screen.queryByRole ( "button", { name: "Second message" } ) ).not.toBeInTheDocument ();
        rows [ 0 ]?.focus ();
    ...
    it ( "progressively renders bounded Console rows while retaining the full accessible row count", () =>
    {
        ...
        expect ( screen.getAllByRole ( "row" ).slice ( 1 ) ).toHaveLength ( initialItemCount );
        expect ( grid ).toHaveAttribute ( "aria-rowcount", String ( entries.length + 1 ) );
        ...
        expect ( screen.getAllByRole ( "row" ).slice ( 1 ) ).toHaveLength ( entries.length );
