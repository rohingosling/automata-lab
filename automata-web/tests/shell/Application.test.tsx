...
import * as documentWorkspace from "../../src/application/document-workspace";
import { createDocumentEditorState } from "../../src/domain/model/commands";
import { createEmptyAuthoringDraft } from "../../src/domain/model/drafts";
import { BrowserFilePort } from "../../src/infrastructure/files";
import
{
    DiagnosticChannel,
...
describe ( "AL-UI-001 Phase 2 application shell", () =>
{
    ...
    it ( "renders the exact desktop regions and initially collapsed route tree", () =>
    {
        ...
        expect ( screen.getAllByText ( "Version 1.6.0" ).some ( element => element.className === "title-version" ) )
            .toBe ( true );
    ...
    it ( "opens the documentation home URL in an isolated browser context", async () =>
    {
        // Initialize the local values needed by this operation.
    ...
    it ( "keeps Save controls available for an invalid open Chart draft and explains the blocking diagnostic", async () =>
    {
        const user = userEvent.setup ();
        const empty = createEmptyAuthoringDraft ();
        const draft = { ...empty,
            stateMachine: { ...empty.stateMachine, states: [ { terminalState: false, name: "state_idle", description: "" } ] },
            chart: { ...empty.chart,
                indicators: { ...empty.chart.indicators, initialStateIndicator: { x: 50, y: 50, state: "state_idle" } },
                states: [ { state: "state_idle", x: 200, y: 100 } ],
            },
        };
        const editorState = createDocumentEditorState ( draft );
        const workspace = { ...documentWorkspace.createNewDocumentWorkspace (), editorState: { ...editorState, dirty: true } };
        vi.spyOn ( documentWorkspace, "createClosedDocumentWorkspace" ).mockReturnValueOnce ( workspace );
        const writeDocument = vi.spyOn ( BrowserFilePort.prototype, "saveTextDocument" ).mockResolvedValue ( null );
        render ( <Application /> );
        expect ( document.querySelector ( "[data-toolbar-entry='toolbar-save']" ) ).toBeEnabled ();
        expect ( document.querySelector ( "[data-toolbar-entry='toolbar-save-as']" ) ).toBeEnabled ();
        for ( const action of [ "toolbar-save", "toolbar-save-as", "menu-save", "menu-save-as",
            "shortcut-save", "shortcut-save-as" ] )
        {
            if ( action.startsWith ( "toolbar" ) )
            {
                const button = document.querySelector<HTMLButtonElement> ( "[data-toolbar-entry='" + action + "']" );
                if ( button === null )
                {
                    throw new Error ( "The Save toolbar command must be available." );
                }
            ...
            {
                await user.click ( screen.getByRole ( "menuitem", { name: "File" } ) );
                const command = screen.getByRole ( "menuitem", { name: action === "menu-save" ? "Save" : "Save As" } );
                expect ( command ).toBeEnabled ();
                await user.click ( command );
            }
            else
            {
                await user.keyboard ( action === "shortcut-save" ? "{Control>}s{/Control}"
                    : "{Control>}{Shift>}s{/Shift}{/Control}" );
            }
            const dialog = await screen.findByRole ( "dialog", { name: "Error" } );
            expect ( within ( dialog ).getByText ( /The connected initial indicator must reference the semantic initial state/u ) )
                .toBeVisible ();
        ...
        const replacement = screen.getByRole ( "dialog", { name: "Unsaved changes" } );
        expect ( within ( replacement ).queryByRole ( "button", { name: "Save and Continue" } ) ).not.toBeInTheDocument ();
        expect ( within ( replacement ).getByRole ( "button", { name: "Discard and Continue" } ) ).toBeEnabled ();
    } );
