    ...
    return {
        ...
        [
            ...
            },
            {
                draftTransitions:
                [
                    { id: 5, rememberedEvent: null, sourceState: null, targetState: null, sourceX: 10, sourceY: 20, targetX: 30, targetY: 40 },
                ],
...
describe ( "AL-PRN-004 printable report surface", () =>
{
    ...
    it ( "renders the semantic identity header and every document table", () =>
    {
        ...
        expect ( within ( statesTable ).getByRole ( "columnheader", { name: "Terminal State" } ) ).toBeInTheDocument ();
        expect ( within ( statesTable ).getByRole ( "cell", { name: "Yes" } ) ).toBeInTheDocument ();
    ...
    it ( "labels attached indicator endpoints and direct visual connections without state substitutions", () =>
    {
        const completeReport = createCompleteReport ();
        const report: PrintableReport = {
            ...completeReport,
            fileVersion: "1.3.0",
            sections: completeReport.sections.map ( section => section.kind === "chart" ? {
                ...section,
                draftTransitions: section.draftTransitions.map ( transition => ( {
                    ...transition, sourceIndicator: { kind: "initial" as const },
                    targetIndicator: { kind: "terminal" as const, id: 7 },
                } ) ),
                indicatorTransitions: [ {
                    id: 4, source: { kind: "initial" as const },
                    target: { kind: "terminal" as const, id: 7 },
                } ],
            } : section ),
        };
        render ( <PrintableReportSurface report={ report } /> );

        const drafts = screen.getByRole ( "table", { name: "Draft Transitions" } );
        const connections = screen.getByRole ( "table", { name: "Indicator Connections" } );
        expect ( within ( drafts ).getByText ( "Initial-state indicator (10, 20)" ) ).toBeInTheDocument ();
        expect ( within ( drafts ).getByText ( "Terminal indicator 7 (30, 40)" ) ).toBeInTheDocument ();
        expect ( within ( connections ).getByText ( "Initial-state indicator" ) ).toBeInTheDocument ();
        expect ( within ( connections ).getByText ( "Terminal indicator 7" ) ).toBeInTheDocument ();
    ...
    it ( "uses semantic table headers that repeat for a long table where the browser supports it", () =>
    {
        ...
        const rows: PrintableState[] = Array.from ( { length: 150 }, ( _value, index ) => ( {
            ...
            entryActions: [], terminalState: false,
            exitActions:  [],
            name:         `state_${index + 1}`,
        } ) );
        const completeReport          = createCompleteReport ();
        ...
        ) ).toEqual ( [ "Name", "Description", "Terminal State", "Entry Actions", "Exit Actions" ] );
