...
import type { SolverWorkspaceState } from "../../src/application/solver-workspace.js";
import { createSolverWorkspaceState, rebaseSolverCandidateAfterChartSettingChange } from "../../src/application/solver-workspace.js";
...
function ControlledSolverPage ( properties: { readonly onSequencesChange: ( sequences: readonly SolverSequence[] ) => void } )
{
    ...
    return (
        <SolverPage
            defaultReviewTab   = { COMPILE_TIME_CONFIGURATION.applicationSettings.solver.solverDefaultReviewTab }
...
describe ( "Solver page", () =>
{
    ...
    it ( "uses the configured view for each new candidate and preserves an existing review", async () =>
    {
        const user   = userEvent.setup ();
        const draft  = createDraft ();
        const result = inferSolverCandidate (
        ...
        if ( result.status !== "success" )
        {
            throw new Error ( "The review test requires a valid candidate." );
        }

        function renderCandidate (
            candidate: SolverCandidate | null,
            defaultReviewTab: CandidateReviewTab,
            status: SolverWorkspaceState["status"] = "review",
            currentDraft: AuthoringDraft = draft,
        )
        {
            return (
                <SolverPage
                    chartNameWrapping  = { { actionNames: false, eventNames: false, stateNames: false } }
                    defaultReviewTab   = { defaultReviewTab }
                    draft              = { currentDraft }
                    onApplyCandidate   = { vi.fn () }
                    onCancelSolve      = { vi.fn () }
                    onDiscardCandidate = { vi.fn () }
                    ...
                    onSolve            = { vi.fn () }
                    onValidate         = { vi.fn () }
                    solverWorkspace    = { { ...createSolverWorkspaceState (), candidate, status } }
                />
            );
        }

        const rendering = render ( renderCandidate ( null, "solver-candidate-summary" ) );

        rendering.rerender ( renderCandidate ( result.candidate, "solver-candidate-transitions" ) );
        expect ( screen.getByRole ( "tab", { name: "Transition Table" } ) ).toHaveAttribute ( "aria-selected", "true" );
        await user.click ( screen.getByRole ( "tab", { name: "State Chart" } ) );

        const collapsedDraft =
        {
            ...draft,
            chart: { ...draft.chart, settings: { ...draft.chart.settings, expandStates: false } },
        };
        const reviewWorkspace: SolverWorkspaceState =
        {
            ...createSolverWorkspaceState (),
            candidate: result.candidate,
            status: "review",
        };
        const collapsedReview = rebaseSolverCandidateAfterChartSettingChange ( reviewWorkspace, 1, 2, 1 );

        expect ( collapsedReview.candidate ).not.toBe ( result.candidate );
        rendering.rerender ( renderCandidate (
            collapsedReview.candidate, "solver-candidate-summary", "review", collapsedDraft,
        ) );
        expect ( screen.getByRole ( "tab", { name: "State Chart" } ) ).toHaveAttribute ( "aria-selected", "true" );
        expect ( screen.getByRole ( "img", { name: "State Chart" } ).querySelector ( ".solver-chart-state-box" ) )
            .toHaveAttribute ( "height", "60" );
        expect ( screen.getByRole ( "button", { name: "Apply Candidate" } ) ).toBeEnabled ();

        const expandedReview = rebaseSolverCandidateAfterChartSettingChange ( collapsedReview, 2, 3, 1 );
        const expandedDraft =
        {
            ...draft,
            chart: { ...draft.chart, settings: { ...draft.chart.settings, expandStates: true } },
        };

        rendering.rerender ( renderCandidate (
            expandedReview.candidate, "solver-candidate-summary", "review", expandedDraft,
        ) );
        expect ( screen.getByRole ( "tab", { name: "State Chart" } ) ).toHaveAttribute ( "aria-selected", "true" );
        expect ( within ( screen.getByRole ( "img", { name: "State Chart" } ) )
            .getAllByText ( "Entry Actions" ).length ).toBeGreaterThan ( 0 );
        await user.click ( screen.getByRole ( "tab", { name: "Inference Report" } ) );

        rendering.rerender ( renderCandidate ( expandedReview.candidate, "solver-candidate-coverage", "stale" ) );
        expect ( screen.getByRole ( "tab", { name: "Inference Report" } ) ).toHaveAttribute ( "aria-selected", "true" );
        expect ( screen.getByRole ( "button", { name: "Apply Candidate" } ) ).toBeDisabled ();

        const nextResult = inferSolverCandidate (
        {
            documentRevision: 3,
            solverRevision:   1,
            observations:
        ...
        {
            throw new Error ( "The next solve must produce a valid candidate." );
        }

        const nextCandidate = nextResult.candidate;

        expect ( nextCandidate.stateMachine ).not.toBe ( result.candidate.stateMachine );
        rendering.rerender ( renderCandidate ( nextCandidate, "solver-candidate-coverage" ) );
        expect ( screen.getByRole ( "tab", { name: "Trace Coverage" } ) ).toHaveAttribute ( "aria-selected", "true" );
        rendering.rerender ( renderCandidate ( nextCandidate, "solver-candidate-summary" ) );
        expect ( screen.getByRole ( "tab", { name: "Trace Coverage" } ) ).toHaveAttribute ( "aria-selected", "true" );
    } );
} );
