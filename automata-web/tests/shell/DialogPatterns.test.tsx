...
describe ( "AL-DLG-001 shared dialog patterns", () =>
{
    ...
    it ( "edits terminal membership only on state dialogs and confirms the complete draft", async () =>
    {
        const user = userEvent.setup ();
        const confirm = vi.fn ();
        render ( <NamedEntityDialog entityKind="state" mode="edit" initialValue={ { name: "A", description: "First", terminalState: false } }
            onClose={ vi.fn () } onConfirm={ confirm } open={ true } /> );
        const checkbox = screen.getByRole ( "checkbox", { name: "Terminal State" } );
        ...
        expect ( confirm ).toHaveBeenCalledExactlyOnceWith ( { name: "A", description: "First", terminalState: true } );
        cleanup ();
        render ( <NamedEntityDialog entityKind="event" mode="add" initialValue={ { name: "event", description: "" } }
            onClose={ vi.fn () } onConfirm={ confirm } open={ true } /> );
        expect ( screen.queryByRole ( "checkbox", { name: "Terminal State" } ) ).not.toBeInTheDocument ();
    } );

    it.each ( [ "Cancel", "Close dialog", "Escape" ] ) ( "discards a state checkbox draft on %s", async dismissal =>
    {
        const user = userEvent.setup ();
        const confirm = vi.fn ();
        const close = vi.fn ();
        render ( <NamedEntityDialog entityKind="state" mode="edit" initialValue={ { name: "A", description: "", terminalState: true } }
            onClose={ close } onConfirm={ confirm } open={ true } /> );
        const checkbox = screen.getByRole ( "checkbox", { name: "Terminal State" } );
        expect ( checkbox ).toBeChecked ();
    ...
    it ( "aligns every modal value column to its longest visible field label plus ten percent", () =>
    {
        ...
        render (
            <NamedEntityDialog entityKind="state" mode="edit"
        ...
        expect ( screen.getByRole ( "dialog", { name: /^(Add|Edit) State$/u } ).style.getPropertyValue (
    ...
    it ( "opens About on Licences and exposes the shipped Release Notes tab", async () =>
    {
        ...
        expect ( within ( dialog ).getByText ( "Version 1.6.0" ) ).toBeVisible ();
        const repositoryLink = within ( dialog ).getByRole ( "link", { name: "GitHub - Automata Lab" } );
        expect ( repositoryLink ).toHaveAttribute ( "href", "https://github.com/rohingosling/automata-lab" );
        ...
        const lines = expectedReleaseNotes.split ( "\n" );
        expect ( Math.max ( ...lines.map ( line => line.length ) ) ).toBeLessThanOrEqual ( 79 );
        const separators = lines.filter ( line => /^-{3,}$/u.test ( line ) );
        expect ( separators.length ).toBeGreaterThan ( 0 );
        expect ( separators.every ( line => line.length === 79 ) ).toBe ( true );
