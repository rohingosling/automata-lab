...
function createFixture (): AuthoringDraft
{
    ...
    return {
        ...
        {
            ...empty.stateMachine,
            states: [ "A", "B", "C" ].map ( name => ( { terminalState: false, name, description: "" } ) ),
...
describe ( "indicator draft connections", () =>
{
    ...
    {
        const original = createIndicatorFixture ();
        const fixture = { ...original, stateMachine: { ...original.stateMachine, states:
            original.stateMachine.states.map ( state => ( { ...state, terminalState: kind === "terminal" && state.name === "A" } ) ) } };
