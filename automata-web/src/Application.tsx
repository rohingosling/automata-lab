...
import { ConsoleEntityNavigation } from "./application/console-entity-navigation";
import type { ConsoleContext } from "./application/contracts";
import type { DocumentCommand } from "./domain/model/commands.js";
import { createTerminalStatePreparation } from "./application/chart-terminal-state-preparation.js";
import { resolveAuthoringDraftAttachments } from "./presentation/chart/chart-projection.js";
import { useEffect, useEffectEvent, useRef, useState } from "react";
import type { CSSProperties } from "react";
import type { Viewport } from "@xyflow/react";
import { buttonColorStyle } from "./presentation/shared/button-colors.js";
...
import
{
    ...
    planWorkspaceDocumentCommand as planPreparedWorkspaceDocumentCommand,
    prepareDocumentWorkspace,
    mergeSavedDocumentWorkspace,
...
const MINIMUM_DETAIL_PANE_BUTTON_HEIGHT = COMPILE_TIME_CONFIGURATION.shell.minimumDetailPaneButtonHeight;
...
export function Application ( properties: ApplicationProperties = {} )
{
    ...
    const [ simulatorAdditionalHeight, setSimulatorAdditionalHeight ] = useState ( 0 );
    ...
    const [ chartFitPending, setChartFitPending ]         = useState ( false );
    const [ chartViewport, setChartViewport ]             = useState<Viewport | null> ( null );
    const [ chartExportStatus, setChartExportStatus ]     = useState<string | null> ( null );
    const [ editorExpanded, setEditorExpanded ]           = useState ( false );
    const entityNavigationReference = useRef ( new ConsoleEntityNavigation () );
    const [ rowNavigation, setRowNavigation ] = useState <{ key: string; sequence: number } | undefined> ();
    const rowNavigationSequence = useRef ( 0 );
    ...
    const preparation = createTerminalStatePreparation ( preferences );
    const authoringDocumentCodec = new AuthoringDocumentCodec ( preparation );
    const documentCodec = new AutomataDocumentCodec ( preparation );
    ...
    const simulatorStartPolicyReference        = useRef ( false );
    const simulatorBufferGenerationReference   = useRef ( 0 );
    ...
    ): void
    {
        ...
        const previousRevision = documentWorkspaceReference.current.editorState?.documentRevision ?? 0;
        const nextRevision = nextWorkspace.editorState?.documentRevision ?? 0;
        entityNavigationReference.current.synchronize ( nextDraft,
            previousDraft !== nextDraft && nextRevision <= previousRevision );
    ...
    function performPrint (): void
    {
        ...
        const capturedDraft = resolveAuthoringDraftAttachments ( editorState.draft, preferences );
    ...
    ): void
    {
        ...
        diagnosticChannel.publish (
            {
                code,
                ...( route === undefined ? {} : { context: { label: entryText, route, ...( entityReference === undefined ? {} : { entityReference } ) } } ),
    ...
    function reportDiagnostics ( diagnostics: readonly DomainDiagnostic[], currentDocument = false ): void
    {
        reportBoundedDiagnostics (
            ...
            diagnostic => publishConsoleEntry (
                ...
                currentDocument ? entityNavigationReference.current.fromDiagnostic ( diagnostic,
                    documentWorkspaceReference.current.editorState?.draft ?? null ) : undefined,
            ),
            "Validation",
            "editor",
        );
    ...
    {
        return planPreparedWorkspaceDocumentCommand ( workspace, command, preparation );
    }

    function prepareCurrentDocument (): boolean
    {
        const before = documentWorkspaceReference.current;
        const result = prepareDocumentWorkspace ( before, preparation );
        if ( !result.isSuccessful )
        {
        ...
        if ( result.workspace !== before )
        {
            ...
            publishConsoleEntry ( "TERMINAL_NOTATION_REPAIRED", "message", "File",
                "Terminal notation was repaired in one undoable document change.", "editor" );
        }
        return true;
    }
    ...
    async function executeSave ( forceSaveAs: boolean ): Promise<boolean>
    {
        ...
        const capturedWorkspace = documentWorkspaceReference.current;
        // Initialize the local values needed by this operation.

        const draft                 = capturedWorkspace.editorState?.draft;
        ...
        if ( saveResult.status === "failed" )
        {
            ...
            const validationResult = validateDocumentWorkspace ( documentWorkspaceReference.current );
        ...
        replaceDocumentWorkspace ( mergeSavedDocumentWorkspace (
            documentWorkspaceReference.current, capturedWorkspace, saveResult.workspace,
        ) );
        publishConsoleEntry (
            "FILE_SAVED",
            "message",
    ...
    async function performSave ( forceSaveAs: boolean ): Promise<boolean>
    {
        ...
        const draft                 = documentWorkspaceReference.current.editorState?.draft;
        const persistenceValidation = draft === undefined ? null : validatePersistableAuthoringDraft ( draft );
    ...
    async function performOpen (): Promise<void>
    {
        ...
        if ( openResult.status === "opened" )
        {
            ...
            setChartViewport ( null );
            setSolverWorkspace ( createSolverWorkspaceState () );
            reportDiagnostics ( openResult.diagnostics, true );
            openResult.notices.forEach ( notice => publishConsoleEntry (
                notice.kind === "legacy_migration" ? "TERMINAL_STATES_MIGRATED" : "TERMINAL_NOTATION_REPAIRED",
                "message", "File", notice.message, "editor",
            ) );
            publishConsoleEntry (
                "FILE_OPENED",
                "message",
    ...
    function reportCommandFailure ( code: string, message: string, entityReference?: number ): void
    {
        publishConsoleEntry ( code, "error", activeRoute === "chart" ? "Chart" : "Editor", message, entityNavigationReference.current.resolve ( entityReference )?.route ?? activeRoute, entityReference );
    ...
    function commitCommandPlan ( plan: DocumentCommandPlan ): boolean
    {
        ...
        if ( result.isSuccessful )
        {
            if ( plan.command.kind === "apply_solver_candidate" )
            {
                entityNavigationReference.current.synchronize ( null );
            }
            replaceDocumentWorkspace ( result.workspace );
    ...
    function dispatchDocumentCommand ( commandFactory: DocumentCommandFactory ): boolean
    {
        ...
        const requestedCommand = commandFactory ( editorState.documentRevision );
        const command = requestedCommand.kind === "delete_entity" || requestedCommand.kind === "delete_chart_selection"
            ? { ...requestedCommand, draftEndpointPositions:
                resolveAuthoringDraftAttachments ( editorState.draft, preferences ).chart.draftTransitions }
            : requestedCommand;
        ...
        if ( !planResult.isSuccessful )
        {
            reportCommandFailure ( planResult.code, planResult.message,
                entityNavigationReference.current.fromCommand ( command, editorState.draft ) );
    ...
    function performValidation (): void
    {
        ...
        replaceDocumentWorkspace ( result.workspace );
        reportDiagnostics ( result.diagnostics, true );
    ...
    {
        simulatorBufferGenerationReference.current++;
        dispatchDocumentCommand ( expectedRevision => ( {
    ...
    async function completeSimulatorSessionStart (
        ...
        enableTerminalStates: boolean,
        loadedModelRevisionForWarning: string | null = null,
    ): Promise<void>
    {
        // Initialize the local values needed by this operation.

        const result = await startServerSession ( startingServerWorkspace, serverGateway, { enableTerminalStates } );
        ...
        if ( !applicationMountedReference.current ||
            serverWorkspaceReference.current.instanceId !== startingServerWorkspace.instanceId ||
            serverWorkspaceReference.current.connectionStatus !== "connected" )
        {
            // Return control to the caller.
    ...
    {
        simulatorStartPolicyReference.current = preferences.simulatorEnableTerminalStates;
        const enableTerminalStates = simulatorStartPolicyReference.current;
        // Handle the case where the begin server operation result condition is not satisfied.
        ...
        try
        {
            ...
            await completeSimulatorSessionStart ( serverWorkspaceReference.current, enableTerminalStates );
        }
        finally
        {
            // Complete the cleanup required after the attempted operation.
    ...
    async function pushModelAndStartSimulatorSession (): Promise<void>
    {
        ...
        try
        {
            ...
            {
                await completeSimulatorSessionStart ( pushedServerWorkspace, enableTerminalStates );
            }
        }
        finally
        {
    ...
    async function startSimulatorSessionWithoutPush (): Promise<void>
    {
        ...
        try
        {
            ...
            if ( loadedModelRevision !== null )
            {
                await completeSimulatorSessionStart ( serverWorkspaceReference.current, enableTerminalStates, loadedModelRevision );
            }
        }
        finally
        {
    ...
    //----------------------------------------------------------------------------------------------
    // Function: reportSimulatorTermination
    //
    // Description:
    //
    //   Reports only an accepted transition into terminal status. Run/Step call this after their
    //   response guards; rendering and snapshot refresh cannot repeat the notification.
    //
    //----------------------------------------------------------------------------------------------

    function reportSimulatorTermination ( previous: HostedSessionDto | null, current: HostedSessionDto ): void
    {
        if ( previous?.executionStatus.kind === "active" && current.executionStatus.kind === "terminated" )
        {
            publishConsoleEntry (
                "TERMINAL_STATE_REACHED",
                "message",
                text ( "tree.simulator" ),
                text ( "simulator.terminalStateReached" ).replace ( "{state}", () => current.currentState ),
                "simulator",
            );
        }
    }
    ...
    async function performSimulatorRun ( events: readonly string[] ): Promise<void>
    {
        ...
        const submittedWorkspace  = serverWorkspaceReference.current;
        const bufferGeneration    = simulatorBufferGenerationReference.current;
        const submittedSequences  = documentWorkspaceReference.current.editorState?.draft.simulator.sequences;
        const submittedEvents     = runEventBuffer ( events, currentCursor );
        const requestStartOffset  = events.length - submittedEvents.length;
        ...
        try
        {
            ...
            if ( !applicationMountedReference.current ||
                serverWorkspaceReference.current.instanceId !== submittedWorkspace.instanceId ||
                serverWorkspaceReference.current.connectionStatus !== "connected" ||
                serverWorkspaceReference.current.activeSession?.sessionId !== submittedWorkspace.activeSession?.sessionId )
            {
                // Return control to the caller.
            ...
            reportSimulatorTermination ( submittedWorkspace.activeSession, result.value.session );
            setSimulatorSession ( current => ( {
                lastWarnings: result.value.warnings,
                session:      result.value.session,
                stepCursor:   bufferGeneration === simulatorBufferGenerationReference.current &&
                    submittedSequences === documentWorkspaceReference.current.editorState?.draft.simulator.sequences
                    ? advanceStepCursor ( requestStartOffset, result.value.consumedEventCount, events.length )
                    : current.stepCursor,
            } ) );
        }
        finally
    ...
    async function performSimulatorStep ( events: readonly string[] ): Promise<void>
    {
        ...
        const bufferGeneration    = simulatorBufferGenerationReference.current;
        const submittedSequences  = documentWorkspaceReference.current.editorState?.draft.simulator.sequences;
        const submittedEvents     = stepEventBuffer ( events, currentCursor );
        const requestStartOffset  = Math.max ( 0, Math.min ( currentCursor, events.length ) );
    ...
    {
        const enableTerminalStates = preferences.simulatorEnableTerminalStates;
        const startingServerWorkspace = serverWorkspaceReference.current;
        // Handle the case where the begin server operation result condition is not satisfied.
        ...
        try
        {
            ...
            const result = await resetActiveServerSession ( serverWorkspaceReference.current, serverGateway, { enableTerminalStates } );
    ...
    const simulatorContextualSegments = activeRoute === "simulator" && simulatorSession.session !== null
        ? [
            ...
            ...( simulatorSession.session.executionStatus.kind === "terminated"
                ? [ text ( "status.simulatorTerminated" ) ] : [] ),
            ...
            entries: [
                {
                    ...
                    onSelect: () => window.open (
                        new URL ( `${ import.meta.env.BASE_URL }docs/`, window.location.origin ).href,
                        "_blank",
                        "noopener",
                    ),
                },
    ...
    const toolbarEntries: readonly ToolbarEntry[] = [
        ...
        { disabled: !documentOpen, icon: fluentIcon ( "ic_fluent_save_20_regular.svg" ), identifier: "toolbar-save", kind: "button", label: text ( "menu.file.save" ), onSelect: () => void performSave ( false ) },
        { disabled: !documentOpen, icon: fluentIcon ( "ic_fluent_save_copy_20_regular.svg" ), identifier: "toolbar-save-as", kind: "button", label: text ( "menu.file.saveAs" ), onSelect: () => void performSave ( true ) },
        { kind: "separator" },
    ...
    // Publish the build-time spacing contract for descendant page, dialog, and pane styles.

    const presentationSpacing = COMPILE_TIME_CONFIGURATION.presentation.spacing;
    const shellStyle =
    {
        ...buttonColorStyle ( preferences ),
        "--console-panel-height":           `${preferences.consolePanelHeight}px`,
        "--master-panel-width":             `${preferences.masterPanelWidth}px`,
        "--detail-page-content-inset":      `${presentationSpacing.detailPageContentInsetPixels}px`,
        "--modal-content-inset":            `${presentationSpacing.modalContentInsetPixels}px`,
        "--splitter-side-gap":              `${presentationSpacing.splitterSideGapPixels}px`,
        "--scroll-content-edge-inset":      `${presentationSpacing.scrollContentEdgeInsetPixels}px`,
        "--group-box-outer-spacing-top":    `${presentationSpacing.groupBoxOuterSpacingPixels.top}px`,
        "--group-box-outer-spacing-right":  `${presentationSpacing.groupBoxOuterSpacingPixels.right}px`,
        "--group-box-outer-spacing-bottom": `${presentationSpacing.groupBoxOuterSpacingPixels.bottom}px`,
        "--group-box-outer-spacing-left":   `${presentationSpacing.groupBoxOuterSpacingPixels.left}px`,
        "--group-box-corner-radius":        `${COMPILE_TIME_CONFIGURATION.presentation.groupBoxCornerRadiusPixels}px`,
    } as CSSProperties;
    const detailContent = activeRoute === "solver"
        ? (
            <SolverPage
                ...
                defaultReviewTab   = { preferences.solverDefaultReviewTab }
        ...
        : EDITOR_ROUTES.includes ( activeRoute ) ? (
            ...
            <ChartPage
                ...
                fitOnFirstView                                = { chartFitPending }
                onFirstViewFitConsumed                         = { () => setChartFitPending ( false ) }
            ...
            <SimulatorPage
                onAdditionalHeightChange = { setSimulatorAdditionalHeight }
                availability             = { simulatorAvailability }
                draft                    = { editorState?.draft ?? null }
                followTraceTail          = { preferences.simulatorFollowTraceTail }
                ...
                onBufferChange           = { () => { simulatorBufferGenerationReference.current++; } }
                onStepCursorChange       = { stepCursor =>
                {
                    simulatorBufferGenerationReference.current++;
                    setSimulatorSession ( current => ( { ...current, stepCursor } ) );
                } }
            ...
            >
                ...
                <main className="workspace">
                    ...
                    <Splitter
                        ...
                        opposingMinimum = { MINIMUM_DETAIL_PANE_BUTTON_HEIGHT +
                            ( activeRoute === "simulator" ? simulatorAdditionalHeight : 0 ) }
                    ...
                    <ConsolePanel
                        ...
                        onNavigateToContext = { ( context: ConsoleContext ) =>
                        {
                            const destination = entityNavigationReference.current.resolve ( context.entityReference );
                            navigate ( context.route, destination === undefined );
                            if ( destination !== undefined && destination.route === context.route )
                            {
                                setRowNavigation ( { key: destination.rowKey, sequence: ++rowNavigationSequence.current } );
                            }
                        } }
                    />
                </main>
                ...
                { pendingSolverCandidate !== null && (
                    <SolverReplacementDialog
                        candidateSummary={
                            ...
                            text ( "solver.candidate.terminalReplacement" ).replace ( "{count}",
                                String ( documentWorkspace.editorState?.draft.stateMachine.states.filter ( state => state.terminalState ).length ?? 0 ) )
                        }
