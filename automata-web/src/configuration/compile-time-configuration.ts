...
//   shell and presentation defaults, and diagnostic switches. Values are compiled into
...
export const COMPILE_TIME_CONFIGURATION =
{
    applicationSettings:
    {
        appearance:
        {
            applicationButtonColor:    "Blue",
            titleBarColor:             "Blue",
            consoleMessageButtonColor: "Gray",
            matchConsoleMessageColor:  false,
            theme:                     "Dark",
        },
        chart:
        {
        ...
        simulator:
        {
            simulatorEnableTerminalStates: false,
            simulatorFollowTraceTail: true,
        },
        solver:
        {
            solverDefaultReviewTab: "solver-candidate-summary",
        },
        workspace:
        {
    ...
    presentation:
    {
        groupBoxCornerRadiusPixels: 6,

        // Each physical content boundary has one inset owner. Page and modal insets take precedence
        // when their outer boundary is also a scroll boundary; independent scroll areas use their
        // own inset. Table/grid scrollports have zero right clearance; bottom insets still apply.
        // These values are CSS pixels and are never persisted as preferences.

        spacing:
        {
            detailPageContentInsetPixels: 8,
            modalContentInsetPixels:      8,
            splitterSideGapPixels:        4,
            scrollContentEdgeInsetPixels: 4,

            // Add space outside every group-box border without changing its internal padding.

            groupBoxOuterSpacingPixels:
            {
                top:    0,
                right:  4,
                bottom: 0,
    ...
    shell:
    {
        consoleColumns:
        {
            widths: { time: 72, severity: 92, code: 250, source: 108, message: 220 },
            minimumWidths: { time: 60, severity: 80, code: 80, source: 80, message: 120 },
            maximumWidth: 1_200,
            keyboardStep: 8,
        ...
        minimumDetailPaneButtonHeight: 160,
...
function createDefaultApplicationPreferences (): ApplicationPreferences
{
    ...
    return {
        applicationButtonColor:    applicationSettings.appearance.applicationButtonColor,
        titleBarColor:             applicationSettings.appearance.titleBarColor,
        consoleMessageButtonColor: applicationSettings.appearance.consoleMessageButtonColor,
        matchConsoleMessageColor:  applicationSettings.appearance.matchConsoleMessageColor,
        ...
        simulatorEnableTerminalStates: applicationSettings.simulator.simulatorEnableTerminalStates,
        simulatorFollowTraceTail:    applicationSettings.simulator.simulatorFollowTraceTail,
        snapToGrid:                  applicationSettings.chart.grid.snapToGrid,
        solverDefaultReviewTab:      applicationSettings.solver.solverDefaultReviewTab,
