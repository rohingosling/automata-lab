...
import { isSessionInitializationValid, isSessionAdvancementValid } from "../../application/session-execution-validation.js";
...
function mapProtocolError<Value = never> ( error: ServerProtocolError ): ServerGatewayResult<Value>
{
    ...
    switch ( error.code )
    {
        ...
        // A terminal rejection preserves the session and is never retryable.

        case "SESSION_TERMINATED":
            return failure ( "SESSION_TERMINATED", error.message,
                "Reset or create a session before running or stepping again.", false );
...
export class BrowserServerWorkerGateway implements ServerGateway
{
    ...
    private readonly sessionsById = new Map<string, HostedSessionDto> ();
    ...
    public async closeSession ( sessionId: string ): Promise<ServerGatewayResult<void>>
    {
        ...
        if ( responseResult.value.operation !== "simulation.close" ||
            responseResult.value.result.sessionId !== sessionId )
        {
            return unexpectedOperationFailure ();
        }
    ...
    ): Promise<ServerGatewayResult<HostedSessionDto>>
    {
        ...
        // Validate the request policy before retaining or returning the session snapshot.

        const session  = copySession ( responseResult.value.result );
        const previous = request.sessionId === null ? null : this.sessionsById.get ( request.sessionId );
        if ( previous === undefined || !isSessionInitializationValid ( session, request.payload, previous ) )
        {
            return failure ( "SERVER_RESPONSE_INVALID", "The session initialization response is inconsistent.",
                "Reconnect and create a new session.", false );
        }
    ...
    ): Promise<ServerGatewayResult<HostedSessionOperationResult>>
    {
        ...
        // Validate the complete operation against the accepted prior execution.

        const result   = copySimulationOperation ( responseResult.value.result );
        const previous = this.sessionsById.get ( request.sessionId );
        if ( previous === undefined || !isSessionAdvancementValid (
            previous, result, request.payload.events.filter ( event => event.trim ().length > 0 ).length,
            operation === "simulation.step",
        ) )
        {
            return failure ( "SERVER_RESPONSE_INVALID", "The session advancement response is inconsistent.",
                "Reconnect and create a new session.", false );
        }
        this.sessionsById.set ( result.session.sessionId, result.session );
