...
function chartElementBounds ( canvas: HTMLElement ): ChartBounds
{
    ...
    const elements = Array.from ( canvas.querySelectorAll<HTMLElement> (
        ".react-flow__node, .react-flow__edge, .chart-draft-transition-path, " +
        ".chart-draft-transition-node .react-flow__edge-textwrapper",
    ) );
...
//--------------------------------------------------------------------------------------------------
// Function: chartTransitionExportPaint
//
// Description:
//
//   Reads normal transition paint tokens instead of selected or focused stroke values.
//   SVG capture and the bounded raster compositor use this same neutral appearance.
//
//--------------------------------------------------------------------------------------------------

export function chartTransitionExportPaint ( path: SVGPathElement ): {
    readonly color: string;
    readonly width: number;
}
{
    const style = window.getComputedStyle ( path );
    const color = style.getPropertyValue ( "--chart-transition-color" ).trim ();
    const width = Number.parseFloat ( style.getPropertyValue ( "--chart-transition-width" ) );

    return {
        color: color || style.getPropertyValue ( "--text-muted" ).trim () || style.stroke,
        width: Number.isFinite ( width ) && width > 0 ? width : 1.8,
    };
}

//--------------------------------------------------------------------------------------------------
// Function: normalizeChartTransitionExportStyles
//
// Description:
//
//   Removes transient transition decoration after styles have been inlined, without touching the
//   live chart. Transparent pointer hit paths are interaction controls and never enter an export.
//
//--------------------------------------------------------------------------------------------------

export function normalizeChartTransitionExportStyles ( source: Element, clone: Element ): void
{
    const selector = ".react-flow__edge-path, .chart-draft-transition-node > svg > path";
    const paths    = source.querySelectorAll<SVGPathElement> ( selector );
    const copies   = clone.querySelectorAll<SVGPathElement> ( selector );
    ...
    paths.forEach ( ( path, index ) =>
    {
        ...
        const paint = chartTransitionExportPaint ( path );

        copy.style.stroke      = paint.color;
        copy.style.strokeWidth = String ( paint.width );
        copy.style.color       = paint.color;

        const container = copy.closest<HTMLElement | SVGElement> (
            ".react-flow__edge, .chart-draft-transition-node" );

        if ( container !== null )
        {
            container.style.color = paint.color;
            container.querySelectorAll<SVGElement> ( "marker *" ).forEach ( markerElement =>
            {
                markerElement.style.color = paint.color;

                if ( markerElement.style.fill !== "none" )
                {
                    markerElement.style.fill = paint.color;
                }

                if ( markerElement.style.stroke !== "none" && markerElement.style.stroke !== "" )
                {
                    markerElement.style.stroke = paint.color;
                }
            } );
        }
    } );
    clone.querySelectorAll ( ".chart-draft-transition-hit, .react-flow__edge-interaction" )
        .forEach ( element => element.remove () );
    clone.querySelectorAll<HTMLElement | SVGElement> (
        ".react-flow__node-draftTransition, .chart-draft-transition-node" ).forEach ( element =>
    {
        element.style.border    = "0";
        element.style.boxShadow = "none";
        element.style.outline   = "none";
        element.classList.remove ( "selected", "chart-node-selected", "chart-transition-selected" );
    } );
}
...
}
{
    ...
    normalizeChartTransitionExportStyles ( viewport, viewportClone );
    ...
    // Computed selection shadows survive class removal and must be cleared from the clone.

    viewportClone.querySelectorAll<HTMLElement | SVGElement> ( ".selected, .chart-node-selected" )
        .forEach ( element =>
        {
...
): Promise<Blob>
{
    ...
    request.canvas.querySelectorAll<HTMLElement> ( ".react-flow__node" ).forEach ( node =>
    {
        ...
        else
        {
            ...
            const draftPath = node.querySelector<SVGPathElement> ( ".chart-draft-transition-node > svg > path" );
            ...
            if ( draftPath !== null )
            {
                ...
                const paint       = chartTransitionExportPaint ( draftPath );
                ...
                const pathTransform = draftPath.transform?.baseVal.consolidate ()?.matrix;

                if ( pathTransform !== undefined )
                {
                    context.transform ( pathTransform.a, pathTransform.b, pathTransform.c,
                        pathTransform.d, pathTransform.e, pathTransform.f );
                }
