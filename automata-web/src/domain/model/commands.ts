...
import
{
    chartIndicatorReferenceKey,
    isAllowedChartIndicatorPair,
    isChartIndicatorReferencePresent,
    normalizeDraftTransitionIndicatorDirection,
    resolveChartIndicatorPoint,
} from "./draft-transition-attachments.js";
...
import { validatePersistableAuthoringDraft } from "./validation.js";
import { hasCoherentTerminalStateNotation } from "./terminal-states.js";
...
export interface AddEntityCommand
{
    ...
    readonly entityKind:       EntityKind;
    readonly entity:           NamedEntity & { readonly terminalState?: boolean };
...
export interface DeleteEntityCommand
{
    ...
    readonly draftEndpointPositions?: readonly ChartDraftTransition[];
    readonly entityKind:       EntityKind;
    readonly name:             string;
    readonly expectedRevision: number;
}
...
//--------------------------------------------------------------------------------------------------
// Interface: DetachTransitionEndpointCommand
//
// Description:
//
//   Atomically replaces a transition with a draft retaining its event and other attachment.
//--------------------------------------------------------------------------------------------------

export interface DetachTransitionEndpointCommand
{
    readonly kind:              "detach_transition_endpoint";
    readonly index:             number;
    readonly endpoint:          "source" | "target";
    readonly point:             ChartPoint;
    readonly oppositePoint:     ChartPoint;
    readonly draftTransitionId: number;
    readonly expectedRevision:  number;
}

//--------------------------------------------------------------------------------------------------
// Interface: UpdateChartDraftEndpointCommand
//
// Description:
//
//   Changes one draft endpoint and restores remembered transitions when attachments are complete.
//--------------------------------------------------------------------------------------------------

export interface UpdateChartDraftEndpointCommand
{
    readonly kind:              "update_chart_draft_endpoint";
...
export interface DeleteChartSelectionCommand
{
    ...
    readonly indicatorTransitionIds?:     readonly number[];
...
export interface PrepareTerminalNotationCommand
{
    readonly kind: "prepare_terminal_notation";
    readonly expectedRevision: number;
}

export type DocumentCommand =
    | PrepareTerminalNotationCommand
    | AddEntityCommand
    | DetachTransitionEndpointCommand
    | UpdateChartDraftEndpointCommand
...
export interface CommandImpactSummary
{
    ...
    readonly chartDraftEndpointAttachmentCount?: number;
    readonly chartDraftEventReferenceCount?:    number;
...
function isValidChartDraftTransition ( draftTransition: ChartDraftTransition, draft: AuthoringDraft ): boolean
{
    // Return the computed result.
    ...
    return Number.isSafeInteger ( draftTransition.id ) && draftTransition.id >= 0 &&
        ...
        [ draftTransition.sourceState, draftTransition.targetState ].every ( stateName =>
            stateName == null || draft.stateMachine.states.some ( state => state.name === stateName ) ) &&
        [ "source", "target" ].every ( endpoint =>
        {
            const indicator = endpoint === "source" ? draftTransition.sourceIndicator : draftTransition.targetIndicator;
            const stateName = endpoint === "source" ? draftTransition.sourceState : draftTransition.targetState;
            return indicator == null || stateName == null && draftTransition.rememberedEvent == null &&
                isChartIndicatorReferencePresent ( indicator, draft.chart.indicators );
        } ) &&
        ( draftTransition.rememberedEvent == null || draft.stateMachine.events.some ( event =>
            event.name === draftTransition.rememberedEvent ) ) &&
        ( draftTransition.rememberedTransitionIndex == null ||
            ( draftTransition.rememberedEvent != null &&
                Number.isSafeInteger ( draftTransition.rememberedTransitionIndex ) &&
                draftTransition.rememberedTransitionIndex >= 0 &&
                draftTransition.rememberedTransitionIndex < MAXIMUM_TRANSITION_COUNT ) );
}
...
): CommandImpactSummary
{
    ...
    if ( entityKind === "state" )
    {
        ...
        return {
            ...
            chartDraftTransitionCount: 0,
            chartDraftEndpointAttachmentCount: draft.chart.draftTransitions.reduce ( ( count, transition ) =>
                count + Number ( transition.sourceState === name ) + Number ( transition.targetState === name ), 0 ),
            chartTerminalIndicatorCount: 0,
    ...
    if ( entityKind === "event" )
    {
        ...
        return {
            ...
            chartDraftEventReferenceCount: draft.chart.draftTransitions.filter ( transition =>
                transition.rememberedEvent === name ).length,
            solverTokenReferenceCount: draft.solver.sequences.reduce (
                ( count, sequence ) => count + sequence.sequence.filter ( ( token ) => token === name ).length,
                0,
            ),
...
function renameDeclaration<Entity extends NamedEntity> (
    declarations: readonly Entity[],
    previousName: string,
    newName: string,
): readonly Entity[]
{
        ...
        return {
            ...
            chart:
            {
                ...
                draftTransitions: draft.chart.draftTransitions.map ( transition => ( {
                    ...transition,
                    ...( transition.sourceState === previousName ? { sourceState: newName } : {} ),
                    ...( transition.targetState === previousName ? { targetState: newName } : {} ),
                } ) ),
                indicators:
                {
    ...
    if ( entityKind === "event" )
    {
        ...
        return {
            ...
            chart:
            {
                ...draft.chart,
                draftTransitions: draft.chart.draftTransitions.map ( transition =>
                    transition.rememberedEvent === previousName ? { ...transition, rememberedEvent: newName } : transition ),
            },
            solver:
            {
                sequences: draft.solver.sequences.map ( ( sequence ) =>
...
function deleteEntity (
    ...
    entityKind: EntityKind,
    name: string,
    endpointPositions: readonly ChartDraftTransition[] = [],
): AuthoringDraft
{
    // Initialize the local values needed by this operation.
    ...
    if ( entityKind === "state" )
    {
        ...
        const positionsById = new Map ( endpointPositions.map ( transition => [ transition.id, transition ] ) );
        ...
        return {
            ...
            chart:
            {
                ...
                draftTransitions: draft.chart.draftTransitions.map ( transition => ( {
                    ...transition,
                    ...( transition.sourceState === name ? { sourceState: null,
                        source: positionsById.get ( transition.id )?.source ?? transition.source } : {} ),
                    ...( transition.targetState === name ? { targetState: null,
                        target: positionsById.get ( transition.id )?.target ?? transition.target } : {} ),
                } ) ),
                indicators:
                {
                    ...draft.chart.indicators,
    ...
    if ( entityKind === "event" )
    {
        ...
        return {
            ...
            chart:
            {
                ...draft.chart,
                draftTransitions: draft.chart.draftTransitions.map ( transition =>
                    transition.rememberedEvent === name
                        ? { ...transition, rememberedEvent: null, rememberedTransitionIndex: null } : transition ),
            },
        };
    }
...
): AuthoringDraft
{
    ...
    if ( entityKind === "state" )
    {
        // Generic entity edits preserve state membership; new declarations default false.

        const existingStates = new Map ( draft.stateMachine.states.map ( state => [ state.name, state ] ) );
        const states = declarations.map ( declaration => ( {
            ...declaration,
            terminalState: existingStates.get ( declaration.name )?.terminalState ?? false,
        } ) );

        return { ...draft, stateMachine: { ...draft.stateMachine, states } };
    }
...
): CommandPlanResult
{
    ...
    if ( command.kind === "delete_entity" )
    {
        ...
        return createPlan (
            command,
            deleteEntity ( state.draft, command.entityKind, command.name, command.draftEndpointPositions ),
    ...
    if ( replacementName === targetName && targetDeclaration.description === command.entity.description &&
        ( command.entityKind !== "state" || command.entity.terminalState === undefined ||
            command.entity.terminalState === state.draft.stateMachine.states.find (
                declaration => declaration.name === targetName )?.terminalState ) )
    {
        // Return the create failure result.
...
): CommandPlanResult
{
    ...
    const directIndicatorReferences = new Set ( ( state.draft.chart.indicators.indicatorTransitions ?? [] ).flatMap (
        relation => [ chartIndicatorReferenceKey ( relation.source ), chartIndicatorReferenceKey ( relation.target ) ],
    ) );
    const initialIndicatorIsOrphaned = state.draft.chart.indicators.initialStateIndicator?.state === null &&
        !directIndicatorReferences.has ( "initial" );
    ...
    const expectedTerminalIdentifiers = deleteOrphanedItems
        ? new Set ( [ ...currentIndicatorIdentifiers ].filter ( identifier =>
            relatedTerminalIndicatorIdentifiers.has ( identifier ) || directIndicatorReferences.has ( "terminal:" + identifier ) ) )
    ...
    const draftTransitionReplacementIsAllowed = deleteOrphanedItems
        ...
        : command.draftTransitions.length === currentDraftTransitionIdentifiers.size &&
            ...
            command.draftTransitions.every ( draftTransition =>
                ...
                isValidChartDraftTransition ( draftTransition, state.draft ) );
...
//--------------------------------------------------------------------------------------------------
// Function: planDetachTransitionEndpointCommand
//
// Description:
//
//   Validates both sides of semantic-to-draft replacement before committing one history entry.
//--------------------------------------------------------------------------------------------------

function planDetachTransitionEndpointCommand (
    state: DocumentEditorState,
    command: DetachTransitionEndpointCommand,
): CommandPlanResult
{
    const transition = state.draft.stateMachine.transitionTable [ command.index ];
    if ( !isValidArrayIndex ( state.draft.stateMachine.transitionTable, command.index ) || transition === undefined )
    {
        return createFailure ( "ENTITY_NOT_FOUND", "The transition no longer exists." );
    }
    ...
    const draftTransition: ChartDraftTransition = {
        id: command.draftTransitionId,
        source: command.endpoint === "source" ? command.point : command.oppositePoint,
        target: command.endpoint === "target" ? command.point : command.oppositePoint,
        sourceState: command.endpoint === "source" ? null : transition.state,
        targetState: command.endpoint === "target" ? null : transition.stateNext,
        rememberedEvent: transition.event,
        rememberedTransitionIndex: command.index,
    };
    if ( !isValidChartDraftTransition ( draftTransition, state.draft ) )
    {
        return createFailure ( "COMMAND_INVALID", "The detached transition coordinates or identifier are invalid." );
    }
    if ( state.draft.chart.draftTransitions.some ( current => current.id === draftTransition.id ) )
    {
        return createFailure ( "ENTITY_EXISTS", "The chart draft-transition identifier is already in use." );
    }
    return createPlan ( command, {
        ...
        {
            ...state.draft.stateMachine,
            transitionTable: state.draft.stateMachine.transitionTable.filter ( ( _current, index ) =>
                index !== command.index ),
        },
        chart:
        {
            ...state.draft.chart,
            draftTransitions: [ ...state.draft.chart.draftTransitions, draftTransition ],
        },
    } );
}

//--------------------------------------------------------------------------------------------------
// Function: planUpdateChartDraftEndpointCommand
//
// Description:
//
//   Preserves the other endpoint and rejects invalid references before changing the draft.
//--------------------------------------------------------------------------------------------------

function planUpdateChartDraftEndpointCommand (
    state: DocumentEditorState,
    command: UpdateChartDraftEndpointCommand,
): CommandPlanResult
{
    const current = state.draft.chart.draftTransitions.find ( transition =>
        transition.id === command.draftTransitionId );
    if ( current === undefined )
    {
        return createFailure ( "ENTITY_NOT_FOUND", "The draft transition no longer exists." );
    }
    const endpointUpdate = command.endpoint === "source"
        ? { ...current, source: command.point, sourceState: command.state, sourceIndicator: command.indicator ?? null }
        : { ...current, target: command.point, targetState: command.state, targetIndicator: command.indicator ?? null };
    if ( !isValidChartDraftTransition ( endpointUpdate, state.draft ) )
    {
        return createFailure ( "REFERENCE_INVALID", "The endpoint or its attached state is invalid." );
    }
    const replacement = normalizeDraftTransitionIndicatorDirection ( endpointUpdate );
    if ( JSON.stringify ( current ) === JSON.stringify ( replacement ) )
    {
        return createFailure ( "COMMAND_INVALID", "The draft endpoint is unchanged." );
    }
    const indicator = replacement.sourceIndicator ?? replacement.targetIndicator;
    const attachedState = replacement.sourceIndicator != null ? replacement.targetState : replacement.sourceState;
    if ( replacement.sourceIndicator != null && replacement.targetIndicator != null )
    {
        return planDraftIndicatorPairConversion ( state, command, replacement );
    }
    if ( indicator != null && attachedState != null )
    {
        const currentIndicator = state.draft.chart.indicators.initialStateIndicator;
        const alreadyConnected = indicator.kind === "initial"
            ? currentIndicator?.state === attachedState && state.draft.stateMachine.initialState === attachedState
            : state.draft.chart.indicators.terminalStateTransitions.some ( relation =>
                relation.state === attachedState && relation.terminalStateIndicatorId === indicator.id );

        // Completing another draft is still a document change when the notation already exists.
        // Consume that draft through the outer atomic plan without adding a duplicate relation.

        const connection = alreadyConnected
            ? createPlan ( command, state.draft )
            : indicator.kind === "initial" && currentIndicator !== null
            ? planSetChartInitialIndicatorCommand ( state, {
                kind: "set_chart_initial_indicator", indicator: { ...currentIndicator, state: attachedState },
                expectedRevision: command.expectedRevision,
            } )
            : indicator.kind === "terminal" ? planConnectChartTerminalIndicatorCommand ( state, {
                kind: "connect_chart_terminal_indicator", state: attachedState, indicatorId: indicator.id,
                expectedRevision: command.expectedRevision,
            } ) : createFailure ( "REFERENCE_INVALID", "The initial indicator no longer exists." );
        return connection.isSuccessful ? createPlan ( command, {
            ...connection.plan.resultingDraft,
            chart:
            {
                ...connection.plan.resultingDraft.chart,
                draftTransitions: state.draft.chart.draftTransitions.filter ( transition => transition.id !== current.id ),
            },
        } ) : connection;
    }
    if ( replacement.sourceState != null && replacement.targetState != null && replacement.rememberedEvent != null )
    {
        return planDraftTransitionConversion ( state, command, replacement, {
            state: replacement.sourceState, event: replacement.rememberedEvent, stateNext: replacement.targetState,
        } );
    }
    return createPlan ( command, {
        ...state.draft,
        ...
        {
            ...state.draft.chart,
            draftTransitions: state.draft.chart.draftTransitions.map ( transition =>
                transition.id === current.id ? replacement : transition ),
        },
    } );
}

// Consumes one completed notation-only draft without changing semantic state or event definitions.

function planDraftIndicatorPairConversion (
    state: DocumentEditorState,
    command: UpdateChartDraftEndpointCommand,
    transition: ChartDraftTransition,
): CommandPlanResult
{
    let source = transition.sourceIndicator;
    let target = transition.targetIndicator;
    if ( source == null || target == null )
    {
        return createFailure ( "REFERENCE_INVALID", "Both indicator references are required." );
    }
    if ( !isAllowedChartIndicatorPair ( source, target ) && isAllowedChartIndicatorPair ( target, source ) )
    {
        [ source, target ] = [ target, source ];
    }
    if ( !isAllowedChartIndicatorPair ( source, target ) )
    {
        return createFailure ( "COMMAND_INVALID", "Those indicators cannot be connected." );
    }
    const relations = state.draft.chart.indicators.indicatorTransitions ?? [];
    const sourceKey = chartIndicatorReferenceKey ( source );
    const targetKey = chartIndicatorReferenceKey ( target );
    if ( relations.some ( relation => chartIndicatorReferenceKey ( relation.source ) === sourceKey &&
        chartIndicatorReferenceKey ( relation.target ) === targetKey ) )
    {
        return createFailure ( "ENTITY_EXISTS", "Those indicators are already connected." );
    }
    if ( relations.length >= MAXIMUM_CHART_DRAFT_TRANSITION_COUNT )
    {
        return createFailure ( "COMMAND_INVALID", "The indicator-relation capacity has been reached." );
    }
    const identifiers = new Set ( relations.map ( relation => relation.id ) );
    let identifier = 0;
    ...
    return createPlan ( command, {
        ...
        {
            ...state.draft.chart,
            indicators: { ...state.draft.chart.indicators, indicatorTransitions: [ ...relations, { id: identifier, source, target } ] },
            draftTransitions: state.draft.chart.draftTransitions.filter ( current => current.id !== transition.id ),
        },
    } );
}
...
): CommandPlanResult
{
    ...
    if ( !isValidChartDraftTransition ( command.draftTransition, state.draft ) )
    {
        // Return the create failure result.
...
): CommandPlanResult
{
    ...
    const draftTransition = state.draft.chart.draftTransitions.find ( current => current.id === command.draftTransitionId );
    if ( draftTransition === undefined )
    {
        return createFailure ( "ENTITY_NOT_FOUND", "The draft transition no longer exists." );
    }
    if ( draftTransition.sourceIndicator != null || draftTransition.targetIndicator != null )
    {
        return createFailure ( "COMMAND_INVALID", "Detach indicator endpoints before configuring an event transition." );
    }
    return planDraftTransitionConversion ( state, command, draftTransition, command.transition, command.chartStatePlacements );
}
//--------------------------------------------------------------------------------------------------
// Function: planDraftTransitionConversion
//
// Description:
//
//   Shares validation and atomic replacement for explicit configuration and remembered restoration.
//--------------------------------------------------------------------------------------------------

function planDraftTransitionConversion (
    state: DocumentEditorState,
    command: ConfigureChartDraftTransitionCommand | UpdateChartDraftEndpointCommand,
    draftTransition: ChartDraftTransition,
    transition: TransitionDefinition,
    chartStatePlacements?: readonly ChartStatePlacement[],
): CommandPlanResult
{
    ...
    if ( !validateTransitionReferences ( state.draft, transition ) )
    {
        // Return the create failure result.
    ...
    if ( transitionKeyExists ( state.draft.stateMachine.transitionTable, transition, null ) )
    {
        // Return the create failure result.

        return createFailure (
            "TRANSITION_EXISTS",
            `A transition already exists for '${transition.state}' and '${transition.event}'.`,
        );
    }
    ...
    const transitions = state.draft.stateMachine.transitionTable;
    const insertionIndex = Math.min ( draftTransition.rememberedTransitionIndex ?? transitions.length, transitions.length );
    const configuredDraft: AuthoringDraft = {
        ...state.draft,
        stateMachine:
        {
            ...state.draft.stateMachine,
            transitionTable: [ ...transitions.slice ( 0, insertionIndex ), transition, ...transitions.slice ( insertionIndex ) ],
        },
        chart:
        {
            ...state.draft.chart,
            draftTransitions: state.draft.chart.draftTransitions.filter ( current =>
                current.id !== draftTransition.id ),
        },
    };
    const resultingDraft = chartStatePlacements === undefined
        ? configuredDraft
        : upsertChartStatePlacements ( configuredDraft, chartStatePlacements );
...
// Releases disappearing indicator references at their last logical centers.

function releaseRemovedIndicatorReferences ( before: AuthoringDraft, after: AuthoringDraft ): AuthoringDraft
{
    const removed = ( reference: ChartIndicatorReference ) =>
        !isChartIndicatorReferencePresent ( reference, after.chart.indicators );
    const drafts = after.chart.draftTransitions.map ( transition =>
    {
        const sourceRemoved = transition.sourceIndicator != null && removed ( transition.sourceIndicator );
        const targetRemoved = transition.targetIndicator != null && removed ( transition.targetIndicator );
        return !sourceRemoved && !targetRemoved ? transition : {
            ...transition,
            ...( sourceRemoved && transition.sourceIndicator != null ? { sourceIndicator: null,
                source: resolveChartIndicatorPoint ( transition.sourceIndicator, before.chart.indicators ) ?? transition.source } : {} ),
            ...( targetRemoved && transition.targetIndicator != null ? { targetIndicator: null,
                target: resolveChartIndicatorPoint ( transition.targetIndicator, before.chart.indicators ) ?? transition.target } : {} ),
        };
    } );
    const relations = after.chart.indicators.indicatorTransitions?.filter ( relation =>
        !removed ( relation.source ) && !removed ( relation.target ) );
    return {
        ...after,
        chart:
        {
            ...after.chart,
            draftTransitions: drafts,
            indicators: { ...after.chart.indicators,
                ...( relations === undefined ? {} : { indicatorTransitions: relations } ) },
        },
    };
}
...
): CommandPlanResult
{
    ...
    // A retained indicator reference may need to restore the model's initial state after deletion.
    // Compare both effects so that an unchanged indicator does not reject that explicit repair.

    const initialState = command.indicator?.state !== undefined && command.indicator.state !== null
    ...
    if ( JSON.stringify ( command.indicator ) === JSON.stringify ( state.draft.chart.indicators.initialStateIndicator ) &&
        initialState === state.draft.stateMachine.initialState )
    {
        return createFailure ( "COMMAND_INVALID", command.indicator === null
            ? "There is no initial-state indicator to remove."
            : initialState === null || command.indicator.state === null
            ? "The initial indicator is already unconnected at this position. Connect it to a state or move it."
            : `The initial indicator is already connected to state '${ initialState }'. Choose another state to change the connection.` );
    }
    ...
    return createPlan (
        command,
        releaseRemovedIndicatorReferences ( state.draft, {
            ...state.draft,
            stateMachine:
            {
                ...state.draft.stateMachine,
...
): CommandPlanResult
{
    ...
    if ( initialIndicatorAdded === terminalIndicatorAdded ||
        ...
        command.draftTransitions.some ( transition =>
            !currentDraftIdentifiers.has ( transition.id ) || !isValidChartDraftTransition ( transition, state.draft ) ) )
    {
        // Return the create failure result.
...
): CommandPlanResult
{
    ...
    return createPlan ( command, releaseRemovedIndicatorReferences ( state.draft, resultingDraft ), calculateDraftReductionImpact ( state.draft, resultingDraft ) );
}
...
): CommandPlanResult
{
    ...
    const indicatorTransitionIds = new Set ( command.indicatorTransitionIds ?? [] );
    ...
    const hasSelection           = indicatorTransitionIds.size > 0 || stateNames.size > 0 || transitionKeys.size > 0 || terminalIndicatorIds.size > 0 ||
    ...
    if ( indicatorTransitionIds.size !== ( command.indicatorTransitionIds?.length ?? 0 ) || !hasSelection || stateNames.size !== command.stateNames.length ||
    ...
    if ( [ ...indicatorTransitionIds ].some ( identifier => !( state.draft.chart.indicators.indicatorTransitions ?? [] ).some ( relation => relation.id === identifier ) ) ||
        [ ...stateNames ].some ( stateName => !declarationExists ( state.draft, "state", stateName ) ) ||
    ...
    {
        resultingDraft = deleteEntity ( resultingDraft, "state", stateName, command.draftEndpointPositions );
    }

    // Removing a connected indicator clears its model relation; removing an orphan is chart-only.

    const deletesConnectedInitialIndicator = command.deleteInitialStateIndicator &&
        state.draft.chart.indicators.initialStateIndicator !== null &&
        state.draft.chart.indicators.initialStateIndicator.state !== null;
    ...
    resultingDraft = {
        ...
        stateMachine:
        {
            ...
            initialState: command.clearInitialStateRelation || deletesConnectedInitialIndicator
                ? null
        ...
        chart:
        {
            ...
            indicators:
            {
                ...
                ...( resultingDraft.chart.indicators.indicatorTransitions === undefined ? {} : {
                    indicatorTransitions: resultingDraft.chart.indicators.indicatorTransitions.filter ( relation =>
                        !indicatorTransitionIds.has ( relation.id ) ),
                } ),
                initialStateIndicator: command.deleteInitialStateIndicator
                    ? null
                    : command.clearInitialStateRelation && resultingDraft.chart.indicators.initialStateIndicator !== null
                    ? { ...resultingDraft.chart.indicators.initialStateIndicator, state: null }
...
function planRawDocumentCommand (
    state: DocumentEditorState,
    command: DocumentCommand,
): CommandPlanResult
{
    ...
    // Deletion captures geometry through the caller but never trusts malformed snapshots.

    if ( ( command.kind === "delete_entity" || command.kind === "delete_chart_selection" ) &&
        command.draftEndpointPositions !== undefined )
    {
        const identifiers = new Set ( state.draft.chart.draftTransitions.map ( transition => transition.id ) );
        const suppliedIdentifiers = new Set ( command.draftEndpointPositions.map ( transition => transition.id ) );
        if ( command.draftEndpointPositions.length > identifiers.size ||
            suppliedIdentifiers.size !== command.draftEndpointPositions.length ||
            command.draftEndpointPositions.some ( transition =>
                !identifiers.has ( transition.id ) || !isValidChartDraftTransition ( transition, state.draft ) ) )
        {
            return createFailure ( "COMMAND_INVALID", "The captured draft endpoint positions are invalid." );
        }
    }

    switch ( command.kind )
    {
        case "prepare_terminal_notation":
            return createPlan ( command, state.draft );
        // Handle the "add_chart_draft_transition" case.

        case "detach_transition_endpoint":
            return planDetachTransitionEndpointCommand ( state, command );

        case "update_chart_draft_endpoint":
            return planUpdateChartDraftEndpointCommand ( state, command );
...
// Semantic changes and notation changes meet here, before either enters history. The injected
// planner owns geometry only; domain validation checks its complete result before commit.

export interface DocumentTerminalPreparation
{
    prepare ( document: AuthoringDraft ):
        | { readonly isSuccessful: true; readonly document: AuthoringDraft }
...
export function planDocumentCommand (
    state: DocumentEditorState,
    command: DocumentCommand,
    preparation?: DocumentTerminalPreparation,
): CommandPlanResult
{
    const result = planRawDocumentCommand ( state, command );
    if ( !result.isSuccessful )
    {
        return result;
    }
    let document = result.plan.resultingDraft;
    const gestureChangesMembership = command.kind === "connect_chart_terminal_indicator" ||
        command.kind === "place_chart_indicator" || command.kind === "update_chart_draft_endpoint" ||
        command.kind === "delete_chart_terminal_indicator" || command.kind === "delete_chart_selection";
    if ( gestureChangesMembership )
    {
        const previousRelations = new Map ( state.draft.chart.indicators.terminalStateTransitions.map (
            relation => [ relation.state, relation.terminalStateIndicatorId ],
        ) );
        const nextRelations = new Map ( document.chart.indicators.terminalStateTransitions.map (
            relation => [ relation.state, relation.terminalStateIndicatorId ],
        ) );
        const states = document.stateMachine.states.map ( declaration =>
            previousRelations.get ( declaration.name ) !== nextRelations.get ( declaration.name ) &&
                declaration.terminalState !== nextRelations.has ( declaration.name )
                ? { ...declaration, terminalState: nextRelations.has ( declaration.name ) } : declaration );
        if ( states.some ( ( declaration, index ) => declaration !== document.stateMachine.states [ index ] ) )
        {
            document = { ...document, stateMachine: { ...document.stateMachine, states } };
        }
    }
    if ( ( command.kind === "add_entity" || command.kind === "update_entity" ) &&
        command.entity.terminalState !== undefined )
    {
        if ( command.entityKind !== "state" || typeof command.entity.terminalState !== "boolean" )
        {
            return createFailure ( "COMMAND_INVALID", "Terminal State must be a boolean on a state." );
        }
        document = { ...document, stateMachine: { ...document.stateMachine,
            states: document.stateMachine.states.map ( declaration => declaration.name === command.entity.name
                ? { ...declaration, terminalState: command.entity.terminalState ?? false } : declaration ),
        } };
    }
    // Reject invalid references before discarding any contradictory notation.

    if ( !hasCoherentTerminalStateNotation ( document ) )
    {
        const integrity = validatePersistableAuthoringDraft ( document );
        if ( !integrity.isValid )
        {
            return createFailure ( "COMMAND_INVALID", integrity.diagnostics.find ( diagnostic =>
                diagnostic.severity === "error" )?.message ?? "Terminal notation has invalid references." );
        }
    }

    // Clearing a flag retains its indicator and all unrelated references.

    const terminalStates = new Set ( document.stateMachine.states.filter ( declaration => declaration.terminalState )
    ...
    if ( relations.length !== document.chart.indicators.terminalStateTransitions.length )
    {
        document = { ...document, chart: { ...document.chart, indicators: {
            ...document.chart.indicators, terminalStateTransitions: relations,
        } } };
    }
    if ( preparation !== undefined )
    {
        const prepared = preparation.prepare ( document );
        if ( !prepared.isSuccessful )
        {
            const diagnostic = prepared.diagnostics [ 0 ];
            return createFailure ( "COMMAND_INVALID", diagnostic === undefined
                ? "Terminal notation preparation failed." : diagnostic.message + " " + diagnostic.remediation );
        }
        if ( prepared.document !== document && (
            JSON.stringify ( prepared.document.stateMachine ) !== JSON.stringify ( document.stateMachine ) ||
            JSON.stringify ( prepared.document.settings ) !== JSON.stringify ( document.settings ) ) )
        {
            return createFailure ( "COMMAND_INVALID", "Notation preparation changed semantic content." );
        }
        document = prepared.document;
    }
    const validation = validatePersistableAuthoringDraft ( document );
    const previousErrors = new Set ( validatePersistableAuthoringDraft ( state.draft ).diagnostics
        .filter ( diagnostic => diagnostic.severity === "error" ).map ( diagnostic => JSON.stringify ( diagnostic ) ) );
    const hasNewErrors = validation.diagnostics.some ( diagnostic => diagnostic.severity === "error" &&
        !previousErrors.has ( JSON.stringify ( diagnostic ) ) );
    if ( hasNewErrors || !hasCoherentTerminalStateNotation ( document ) )
    {
        return createFailure ( "COMMAND_INVALID", validation.diagnostics.find ( diagnostic =>
            diagnostic.severity === "error" )?.message ?? "The command requires atomic terminal notation preparation." );
    }
    return createPlan ( command, document, result.plan.impact );
}
