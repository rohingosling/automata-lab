...
): CommandPlanResult
{
    ...
    const replacement = command.endpoint === "source"
        ? { ...current, source: command.point, sourceState: command.state }
        : { ...current, target: command.point, targetState: command.state };
    if ( !isValidChartDraftTransition ( replacement, state.draft ) )
    {
