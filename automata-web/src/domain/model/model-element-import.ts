...
export interface StateImportValue extends NamedEntity
{
...
export type ModelElementImport =
    ...
    | {
        readonly kind:       "named_entities";
        readonly entityKind: "state";
        readonly rows:       readonly ModelImportRow<StateImportValue>[];
    }
    | {
...
): AuthoringDraft
{
    ...
    if ( entityKind === "state" )
    {
        // A legacy overwrite preserves the existing flag; a new declaration defaults false.
...
): ModelElementImportInspection
{
    ...
    if ( modelImport.entityKind === "state" )
    {
        for ( const row of modelImport.rows )
        {
            if ( row.value.terminalState !== undefined && typeof row.value.terminalState !== "boolean" )
            {
                diagnostics.push ( createImportDiagnostic (
                    "CSV_TERMINAL_STATE_INVALID",
                    "Terminal State must be true or false.",
                    "Correct the terminal_state value before importing.",
                    row.rowNumber,
                ) );
            }
        }
    ...
    // Explicit values override the preserved flag in this proposal; preparation precedes commit.

    const mergedDraft = replaceDeclarations ( draft, modelImport.entityKind, mergedDeclarations );

    if ( modelImport.entityKind === "state" )
    {
        const importedStates = new Map ( modelImport.rows.map ( row => [ row.value.name, row.value ] ) );
        const states = mergedDraft.stateMachine.states.map ( state => ( {
            ...state,
            terminalState: importedStates.get ( state.name )?.terminalState ?? state.terminalState,
        } ) );

        return {
            isSuccessful: true,
            conflicts,
            resultingDraft: { ...mergedDraft, stateMachine: { ...mergedDraft.stateMachine, states } },
        };
    }

    return { isSuccessful: true, conflicts, resultingDraft: mergedDraft };
}
