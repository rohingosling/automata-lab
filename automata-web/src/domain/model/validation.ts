...
): void
{
    ...
    // Membership is explicit even when terminal execution is disabled.

    stateMachine.states.forEach ( ( state, stateIndex ) =>
    {
        if ( typeof state.terminalState !== "boolean" )
        {
            addDiagnostic (
                diagnostics,
                "STATE_TERMINAL_FLAG_INVALID",
                "error",
                "model",
                "Every state must declare a boolean terminalState value.",
                "Use true or false for the terminal state flag.",
                "/state_machine/states/" + stateIndex + "/terminal_state",
                state.name,
            );
        }
    } );
...
function validateChart ( draft: AuthoringDraft, diagnostics: DomainDiagnostic[] ): void
{
    ...
    const eventNames                 = new Set ( draft.stateMachine.events.map ( event => event.name ) );
    ...
    draft.chart.draftTransitions.forEach ( ( draftTransition, draftTransitionIndex ) =>
    {
        ...
        for ( const endpoint of [ "source", "target" ] as const )
        {
            ...
            if ( indicator != null && ( stateName != null || draftTransition.rememberedEvent != null ||
                !isChartIndicatorReferencePresent ( indicator, draft.chart.indicators ) ) )
            {
                addDiagnostic ( diagnostics, "CHART_REFERENCE_INVALID", "error", "chart",
                    "A draft indicator attachment is missing, conflicts with a state, or retains an event.",
                    "Use one existing state or indicator per endpoint without an event on indicator drafts.",
                    draftTransitionPath + "/" + endpoint + "_indicator" );
            }
            if ( stateName != null && !stateNames.has ( stateName ) )
            {
                addDiagnostic (
                    diagnostics, "CHART_REFERENCE_INVALID", "error", "chart",
                    "A draft endpoint references an undeclared state.",
                    "Attach the endpoint to a declared state or detach it.",
                    `${draftTransitionPath}/${endpoint}_state`,
                );
            }
        }
        if ( draftTransition.rememberedEvent != null && !eventNames.has ( draftTransition.rememberedEvent ) )
        {
            addDiagnostic (
                diagnostics, "CHART_REFERENCE_INVALID", "error", "chart",
                "A detached transition references an undeclared event.",
                "Choose a declared event or clear the remembered event.",
                draftTransitionPath + "/remembered_event",
            );
        }
        if ( draftTransition.rememberedTransitionIndex != null &&
                ...
                draftTransition.rememberedTransitionIndex >= MAXIMUM_TRANSITION_COUNT ) )
        {
            addDiagnostic (
                diagnostics, "CHART_REFERENCE_INVALID", "error", "chart",
                "A detached transition has an invalid remembered table position.",
                "Use a bounded non-negative table position with a remembered event, or clear the position.",
                draftTransitionPath + "/remembered_transition_index",
            );
        }
    ...
    const directRelationIdentifiers = new Set<number> ();
    const directRelationPairs = new Set<string> ();
    const indicatorTransitions = draft.chart.indicators.indicatorTransitions ?? [];
    if ( indicatorTransitions.length > MAXIMUM_CHART_DRAFT_TRANSITION_COUNT )
    {
        addDiagnostic ( diagnostics, "CHART_REFERENCE_INVALID", "error", "chart",
            "The indicator-relation capacity has been exceeded.", "Remove excess indicator relations.",
            "/chart/indicators/indicator_transitions" );
    }
    indicatorTransitions.forEach ( ( relation, index ) =>
    {
        const path = "/chart/indicators/indicator_transitions/" + index;
        const validReferences = isChartIndicatorReference ( relation.source ) && isChartIndicatorReference ( relation.target );
        const pair = validReferences ? chartIndicatorReferenceKey ( relation.source ) + ">" + chartIndicatorReferenceKey ( relation.target ) : "";
        if ( !Number.isSafeInteger ( relation.id ) || relation.id < 0 || directRelationIdentifiers.has ( relation.id ) ||
            !validReferences || !isChartIndicatorReferencePresent ( relation.source, draft.chart.indicators ) ||
            !isChartIndicatorReferencePresent ( relation.target, draft.chart.indicators ) ||
            !isAllowedChartIndicatorPair ( relation.source, relation.target ) || directRelationPairs.has ( pair ) )
        {
            addDiagnostic ( diagnostics, "CHART_REFERENCE_INVALID", "error", "chart",
                "An indicator relation has an invalid identity, missing endpoint, disallowed direction, or duplicate pair.",
                "Use a unique allowed relation between existing indicators.", path );
        }
        directRelationIdentifiers.add ( relation.id );
        directRelationPairs.add ( pair );
    } );
