...
// Description:
//
//   Selects the pinned initial state silently and copies the policy for this execution.
//
// Parameters:
//
//   - model:
...
// A terminated execution cannot emit more actions or consume another event before Reset.

export class RuntimeSessionTerminatedError extends Error
{
    public readonly code = "SESSION_TERMINATED";

    public constructor ()
    {
        super ( "Reset or create a session before advancing a terminated execution." );
        this.name = "RuntimeSessionTerminatedError";
    }
}
...
// Description:
//
//   Executes an exact event prefix, completing terminal entry before stopping the batch.
//
// Parameters:
//
//   - model:
...
): RuntimeOperationResult
{
    ...
    if ( session.executionStatus.kind === "terminated" )
    {
        throw new RuntimeSessionTerminatedError ();
    }

    let executionStatus: RuntimeExecutionStatus = { kind: "active" };
    ...
    if ( initialEntryActionsPending )
    {
        ...
        if ( session.executionPolicy.enableTerminalStates && initialState.terminalState )
        {
            executionStatus = { kind: "terminated", reason: "terminal_state", state: currentState };
        }
    }
    ...
    {
        if ( executionStatus.kind === "terminated" )
        {
            break;
        }
        // Initialize the local values needed by this operation.
        ...
        if ( session.executionPolicy.enableTerminalStates && destinationStateDefinition.terminalState )
        {
            executionStatus = { kind: "terminated", reason: "terminal_state", state: currentState };
        }
    }
