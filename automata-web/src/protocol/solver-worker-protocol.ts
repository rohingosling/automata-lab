...
//--------------------------------------------------------------------------------------------------
// Function: decodeStateDefinition
//
// Description:
//
//   Rejects missing, non-boolean, or true terminal flags in generated Solver candidate states.
//
//--------------------------------------------------------------------------------------------------

function decodeStateDefinition ( value: unknown ): StateDefinition | null
{
    const state = exactRecord ( value, [ "name", "description", "terminalState" ] );

    if ( state === null || state [ "terminalState" ] !== false )
    {
        return null;
    }

    const entity = decodeNamedEntity ( { name: state [ "name" ], description: state [ "description" ] } );

    return entity === null ? null : { ...entity, terminalState: false };
}
...
function decodeStateMachine ( value: unknown ): StateMachineDefinition<string> | null
{
    ...
    const states          = decodeArray ( stateMachine [ "states" ], MAXIMUM_STATE_COUNT, decodeStateDefinition );
...
function decodeDraftTransition ( value: unknown ): ChartDraftTransition | null
{
    ...
    const transition = exactRecord ( value, [ "id", "source", "target" ], [ "sourceState", "targetState", "sourceIndicator", "targetIndicator", "rememberedEvent", "rememberedTransitionIndex" ] );
    ...
    const sourceIndicator = transition [ "sourceIndicator" ];
    const targetIndicator = transition [ "targetIndicator" ];
    const sourceState = transition [ "sourceState" ];
    const targetState = transition [ "targetState" ];
    const rememberedEvent = transition [ "rememberedEvent" ];
    const rememberedTransitionIndex = transition [ "rememberedTransitionIndex" ];
    if ( sourceIndicator != null && ( !isChartIndicatorReference ( sourceIndicator ) || sourceState != null || rememberedEvent != null ) ||
        targetIndicator != null && ( !isChartIndicatorReference ( targetIndicator ) || targetState != null || rememberedEvent != null ) ||
        sourceState != null && typeof sourceState !== "string" ||
        targetState != null && typeof targetState !== "string" ||
        rememberedEvent != null && !isBoundedName ( rememberedEvent ) ||
        rememberedTransitionIndex != null && ( rememberedEvent == null ||
            !isNonNegativeSafeInteger ( rememberedTransitionIndex ) || rememberedTransitionIndex >= MAXIMUM_TRANSITION_COUNT ) )
    {
        return null;
    }
    return source === null || target === null ? null : {
        id: transition [ "id" ], source, target,
        ...( sourceIndicator === undefined ? {} : { sourceIndicator } ),
        ...( targetIndicator === undefined ? {} : { targetIndicator } ),
        ...( sourceState === undefined ? {} : { sourceState } ),
        ...( targetState === undefined ? {} : { targetState } ),
        ...( rememberedEvent === undefined ? {} : { rememberedEvent } ),
        ...( rememberedTransitionIndex === undefined ? {} : { rememberedTransitionIndex } ),
    };
}
...
function decodeIndicatorTransition ( value: unknown ): ChartIndicatorTransition | null
{
    const relation = exactRecord ( value, [ "id", "source", "target" ] );
    if ( relation === null || !isNonNegativeSafeInteger ( relation [ "id" ] ) ||
        !isChartIndicatorReference ( relation [ "source" ] ) || !isChartIndicatorReference ( relation [ "target" ] ) ||
        !isAllowedChartIndicatorPair ( relation [ "source" ], relation [ "target" ] ) )
    {
        return null;
    }
    return { id: relation [ "id" ], source: relation [ "source" ], target: relation [ "target" ] };
}
...
function decodeChart ( value: unknown ): ChartProjection | null
{
    ...
    const indicatorTransitions = decodeArray ( indicators [ "indicatorTransitions" ] === undefined ? [] : indicators [ "indicatorTransitions" ],
        MAXIMUM_CHART_DRAFT_TRANSITION_COUNT, decodeIndicatorTransition );
    ...
    if ( initialIndicatorValue !== null && initialStateIndicator === null || terminalStateIndicators === null ||
        terminalStateTransitions === null || indicatorTransitions === null || states === null || draftTransitions === null )
    {
        // Return the computed result.
    ...
    const chartIndicators = { initialStateIndicator, terminalStateIndicators, terminalStateTransitions,
        ...( indicators [ "indicatorTransitions" ] === undefined ? {} : { indicatorTransitions } ) };
    const relationIdentifiers = new Set<number> ();
    const relationPairs = new Set<string> ();
    for ( const relation of indicatorTransitions )
    {
        const pair = chartIndicatorReferenceKey ( relation.source ) + ">" + chartIndicatorReferenceKey ( relation.target );
        if ( relationIdentifiers.has ( relation.id ) || relationPairs.has ( pair ) ||
            !isChartIndicatorReferencePresent ( relation.source, chartIndicators ) ||
            !isChartIndicatorReferencePresent ( relation.target, chartIndicators ) )
        {
            return null;
        }
        relationIdentifiers.add ( relation.id );
        relationPairs.add ( pair );
    }
    if ( draftTransitions.some ( transition => [ transition.sourceIndicator, transition.targetIndicator ].some (
        reference => reference != null && !isChartIndicatorReferencePresent ( reference, chartIndicators ) ) ) )
    {
        return null;
    }
    return {
...
function decodeCandidate ( value: unknown ): SolverCandidate | null
{
    ...
    // Validate cross-references after both independently bounded structures have been decoded.

    const stateNames = new Set ( stateMachine.states.map ( state => state.name ) );
    const eventNames = new Set ( stateMachine.events.map ( event => event.name ) );
    if ( chart.draftTransitions.some ( transition =>
        ( transition.sourceState != null && !stateNames.has ( transition.sourceState ) ) ||
        ( transition.targetState != null && !stateNames.has ( transition.targetState ) ) ||
        ( transition.rememberedEvent != null && !eventNames.has ( transition.rememberedEvent ) ) ) )
    {
        return null;
    }
