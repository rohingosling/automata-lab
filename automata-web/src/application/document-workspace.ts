...
import { createTerminalStatePreparation } from "./chart-terminal-state-preparation.js";
import type { DocumentTerminalPreparation } from "../domain/model/commands.js";
...
): Promise<DocumentSaveResult>
{
    ...
    if ( !hasCoherentTerminalStateNotation ( validation.document ) )
    {
        return { status: "failed", diagnostics: [ createLifecycleDiagnostic (
            "TERMINAL_STATE_PREPARATION_REQUIRED", "Prepare terminal notation before saving.",
            "Run the revision-checked document preparation before capturing the save revision.",
        ) ] };
    }
...
export function planWorkspaceDocumentCommand (
    ...
    preparation: DocumentTerminalPreparation = createTerminalStatePreparation (),
): CommandPlanResult
{
    ...
    return workspace.editorState === null
        ...
        : planDocumentCommand ( workspace.editorState, command, preparation );
}
...
// Defensive repair is an explicit revision, before Save captures its immutable input. Cancellation
// of any later confirmation or destination selection therefore retains this undoable dirty change.

export function prepareDocumentWorkspace (
    workspace: DocumentWorkspaceState,
    preparation: DocumentTerminalPreparation = createTerminalStatePreparation (),
): WorkspaceCommandResult
{
    if ( workspace.editorState === null )
    {
        return { isSuccessful: true, workspace };
    }
    const prepared = preparation.prepare ( workspace.editorState.draft );
    if ( !prepared.isSuccessful )
    {
        const diagnostic = prepared.diagnostics [ 0 ];
        return { isSuccessful: false, code: "COMMAND_INVALID",
            message: diagnostic === undefined ? "Terminal notation preparation failed."
                : diagnostic.message + " " + diagnostic.remediation };
    }
    if ( prepared.document === workspace.editorState.draft )
    {
        return { isSuccessful: true, workspace };
    }
    const plan = planWorkspaceDocumentCommand ( workspace, {
        kind: "prepare_terminal_notation", expectedRevision: workspace.editorState.documentRevision,
    }, { prepare: () => prepared } );
    return plan.isSuccessful ? commitWorkspaceDocumentCommand ( workspace, plan.plan ) : plan;
}

export function mergeSavedDocumentWorkspace (
    current: DocumentWorkspaceState,
    captured: DocumentWorkspaceState,
    saved: DocumentWorkspaceState,
): DocumentWorkspaceState
{
    if ( current.editorState === captured.editorState )
    {
        return saved;
    }
    if ( current.editorState === null || captured.editorState === null ||
        current.editorState.cleanDraft !== captured.editorState.cleanDraft )
    {
        return current;
    }
    return { ...current, association: saved.association, displayName: saved.displayName,
        previousDocument: saved.previousDocument,
        editorState: { ...current.editorState, cleanDraft: captured.editorState.draft,
            dirty: current.editorState.draft !== captured.editorState.draft },
    };
}
