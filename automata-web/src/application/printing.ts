...
import { CANONICAL_FILE_VERSION } from "../domain/model/limits.js";
...
function createChartSection ( draft: AuthoringDraft ): PrintableChartSection
{
    ...
    return {
        draftTransitions: draft.chart.draftTransitions.map ( transition => ( {
            id:      transition.id,
            rememberedEvent: transition.rememberedEvent ?? null,
            sourceIndicator: transition.sourceIndicator == null ? null : { ...transition.sourceIndicator },
            targetIndicator: transition.targetIndicator == null ? null : { ...transition.targetIndicator },
            sourceState: transition.sourceState ?? null,
            targetState: transition.targetState ?? null,
            sourceX: transition.source.x,
            sourceY: transition.source.y,
            targetX: transition.target.x,
            targetY: transition.target.y,
        } ) ),
        indicatorTransitions: ( draft.chart.indicators.indicatorTransitions ?? [] ).map ( transition => ( {
            id:     transition.id,
...
{
    const prepared = createTerminalStatePreparation ( preferences ).prepare ( draft );
    if ( !prepared.isSuccessful )
    {
        throw new Error ( prepared.diagnostics.map ( diagnostic => diagnostic.message ).join ( " " ) );
    }
    ...
    return {
        ...
        fileName,
        fileVersion:              CANONICAL_FILE_VERSION,
