...
import { isSessionInitializationValid, isSessionAdvancementValid } from "./session-execution-validation.js";
...
//--------------------------------------------------------------------------------------------------
// Type: ActiveServerSessionReference
//
// Description:
//
//   Retains the accepted immutable snapshot for correlated policy and counter validation.
//
//--------------------------------------------------------------------------------------------------

export type ActiveServerSessionReference = HostedSessionDto;
...
// Reject the whole response before accepting a snapshot, cursor, trace, or diagnostic.

function invalidExecutionResponse (): ServerWorkspaceFailure
{
    return localFailure ( "SERVER_RESPONSE_INVALID", "The session execution result is inconsistent.",
...
export async function startServerSession (
    state: ServerWorkspaceState,
    gateway: ServerGateway,
    executionPolicy: RuntimeExecutionPolicy = { enableTerminalStates: false },
): Promise<ServerWorkspaceOperationResult<HostedSessionDto>>
{
    // Initialize the local values needed by this operation.

    const capturedPolicy = { ...executionPolicy };
    ...
    try
    {
        gatewayResult = await gateway.startSession ( capturedPolicy );
    }
    catch ( error )
    {
        // Recover from the reported failure without hiding its outcome.
    ...
    const responseFailure = validateSession ( gatewayResult.value, null, state.lastKnownHostedRevision ) ??
        ( isSessionInitializationValid ( gatewayResult.value, capturedPolicy ) ? null : invalidExecutionResponse () );
...
): Promise<ServerWorkspaceOperationResult<HostedSessionOperationResult>>
{
    ...
    const submittedEvents = [ ...eventBuffer ];
    if ( state.activeSession.executionStatus.kind === "terminated" )
    {
        return { isSuccessful: false, serverWorkspace: state, failure: localFailure (
            "SESSION_TERMINATED", "The session has reached a terminal state.",
            "Reset or create a session before running or stepping again.",
        ) };
    }
    ...
    try
    {
        gatewayResult = await operation ( { eventBuffer: submittedEvents, sessionId: state.activeSession.sessionId } );
    }
    catch ( error )
    {
        // Recover from the reported failure without hiding its outcome.
    ...
    ) ?? ( isSessionAdvancementValid ( state.activeSession, gatewayResult.value,
        submittedEvents.filter ( event => event.trim ().length > 0 ).length, isStep )
        ? null : invalidExecutionResponse () );
...
): Promise<ServerWorkspaceOperationResult<HostedSessionOperationResult>>
{
    ...
    return operateActiveSession ( state, eventBuffer, false, request => gateway.runSession ( request ) );
}
...
): Promise<ServerWorkspaceOperationResult<HostedSessionOperationResult>>
{
    ...
    return operateActiveSession ( state, eventBuffer, true, request => gateway.stepSession ( request ) );
}
...
): Promise<ServerWorkspaceOperationResult<HostedSessionDto>>
{
    ...
    try
    {
        gatewayResult = await gateway.resetSession ( state.activeSession.sessionId, capturedPolicy );
    }
    catch ( error )
    {
        // Recover from the reported failure without hiding its outcome.
    ...
    const responseFailure = validateSession ( gatewayResult.value, state.activeSession, state.lastKnownHostedRevision ) ??
        ( isSessionInitializationValid ( gatewayResult.value, capturedPolicy, state.activeSession )
            ? null : invalidExecutionResponse () );
