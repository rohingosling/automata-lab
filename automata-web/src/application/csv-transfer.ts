...
): CsvImportPreparationResult
{
    ...
    const hasTerminalColumn = entityKind === "state" && preparedRows.headerIndexes.has ( "terminal_state" );

    if ( hasTerminalColumn && !includeTerminalState )
    {
        return {
            isSuccessful: false,
            diagnostics: [ createCsvDiagnostic (
                "CSV_TERMINAL_PREPARATION_REQUIRED",
                "This States CSV contains terminal markings that this import path cannot yet preserve.",
                "Use the terminal-aware import workflow when it becomes available.",
            ) ],
        };
    }
    ...
    {
        const diagnostics = preparedRows.rows.filter ( row =>
        {
            const value = cellValue ( row, preparedRows.headerIndexes, "terminal_state" ).trim ().toLowerCase ();
            return value !== "true" && value !== "false";
        } ).map ( row => createCsvDiagnostic (
            "CSV_TERMINAL_STATE_INVALID",
            "Terminal State must be true or false; blank cells are not accepted.",
            "Correct the terminal_state cell before importing.",
            row.rowNumber,
        ) );
    ...
    const rows = preparedRows.rows.map ( row => ( {
        ...
        value:
        {
            ...
            ...( hasTerminalColumn ? {
                terminalState: cellValue ( row, preparedRows.headerIndexes, "terminal_state" )
...
): CsvImportPreparationResult
{
    ...
    switch ( transferKind )
    {
        ...
        case "states":
            ...
            return prepareNamedEntities ( text, "state", true );
...
): CsvExportDocument
{
    ...
    switch ( transferKind )
    {
        ...
        case "states":
            rows = [ [ "name", "description", "terminal_state" ], ...draft.stateMachine.states.map ( state =>
                [ state.name, state.description, String ( state.terminalState ) ] ) ];
            break;
...
// Explicit state projections also serve callers that do not use the transfer-kind dispatcher.
// A successful parse is an import proposal, never permission to commit incoherent Chart notation.

export function prepareTerminalStateCsvImport ( text: string ): CsvImportPreparationResult
{
    return prepareNamedEntities ( text, "state", true );
}

export function createTerminalStateCsvExportDocument ( draft: AuthoringDraft ): CsvExportDocument
{
    return {
        rowCount: draft.stateMachine.states.length,
        suggestedName: fileStem ( draft.settings.name ) + "-states.csv",
        text: serializeRows ( [
            [ "name", "description", "terminal_state" ],
            ...draft.stateMachine.states.map ( state => [
                state.name, state.description, String ( state.terminalState ),
            ] ),
        ] ),
    };
