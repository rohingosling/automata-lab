...
export class ServerState
{
    ...
    public startSession ( executionPolicy: RuntimeExecutionPolicy = { enableTerminalStates: false } ): ServerStateResult<ServerSessionSnapshot>
    {
        // Initialize the local values needed by this operation.
        ...
        for ( let attempt = 0; attempt <= MAXIMUM_SERVER_SESSION_COUNT; attempt++ )
        {
            ...
            const creationResult = this.sessions.create ( sessionId, hostedModelResult.value, executionPolicy );
    ...
    public resetSession ( sessionId: string,
        executionPolicy: RuntimeExecutionPolicy = { enableTerminalStates: false } ): ServerStateResult<ServerSessionSnapshot>
    {
        // Initialize the local values needed by this operation.

        const session = this.sessions.reset ( sessionId, executionPolicy );
    ...
    ): ServerStateResult<ServerSessionOperation>
    {
        ...
        if ( previousSession?.runtimeSession.executionStatus.kind === "terminated" )
        {
            return { isSuccessful: false, failure: createFailure (
                "SESSION_TERMINATED", "Reset or create a session before advancing a terminated execution.",
            ) };
        }
    ...
    private projectSession ( session: SimulationSessionRecord ): ServerSessionSnapshot
    {
        ...
        return {
            ...
            executionPolicy: { ...session.runtimeSession.executionPolicy },
            executionStatus: { ...session.runtimeSession.executionStatus },
