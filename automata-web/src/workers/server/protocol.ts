...
import type { RuntimeExecutionPolicy, RuntimeExecutionStatus } from "../../domain/runtime/contracts.js";
...
export const SERVER_PROTOCOL_VERSION = "automata-lab-server/2";
...
export interface ServerRequestPayloadByOperation
{
    ...
    readonly "simulation.start": RuntimeExecutionPolicy;
    ...
    readonly "simulation.reset": RuntimeExecutionPolicy;
...
export function decodeServerRequestEnvelope ( value: unknown ): ServerRequestDecodeResult
{
    ...
    switch ( operation )
    {
        ...
        {
            const executionPolicy = decodeExecutionPolicy ( payload );
            if ( executionPolicy === null || operation === "simulation.reset" && !isValidServerUuid ( sessionId ) )
            {
                return requestDecodeFailure ( value, "PAYLOAD_INVALID",
                    "Session creation and Reset require exactly one enableTerminalStates boolean." );
            }
            const common = { protocol: SERVER_PROTOCOL_VERSION, kind: "request", requestId,
                conditionalModelRevision: null, payload: executionPolicy } as const;
            return operation === "simulation.start"
                ? { isSuccessful: true, request: { ...common, operation, sessionId: null } }
                : { isSuccessful: true, request: { ...common, operation, sessionId: String ( sessionId ) } };
        }
...
// Execution objects use exact nested keys; state identity uses the semantic name bound.

function decodeExecutionPolicy ( value: unknown ): RuntimeExecutionPolicy | null
{
    return isRecord ( value ) && hasExactKeys ( value, [ "enableTerminalStates" ] ) &&
        typeof value [ "enableTerminalStates" ] === "boolean"
        ? { enableTerminalStates: value [ "enableTerminalStates" ] } : null;
}

function isStateName ( value: unknown ): value is string
{
    return typeof value === "string" && value.trim ().length > 0 &&
        [ ...value ].length <= MAXIMUM_NAME_CODE_POINT_COUNT;
}

function decodeExecutionStatus ( value: unknown ): RuntimeExecutionStatus | null
{
    if ( !isRecord ( value ) )
    {
        return null;
    }
    if ( value [ "kind" ] === "active" && hasExactKeys ( value, [ "kind" ] ) )
    {
        return { kind: "active" };
    }
    if ( value [ "kind" ] === "terminated" &&
        hasExactKeys ( value, [ "kind", "reason", "state" ] ) &&
        value [ "reason" ] === "terminal_state" && isStateName ( value [ "state" ] ) )
    {
        return { kind: "terminated", reason: "terminal_state", state: value [ "state" ] };
    }
    return null;
}
...
function decodeSessionSnapshot ( value: unknown ): ServerSessionSnapshot | null
{
    ...
    if ( !isRecord ( value ) || !hasExactKeys (
        ...
        typeof value [ "isStale" ] !== "boolean" || !isStateName ( value [ "currentState" ] ) ||
    ...
    const executionPolicy = decodeExecutionPolicy ( value [ "executionPolicy" ] );
    const executionStatus = decodeExecutionStatus ( value [ "executionStatus" ] );
    const transitionTrace = decodeTransitionTrace ( value [ "transitionTrace" ] );
    const actionTrace     = decodeActionTrace ( value [ "actionTrace" ] );

    // Handle the case where at least one branch condition is satisfied.

    if ( transitionTrace === null || actionTrace === null || executionPolicy === null ||
        executionStatus === null || executionStatus.kind === "terminated" &&
        ( !executionPolicy.enableTerminalStates || value [ "initialEntryActionsPending" ] ||
            executionStatus.state !== value [ "currentState" ] ) )
    {
        // Return the computed result.
...
function decodeSimulationOperationResult ( value: unknown ): ServerSimulationOperationResult | null
{
    ...
    if ( session === null || emittedActions === null || warnings === null ||
        value [ "consumedEventCount" ] > MAXIMUM_SERVER_EVENT_COUNT_PER_REQUEST ||
        session.initialEntryActionsPending )
    {
        // Return the computed result.
    ...
    if ( kind === "success" )
    {
        ...
        switch ( operation )
        {
            ...
            case "simulation.reset":
            {
                ...
                return result === null || result.executionStatus.kind !== "active" ||
                    !result.initialEntryActionsPending || result.processedEventCount !== 0 ||
                    result.actionTrace.length !== 0 || result.transitionTrace.length !== 0 || result.traceTruncated
            ...
            case "simulation.step":
            {
                ...
                return result === null || operation === "simulation.step" && result.consumedEventCount > 1
