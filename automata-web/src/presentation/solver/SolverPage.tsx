...
import type { CandidateReviewTab } from "../../application/ports/contracts.js";
...
interface SolverPageProperties
{
    ...
    readonly defaultReviewTab:  CandidateReviewTab;
            ...
            content = (
                <table className="data-table solver-review-table">
                    ...
                    <tbody>
                        ...
                        <tr><th scope="row">{ text ( "solver.candidate.terminalStates" ) }</th><td>{ currentDraft.stateMachine.states.filter ( state => state.terminalState ).length }</td><td>0</td></tr>
        ...
        case "solver-candidate-summary":
            content = (
                <div className="solver-summary-grid">
                    <p>{ text ( "solver.candidate.terminalReplacement" ).replace ( "{count}",
                        String ( currentDraft.stateMachine.states.filter ( state => state.terminalState ).length ) ) }</p>
                    <dl>
...
export function SolverPage ( properties: SolverPageProperties )
{
    ...
    const [ candidateReview, setCandidateReview ] = useState (
        { stateMachine: properties.solverWorkspace.candidate?.stateMachine ?? null, tab: properties.defaultReviewTab },
    );
    ...
    const candidateStateMachine = candidate?.stateMachine ?? null;

    // Chart-setting rebases replace the candidate wrapper while preserving its immutable machine.
    // Only a fresh inference result selects the default; rebases and stale reviews keep their tab.

    if ( candidateStateMachine !== candidateReview.stateMachine )
    {
        setCandidateReview ( { stateMachine: candidateStateMachine, tab: properties.defaultReviewTab } );
    }
        ...
        return (
            <section className="solver-candidate-review" aria-labelledby="solver-candidate-title">
                ...
                { CandidateReview (
                    ...
                    candidateReview.tab,
                    tab => setCandidateReview ( { stateMachine: candidate.stateMachine, tab } ),
                ) }
            </section>
        );
    }
