...
import type { ChartIndicatorReference } from "../../domain/model/contracts.js";
import { text } from "../../localization/messages.js";
import { createPrintPageStyle } from "./print-page-style.js";


//--------------------------------------------------------------------------------------------------
// Function: indicatorReferenceLabel
//
// Description:
//
//   Labels an immutable indicator reference without exposing persistence identifiers as state names.
//--------------------------------------------------------------------------------------------------

function indicatorReferenceLabel ( reference: ChartIndicatorReference ): string
{
    return reference.kind === "initial"
        ? text ( "chart.indicator.initial" )
        : `${text ( "chart.indicator.terminal" )} ${reference.id}`;
}


//--------------------------------------------------------------------------------------------------
// Function: draftEndpointLabel
//
// Description:
//
//   Describes a saved draft endpoint using its attachment and captured chart-space coordinates.
//--------------------------------------------------------------------------------------------------

function draftEndpointLabel (
    state: string | null,
    indicator: ChartIndicatorReference | null | undefined,
    x: number,
    y: number,
): string
{
    const attachment  = state ?? ( indicator == null ? null : indicatorReferenceLabel ( indicator ) );
    const coordinates = `${x}, ${y}`;

    return attachment === null ? coordinates : `${attachment} (${coordinates})`;
}
...
function ReportTable ( properties: ReportTableProperties )
{
    ...
    return (
        ...
        >
            <thead>
                <tr>{ properties.headings.map ( ( heading, index ) => (
                    <th
                        key   = { heading }
                        scope = "col"
                        style = { index === properties.compactColumn ? { width: "1%" } : undefined }
                    >
                        { heading }
                    </th>
                ) ) }</tr>
            ...
            >
                { properties.section.rows.map ( row => (
                    <tr key={ row.name }>
                        <th scope="row">{ row.name }</th>
                        <td>{ emptyValue ( row.description ) }</td>
                        <td>{ text ( row.terminalState ? "shared.yes" : "shared.no" ) }</td>
...
function ChartSection ( properties: { readonly section: PrintableChartSection } )
{
    ...
    return (
        <section className="print-report-section">
            ...
            <h3>{ text ( "report.chart.indicatorRelations" ) }</h3>
            <ReportTable
                headings={ [
                    text ( "report.column.identifier" ),
                    text ( "report.column.sourceIndicator" ),
                    text ( "report.column.targetIndicator" ),
                ] }
                label={ text ( "report.chart.indicatorRelations" ) }
            >
                { ( section.indicatorTransitions ?? [] ).map ( row => (
                    <tr key={ row.id }>
                        <th scope="row">{ row.id }</th>
                        <td>{ indicatorReferenceLabel ( row.source ) }</td>
                        <td>{ indicatorReferenceLabel ( row.target ) }</td>
                    </tr>
                ) ) }
            </ReportTable>
            ...
            >
                { section.draftTransitions.map ( row => (
                    <tr key={ row.id }>
                        <th scope="row">{ row.id }</th>
                        <td>{ row.rememberedEvent ?? "—" }</td>
                        <td>{ draftEndpointLabel (
                            row.sourceState, row.sourceIndicator, row.sourceX, row.sourceY,
                        ) }</td>
                        <td>{ draftEndpointLabel (
                            row.targetState, row.targetIndicator, row.targetX, row.targetY,
                        ) }</td>
                    </tr>
                ) ) }
            </ReportTable>
