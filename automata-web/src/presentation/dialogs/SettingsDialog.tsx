...
import type { ApplicationPreferences, CandidateReviewTab } from "../../application/ports/contracts";
import { BUTTON_COLORS, readButtonColor } from "../../application/button-colors.js";
...
type SettingsGroup = "appearance" | "chart" | "general" | "print" | "server" | "simulator" | "solver";
...
const SETTINGS_GROUPS: readonly SettingsGroupDefinition[] = [
    { labelKey: "settings.general", value: "general" },
    { labelKey: "settings.appearance", value: "appearance" },
    { labelKey: "settings.chart", value: "chart" },
    { labelKey: "settings.server", value: "server" },
    { labelKey: "settings.solver", value: "solver" },
    { labelKey: "settings.simulator", value: "simulator" },
    { labelKey: "settings.print", value: "print" },
];

const SOLVER_REVIEW_TABS: readonly { readonly value: CandidateReviewTab; readonly labelKey: MessageKey }[] =
[
    { value: "solver-candidate-summary", labelKey: "solver.candidate.summary" },
    { value: "solver-candidate-chart", labelKey: "solver.candidate.stateChart" },
    { value: "solver-candidate-states", labelKey: "solver.candidate.statesActions" },
    { value: "solver-candidate-transitions", labelKey: "solver.candidate.transitionTable" },
    { value: "solver-candidate-coverage", labelKey: "solver.candidate.traceCoverage" },
    { value: "solver-candidate-report", labelKey: "solver.candidate.inferenceReport" },
    { value: "solver-candidate-comparison", labelKey: "solver.candidate.comparison" },
];
    ...
    return (
        <FormField label={ properties.label } name={ properties.name }>
            <span className="checkbox-setting">
                <input
                    aria-describedby = { properties.description === undefined ? undefined : `${properties.name}-description` }
                ...
                { properties.description !== undefined && (
                    <>
                        <span aria-hidden="true" style={ { flexShrink: 0, visibility: "hidden" } }>WW</span>
                        <span className="field-description" id={ `${properties.name}-description` }>
...
export function SettingsDialog ( properties: SettingsDialogProperties )
{
    ...
    function renderDetail ()
    {
        ...
        if ( activeGroup === "general" )
        {
            ...
            return (
                <div className="settings-general-groups">
                    <fieldset className="settings-choice-group">
                        <legend>{ text ( "settings.fileSaving" ) }</legend>
                        <CheckboxSetting
                    ...
                    <fieldset className="settings-choice-group">
                        <legend>{ text ( "settings.console" ) }</legend>
                        <CheckboxSetting
        ...
        if ( activeGroup === "appearance" )
        {
            ...
            return (
                <div className="settings-appearance-groups">
                    <fieldset className="settings-choice-group">
                    ...
                    <fieldset className="settings-choice-group">
                        <legend>{ text ( "settings.applicationColors" ) }</legend>
                        <FormField label={ text ( "settings.titleBarColor" ) } name="settings-title-bar-color">
                            <select
                                id       = "settings-title-bar-color"
                                value    = { properties.preferences.titleBarColor }
                                onChange = { event => updateDraft (
                                    {
                                        titleBarColor: readButtonColor (
                                            event.currentTarget.value, properties.preferences.titleBarColor,
                                        ),
                                    },
                                ) }
                            >
                                { BUTTON_COLORS.map ( color => (
                                    <option key={ color } value={ color }>
                                        { text ( `settings.buttonColor.${color}` ) }
                                    </option>
                                ) ) }
                            </select>
                        </FormField>
                        <FormField label={ text ( "settings.applicationButtons" ) } name="settings-application-buttons">
                            <select
                                id       = "settings-application-buttons"
                                value    = { properties.preferences.applicationButtonColor }
                                onChange = { event => updateDraft (
                                    {
                                        applicationButtonColor: readButtonColor (
                                            event.currentTarget.value,
                                            properties.preferences.applicationButtonColor,
                                        ),
                                    },
                                ) }
                            >
                        ...
                        <FormField label={ text ( "settings.consoleMessageButtons" ) } name="settings-console-message-buttons">
                            <select
                                disabled = { properties.preferences.matchConsoleMessageColor }
                                id       = "settings-console-message-buttons"
                                value    = { properties.preferences.consoleMessageButtonColor }
                                onChange = { event => updateDraft (
                                    {
                                        consoleMessageButtonColor: readButtonColor (
                                            event.currentTarget.value,
                                            properties.preferences.consoleMessageButtonColor,
                                        ),
                                    },
                                ) }
                            >
                        ...
                        <CheckboxSetting
                            checked  = { properties.preferences.matchConsoleMessageColor }
                            label    = { text ( "settings.matchConsoleMessageColor" ) }
                            name     = "settings-match-console-message-color"
                            onChange = { matchConsoleMessageColor => updateDraft ( { matchConsoleMessageColor } ) }
                        />
                    </fieldset>
                </div>
            );
        ...
        if ( activeGroup === "solver" )
        {
            return (
                <div className="settings-solver-groups">
                    <fieldset className="settings-choice-group">
                        <legend>{ text ( "settings.results" ) }</legend>
                        <FormField
                            label = { text ( "settings.defaultResultsTab" ) }
                            name  = "settings-default-results-tab"
                        >
                            <select
                                id       = "settings-default-results-tab"
                                value    = { properties.preferences.solverDefaultReviewTab }
                                onChange = { event =>
                                {
                                    const selectedTab = SOLVER_REVIEW_TABS.find (
                                        tab => tab.value === event.currentTarget.value,
                                    );
                                    if ( selectedTab !== undefined )
                                    {
                                        updateDraft ( { solverDefaultReviewTab: selectedTab.value } );
                                    }
                                } }
                            >
                                { SOLVER_REVIEW_TABS.map ( tab => (
        ...
        if ( activeGroup === "simulator" )
        {
            return (
                <div className="settings-simulator-groups">
                    <fieldset className="settings-choice-group">
                        <legend>{ text ( "settings.modelParameters" ) }</legend>
                        <CheckboxSetting
                            checked     = { properties.preferences.simulatorEnableTerminalStates }
                            description = { text ( "settings.enableTerminalStates.description" ) }
                            label       = { text ( "settings.enableTerminalStates" ) }
                            name        = "settings-enable-terminal-states"
                            onChange    = { simulatorEnableTerminalStates => updateDraft ( { simulatorEnableTerminalStates } ) }
                        />

                    </fieldset>
                    <fieldset className="settings-choice-group">
                        <legend>{ text ( "settings.traces" ) }</legend>
                        <CheckboxSetting
                            checked  = { properties.preferences.simulatorFollowTraceTail }
                            label    = { text ( "settings.followTraceTail" ) }
                            name     = "settings-follow-trace-tail"
                            onChange = { simulatorFollowTraceTail => updateDraft ( { simulatorFollowTraceTail } ) }
                        />
                    </fieldset>
                </div>
            );
        ...
        if ( activeGroup === "server" )
        {
            ...
            return (
                <div className="settings-server-groups">
                    <fieldset className="settings-choice-group">
                        <legend>{ text ( "settings.builtInServer" ) }</legend>
                        ...
                        <button
                            className = "settings-server-test"
