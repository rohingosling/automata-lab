...
import { useMeasuredInlineSize } from "../shared/useMeasuredInlineSize";
...
interface EditorWorkspaceProperties
{
    readonly rowNavigation?: { readonly key: string; readonly sequence: number } | undefined;
...
const NAMED_ENTITY_COLUMNS: readonly DataGridColumn<NamedEntityEditorValue>[] =
[
    {
    ...
    {
        heading: text ( "field.description" ),
...
function CatalogPage ( properties: CatalogPageProperties )
{
    ...
    return (
        <div className="editor-list-page">
            <DataGrid
                columns              = { NAMED_ENTITY_COLUMNS }
                getKey               = { entity => entity.name }
                label                = { properties.label }
                onRowSelectionChange = { setSelectedName }
                rows                 = { properties.entities }
                selectedKey          = { selectedName }
                rowNavigation        = { properties.rowNavigation }
            />
            <ListActions
            ...
            { dialogMode !== null && (
                <NamedEntityDialog
                    entityKind = { properties.entityKind }
                    mode       = { dialogMode }
...
function StatesPage ( properties: Pick <EditorWorkspaceProperties, "onCommand" | "rowNavigation"> & { readonly draft: EditorDraftViewModel } )
{
    // Initialize the local values needed by this operation.
    ...
    const statesListPaneReference             = useRef<HTMLElement> ( null );
    const measuredListWidth                   = useMeasuredInlineSize ( statesListPaneReference );
    ...
    return (
        <div
            className = "states-editor"
            style     = { ( listWidth === null
                ? {}
                : { "--state-list-width": `${listWidth}px` } ) as CSSProperties }
        >
            <section className="states-list-pane" ref={ statesListPaneReference }>
                <DataGrid
                    columns              = { NAMED_ENTITY_COLUMNS }
                    getKey               = { state => state.name }
            ...
            <Splitter
                ...
                value           = { listWidth ?? Math.max ( measuredListWidth, MINIMUM_EDITOR_ACTION_PANE_WIDTH ) }
            />
            ...
            { dialogMode !== null && (
                <NamedEntityDialog
                    ...
                    initialValue={ dialogMode === "edit" && selectedEntity !== undefined
                        ? selectedEntity
                        : { name: "", description: "", terminalState: false } }
                    onClose   = { () => setDialogMode ( null ) }
                    onConfirm = { entity => properties.onCommand ( expectedRevision => dialogMode === "add"
...
function TransitionTablePage ( properties: Pick <EditorWorkspaceProperties, "onCommand" | "rowNavigation"> & { readonly draft: EditorDraftViewModel } )
{
    // Initialize the local values needed by this operation.
...
export function EditorWorkspace ( properties: EditorWorkspaceProperties )
{
    ...
    switch ( properties.route )
    {
        ...
        case "states":
            ...
            return <StatesPage rowNavigation={ properties.rowNavigation } draft={ properties.draft } onCommand={ properties.onCommand } />;
        ...
        case "events":
            ...
            return <CatalogPage key={ properties.route } rowNavigation={ properties.rowNavigation } entities={ properties.draft.stateMachine.events } entityKind="event" label={ text ( "editor.events.label" ) } onCommand={ properties.onCommand } />;
        ...
        case "actions":
            ...
            return <CatalogPage key={ properties.route } rowNavigation={ properties.rowNavigation } entities={ properties.draft.stateMachine.actions } entityKind="action" label={ text ( "editor.actions.label" ) } onCommand={ properties.onCommand } />;
        ...
        case "transitionTable":
            ...
            return <TransitionTablePage rowNavigation={ properties.rowNavigation } draft={ properties.draft } onCommand={ properties.onCommand } />;
