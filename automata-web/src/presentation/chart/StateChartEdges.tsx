...
//--------------------------------------------------------------------------------------------------
// Interface: StateChartRelationGeometry
//
// Description:
//
//   Shares routing and curve presentation data across semantic and unfinished relations.
//
//--------------------------------------------------------------------------------------------------

export interface StateChartRelationGeometry extends Record<string, unknown>
{
    ...
    readonly sourceBoundary?:                 ChartNodeBoundary | undefined;
    readonly targetBoundary?:                 ChartNodeBoundary | undefined;
}
...
// Description:
//
//   Adds required state boundaries and technical handles for configured React Flow edges.
//
//--------------------------------------------------------------------------------------------------

interface StateChartCenterGeometry extends StateChartRelationGeometry
{
    readonly endpointPreview?: {
        readonly source: ChartPoint;
        readonly target: ChartPoint;
        readonly geometry: StateChartRelationGeometry;
    };
...
export type StateChartEdgeData = Record<string, unknown> & StateChartCenterGeometry & (
    ...
    | { readonly kind: "indicator"; readonly indicatorTransitionId: number }
    | {
...
function calculateOrthogonalGeometry (
    ...
    data: StateChartRelationGeometry,
): RoutedBackboneGeometry
{
...
): CubicBezierCurve[] | null
{
    ...
    // A free endpoint keeps its exact curve endpoint; an attached end clips at its boundary.

    const sourceLocation = clipping.sourceBoundary === undefined
        ? { curveIndex: 0, position: 0 }
        : boundaryExitLocation ( curves, clipping.sourceCenter, clipping.sourceBoundary, true );
    const targetLocation = clipping.targetBoundary === undefined
        ? { curveIndex: curves.length - 1, position: 1 }
        : boundaryExitLocation ( curves, clipping.targetCenter, clipping.targetBoundary, false );
...
): StateChartEdgeGeometry
{
    ...
    const source          = data.sourceBoundary === undefined
        ? center : rayBoundary ( center, center, sourceDirection, data.sourceBoundary );
    const target          = data.targetBoundary === undefined
        ? center : rayBoundary ( center, center, targetDirection, data.targetBoundary );
...
): ChartRoutingRelation
{
    ...
    return {
        ...
        ...( data.selfLoopIndex === null
            ? {
                ...( data.sourceBoundary === undefined ? {} : { sourceBoundary: data.sourceBoundary } ),
                ...( data.targetBoundary === undefined ? {} : { targetBoundary: data.targetBoundary } ),
            }
            : {} ),
    };
}
...
export function StateChartEdgeLabel ( properties: {
...
export function StateChartCenterEdgeComponent ( properties: EdgeProps<StateChartEdge> )
{
    ...
    const preview = properties.data.endpointPreview;
    const geometry = preview === undefined
        ? calculateCenterRoutedEdgeGeometry (
            { x: properties.sourceX, y: properties.sourceY },
            { x: properties.targetX, y: properties.targetY },
            properties.data,
        )
        : calculateCenterRoutedEdgeGeometryFromCenters ( preview.source, preview.target, preview.geometry );
