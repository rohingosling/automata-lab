...
import { resolveDraftTransitionEndpoints } from "../../domain/model/draft-transition-attachments.js";
import
{
    Background,
    BackgroundVariant,
...
function resolveAttachedDraftNodes ( chartNodes: readonly StateChartNode[] ): StateChartNode[]
{
    ...
    return chartNodes.map ( node =>
    {
        ...
        const resolved = resolveDraftTransitionEndpoints ( current, centers );
...
): StateChartNode[]
{
    ...
    return chartNodes.map ( node =>
    {
        ...
        const replacement     = endpoint === "source"
            ? { ...draftTransition, source: point, sourceState: state }
            : { ...draftTransition, target: point, targetState: state };
...
): ChartFlowRelations
{
    ...
    for ( const node of flowNodes )
    {
        ...
        draftRelations.set ( node.id, relationGeometry (
            node.id,
            transition.sourceState == null ? undefined : stateNodesByName.get ( transition.sourceState ),
            transition.targetState == null ? undefined : stateNodesByName.get ( transition.targetState ),
    ...
    return { edges: [ ...transitionEdges, ...initialEdges, ...terminalEdges ], draftRelations };
}
...
function ChartCanvas ( properties: ChartCanvasProperties )
{
    ...
    ): boolean =>
    {
        ...
        const previousState = ( endpoint === "source" ? current.sourceState : current.targetState ) ?? null;
        if ( previousState === state && chartPointsApproximatelyEqual ( current [ endpoint ], point ) )
        {
            return false;
        }
        const replacement = endpoint === "source"
            ? { ...current, source: point, sourceState: state }
            : { ...current, target: point, targetState: state };
        const completed = state !== null && previousState !== state &&
            replacement.sourceState != null && replacement.targetState != null;
        const committed = onCommand ( expectedRevision => ( {
            kind: "update_chart_draft_endpoint", draftTransitionId, endpoint, point, state, expectedRevision,
        } ) );
        if ( !committed )
        {
            return false;
        }
        if ( completed && replacement.rememberedEvent != null )
        {
            onFocusAfterRevision ( {
                kind: "edge",
                identifier: transitionEdgeIdentifier ( replacement.sourceState ?? "", replacement.rememberedEvent ),
            } );
        }
        else if ( completed )
        {
            ...
            onDraftConfigurationRequest ( {
                ...
                initialValue: {
                    state: replacement.sourceState ?? "", stateNext: replacement.targetState ?? "", event: "",
                },
            } );
        }
        else
    ...
    function handleDraftEndpointPointerDown ( event: PointerEvent<HTMLButtonElement> ): void
    {
        ...
        const drag         = {
            ...
            initialState: ( endpointTarget.endpoint === "source"
                ? draftNode.data.sourceState : draftNode.data.targetState ) ?? null,
        };
    ...
    function finishDraftEndpointPointerMove ( event: PointerEvent<HTMLButtonElement>, cancelled: boolean ): void
    {
        ...
        if ( drag.initialState !== null && chartPointsApproximatelyEqual ( pointerPoint, drag.initialPointer ) )
        {
            setNodes ( [ ...drag.originalNodes ] );
            return;
        }
        ...
        const updatedNodes = replaceDraftTransitionEndpoint (
            ...
            drag.endpoint,
            snappedPoint,
            releasedState === null ? null : stateNameFromChartIdentifier ( releasedState.identifier, currentNodes ),
        );
        ...
        const releasedStateName = releasedState === null
            ? null : stateNameFromChartIdentifier ( releasedState.identifier, currentNodes );
        const storedStateName = ( drag.endpoint === "source"
            ? storedDraftTransition?.sourceState : storedDraftTransition?.targetState ) ?? null;
        if ( storedStateName === releasedStateName && storedDraftTransition !== undefined && chartPointsApproximatelyEqual (
        ...
        if ( !commitDraftEndpoint ( drag.draftTransitionId, drag.endpoint, snappedPoint, releasedStateName ) )
        {
            setNodes ( [ ...drag.originalNodes ] );
    ...
    const flushKeyboardGeometry = useCallback ( (): void =>
    {
        ...
        if ( pendingEndpoint !== null )
        {
            if ( !commitDraftEndpoint ( pendingEndpoint.draftTransitionId, pendingEndpoint.endpoint,
                pendingEndpoint.point, pendingEndpoint.state ) )
            {
        ...
        } ).then ( result =>
        {
            ...
            if ( focusBeforeRouteUpdate !== null )
            {
                window.setTimeout ( () =>
                {
                    ...
                    if ( active && document.contains ( focusBeforeRouteUpdate ) &&
    ...
    function moveFocusedDraftEndpoint ( event: KeyboardEvent<HTMLDivElement> ): boolean
    {
        ...
        const retainedState = endpointTarget.endpoint === "source" ? transition.sourceState : transition.targetState;
        const currentStateIdentifier = retainedState == null
            ? stateContainingPoint ( endpoint, currentNodes )?.identifier ?? null
            : stateNodeIdentifier ( retainedState );
        ...
        const movement = proposedState !== null && proposedState.identifier !== currentStateIdentifier
            ...
            : currentStateIdentifier !== null
                ...
                : text ( "chart.announcement.moved" );
        const attachedState = proposedState !== null && proposedState.identifier !== currentStateIdentifier
            ? stateNameFromChartIdentifier ( proposedState.identifier, currentNodes ) : null;
        const updatedNodes = replaceDraftTransitionEndpoint (
            currentNodes,
        ...
        pendingDraftEndpointReference.current = {
            ...
            point: snappedPoint, state: attachedState,
        };
