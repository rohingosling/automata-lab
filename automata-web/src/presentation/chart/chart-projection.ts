...
export const CHART_INDICATOR_SYMBOL_DIAMETERS = { initial: 50, terminal: 60 } as const;
...
//--------------------------------------------------------------------------------------------------
// Function: resolveAuthoringDraftAttachments
//
// Description:
//
//   Captures current attachment coordinates for deletion and immutable report projections.
//--------------------------------------------------------------------------------------------------

export function resolveAuthoringDraftAttachments (
    draft: AuthoringDraft,
    preferences: ApplicationPreferences,
): AuthoringDraft
{
    if ( !draft.chart.draftTransitions.some ( transition =>
        transition.sourceState != null || transition.targetState != null ||
        transition.sourceIndicator != null || transition.targetIndicator != null ) )
    {
        return draft;
    }
    const projection = createAuthoringChartProjection ( draft, [], {
        actionNames: preferences.wrapActionNames,
        eventNames: preferences.wrapEventNames,
        stateNames: preferences.wrapStateNames,
    }, preferences );
    const centers = new Map ( projection.states.map ( state => [ state.name, {
        x: state.x + state.width / 2, y: state.y + state.height / 2,
    } ] ) );
    return {
        ...draft,
        chart: { ...draft.chart, draftTransitions: draft.chart.draftTransitions.map ( transition =>
            resolveDraftTransitionEndpoints ( transition, centers, draft.chart.indicators ) ) },
    };
}
