...
export function Splitter ( properties: SplitterProperties )
{
    ...
    useEffect ( () =>
    {
        ...
        {
            // Hidden layouts have no divider range and must retain their stored pane sizes.

            if ( observedSplitter.getClientRects ().length === 0 )
            {
                setMeasuredMaximum ( null );
                return;
            }

            const vertical       = properties.orientation === "vertical";
            const containerStyle = window.getComputedStyle ( observedContainer );
            const pixelValue     = ( value: string ): number => Number.parseFloat ( value ) || 0;
            const paddingSize    = vertical
                ? pixelValue ( containerStyle.paddingLeft ) + pixelValue ( containerStyle.paddingRight )
                : pixelValue ( containerStyle.paddingTop ) + pixelValue ( containerStyle.paddingBottom );
            const range = ( vertical ? observedContainer.clientWidth : observedContainer.clientHeight ) - paddingSize;
            const children = Array.from ( observedContainer.children ).filter (
                child => child.getClientRects ().length > 0,
            );
            const gutterSize = pixelValue ( vertical ? containerStyle.columnGap : containerStyle.rowGap ) *
                Math.max ( 0, children.length - 1 );
            const separatorSize = children.reduce ( ( totalSize, child ) =>
            {
                if ( !child.classList.contains ( `splitter-${properties.orientation}` ) )
                {
                    return totalSize;
                }
                ...
                return totalSize + ( vertical
                    ? bounds.width + pixelValue ( style.marginLeft ) + pixelValue ( style.marginRight )
                    : bounds.height + pixelValue ( style.marginTop ) + pixelValue ( style.marginBottom ) );
            }, 0 );

            if ( range > 0 )
            {
                // Multi-pane callers reserve fixed siblings as well as the flexible pane minimum.

                const fractionMaximum     = Math.floor ( range * MAXIMUM_SPLITTER_RANGE_FRACTION );
                const opposingPaneMaximum = range - separatorSize - gutterSize - ( properties.opposingMinimum ?? 0 );
        ...
        // Pane sizes also change when divider gaps change without resizing the container.

        for ( const child of observedContainer.children )
        {
    ...
    useEffect ( () =>
    {
        ...
        if ( splitterReference.current?.getClientRects ().length !== 0 && measuredMaximum !== null &&
            ( properties.value < properties.minimum || properties.value > measuredMaximum ) )
        {
