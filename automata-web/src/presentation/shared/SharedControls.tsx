...
export function DataGrid<RowValue> ( properties: DataGridProperties<RowValue> )
{
    ...
    useEffect ( () =>
    {
        const request = properties.rowNavigation;
        if ( request === undefined || handledNavigation.current === request.sequence )
        {
            return;
        }
        const rowIndex = properties.rows.findIndex ( row => properties.getKey ( row ) === request.key );
        if ( rowIndex < 0 )
        {
            handledNavigation.current = request.sequence;
            return;
        }
        const element = cellReferences.current.get ( cellIdentifier ( rowIndex, 0 ) );
        if ( element === undefined )
        {
            progressiveRendering.revealThrough ( rowIndex );
            return;
        }
        handledNavigation.current = request.sequence;
        properties.onRowSelectionChange?.( request.key );
        element.focus ();
        element.scrollIntoView?.( { block: "nearest", inline: "nearest" } );
    }, [ properties, progressiveRendering ] );
