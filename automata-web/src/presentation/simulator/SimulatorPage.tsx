...
export interface SimulatorPageProperties
{
    ...
    readonly followTraceTail:           boolean;
    readonly onAdditionalHeightChange?: ( height: number ) => void;
    readonly onBufferChange?:          () => void;
...
export function SimulatorPage ( properties: SimulatorPageProperties )
{
    ...
    const blockerReference             = useRef<HTMLDivElement> ( null );
    const blockerCount                 = properties.availability.blockers.length;
    const draftMissing                 = properties.draft === null;
    const onAdditionalHeightChange     = properties.onAdditionalHeightChange;
    ...
    // Reserve the optional message and its row gap in addition to the shell's normal pane minimum.

    useLayoutEffect ( () =>
    {
        const blocker = blockerReference.current;
        if ( blocker === null )
        {
            onAdditionalHeightChange?.( 0 );
            return;
        }

        const observedBlocker = blocker;

        function measureAdditionalHeight (): void
        {
            const rowGap = Number.parseFloat ( window.getComputedStyle ( observedBlocker.parentElement! ).rowGap ) || 0;
            onAdditionalHeightChange?.( Math.ceil ( observedBlocker.getBoundingClientRect ().height + rowGap ) );
        }

        measureAdditionalHeight ();
        if ( typeof ResizeObserver === "undefined" )
        {
            window.addEventListener ( "resize", measureAdditionalHeight );
            return () =>
            {
                window.removeEventListener ( "resize", measureAdditionalHeight );
                onAdditionalHeightChange?.( 0 );
            };
        }

        const observer = new ResizeObserver ( measureAdditionalHeight );
        observer.observe ( observedBlocker );
        return () =>
        {
            observer.disconnect ();
            onAdditionalHeightChange?.( 0 );
        };
    }, [ blockerCount, draftMissing, onAdditionalHeightChange ] );
    ...
    return (
        <section className="simulator-workspace">
            { properties.availability.blockers.length > 0 && (
                <div className="simulator-blocked" ref={ blockerReference } role="status">
            ...
            >
                ...
                <Splitter
                    ...
                    opposingMinimum = { Math.max ( eventEditorWidth, eventEditorMinimumWidth ) + MINIMUM_INSPECTOR_WIDTH }
                ...
                <Splitter
                    ...
                    opposingMinimum = { Math.max ( sequenceListWidth, sequenceListMinimumWidth ) + MINIMUM_INSPECTOR_WIDTH }
                    orientation     = "vertical"
                ...
                <div className="simulator-inspector">
                    ...
                    >
                        ...
                        <TraceTable
                            ...
                            followTail   = { properties.followTraceTail }
            ...
            <div className="detail-button-panel simulator-command-panel">
                ...
                <button
                    disabled = { !commandsEnabled || session?.executionStatus.kind === "terminated" }
                    onClick  = { () =>
                    {
                        commitEvents ();
