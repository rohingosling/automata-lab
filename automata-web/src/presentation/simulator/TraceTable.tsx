...
//
//   When Follow Trace Tail is enabled, a table at its end follows each newly appended row.
//   Scrolling away releases the follow, so a user reading earlier rows is not dragged to the
//   bottom by traffic still arriving, and returning to the end resumes it.
//
...
export function TraceTable<TEntry> ( properties: TraceTableProperties<TEntry> )
{
    ...
    const previousFollowTail = useRef ( properties.followTail );
    ...
    // the table first mounted. Attach after an empty trace receives its first entries, since the
    // scrolling element does not exist while the empty message is shown.
    ...
    useEffect ( () =>
    {
        ...
        function measureViewport (): void
        {
            ...
            const bottomPadding = Number.parseFloat ( scrollStyle.paddingBottom ) || 0;

            // Bottom clearance occupies viewport space without becoming part of a trace row.

            setViewportHeight ( Math.max ( 0, scrollElement.clientHeight - bottomPadding ) );
        }
    ...
    // no-ops so the sequence terminates rather than cycling. Recheck after viewport or clearance
    // changes as well, preserving the trailing inset when the trace remains pinned.
    ...
    useLayoutEffect ( () =>
    {
        ...
        const scrollElement     = scrollReference.current;
        const followTailChanged = previousFollowTail.current !== properties.followTail;

        previousFollowTail.current = properties.followTail;

        if ( scrollElement === null || !properties.followTail )
        {
            // Return control to the caller.
        ...
        // Enabling follow respects the current reading position. Rows appended while disabled
        // may have moved the end even though no scroll event changed the earlier pinned state.

        if ( followTailChanged )
        {
            const isAtEnd = endPosition - scrollElement.scrollTop <= END_PROXIMITY_THRESHOLD;
    ...
    }, [ entryCount, isPinnedToEnd, properties.followTail, scrollTop, viewportHeight ] );
    ...
    if ( entryCount === 0 )
    {
        // Clearing removes the scroll element. Reset its virtual counterpart before a new trace
        // mounts at scrollTop zero, and let an enabled follow start afresh with the new entries.
