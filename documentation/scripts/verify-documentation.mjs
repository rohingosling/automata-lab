...
function verifyHtmlReferences ( artifactPath, htmlContent, outputPathSet, htmlContentByPath )
{
    ...
    const imagePattern = /<img\b(?<attributes>[^>]*)>/gu;

    for ( const match of htmlContent.matchAll ( imagePattern ) )
    {
        const attributes      = match.groups?.attributes ?? "";
        const source          = /\bsrc="(?<value>[^"]+)"/u.exec ( attributes )?.groups?.value ?? "";
        const alternativeText = /\balt="(?<value>[^"]*)"/u.exec ( attributes )?.groups?.value ?? "";

        if ( !source.startsWith ( DOCUMENTATION_BASE_PATH ) )
        {
            throw new Error ( `${ artifactPath } loads a non-local image '${ source }'.` );
        }

        const resolvedImage = resolveDocumentationReference ( artifactPath, source );

        if ( resolvedImage === null || !outputPathSet.has ( resolvedImage.outputPath ) )
        {
            throw new Error ( `${ artifactPath } loads a missing image '${ source }'.` );
        }

        if ( alternativeText.trim () === "" )
        {
            throw new Error ( `${ artifactPath } has an image without descriptive alternative text.` );
        }
    }
