...
The static Pages artifact contains no documentation `node_modules` tree or development server. The pinned
`vitepress@1.6.4` package uses an older Vite development toolchain; recheck advisories and supported upgrade options
when reviewing dependency changes. Integrated application development builds and hardens the guides, then
