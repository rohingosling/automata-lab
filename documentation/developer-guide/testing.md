...
control typography, the configured corner radius, and checks that narrow content fits inside group borders.
Settings coverage checks the current category order, named control groups, equal side clearance with and without
scrollbars, and centered checkbox labels at desktop and narrow widths. It also verifies Cancel, Apply, and reload
for Solver Default Results Tab and Simulator Follow Trace Tail.
...
`tests/shell/preferences.test.ts` covers defaults, allowlisting, persistence, invalid values, and older stored
envelopes for the two preferences. `SettingsDialog.test.tsx` verifies that both fields participate in the same
complete draft transaction as other settings. `SolverPage.test.tsx` checks all default review views on new
candidates and preserves a reader's chosen tab on an existing candidate.

`SimulatorPage.test.tsx` verifies propagation to both traces. `TraceTable.test.tsx` covers appends and resizes while
following is disabled, preserving the reading position when re-enabled, and resuming only at the end. Shell
integration also checks that applying the preferences does not dirty the project or change its model data.
...
`tests/shell/SettingsDialog.test.tsx` covers all three dropdowns, the unchecked matching default, disabled-state/value retention, and draft transactions.
...
They compare matched fills/text/icons against the shared palettes, check manual Light Gray/White
mapping and standard black borders, preserve severity-label colors and run a Console-scoped Axe
...
## Console Navigation and Columns

`tests/shell/ConsoleEntityNavigation.test.tsx` covers token lifetime, transition reordering, unrelated
edits, rename/delete/recreate, and replacement invalidation. `ConsolePanel.test.tsx` covers concise
labels, column headers, widths, keyboard resizing, and reset. The Console cases in
`tests/browser/shell.spec.ts` exercise dragging, sticky alignment and horizontal scrolling at several
viewport sizes, all four entity destinations, progressive row reveal, repeated focus, historical
fallback, and Light/Dark grips. These supplement the palette coverage described above.
...
The main application JavaScript budget is 1,108,000 uncompressed bytes and the application CSS budget
is 68,000 bytes. Version 1.7.0 raises these per-file allowances for Console columns and entity navigation.
Worker and aggregate JavaScript limits remain unchanged: total JavaScript is limited to 2,825,000 bytes.
The application artifact allowance is 3,055,000 bytes, including the measured Windows text-ending
overhead in a promoted clean-room build. These build safeguards are separate from compressed transfer
size and browser capabilities. The executable authority is `scripts/artifact-verifier.mjs`.
...
The legacy light-switch example and its current terminal-state counterpart are maintained end-to-end documents.
Strict codec/schema tests verify identical migrated models and stable canonical `1.4.0` round trips. Larger comprehensive,
