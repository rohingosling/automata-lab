...
Each chapter ends with explicit Previous and Next navigation in addition to the sidebar.
...
Use normative words only when describing an actual contract. Do not claim global Solver minimality, terminal-state inference, remote Server support, automatic persistence,
or another feature the product does not provide. Describe terminal stopping as conditional on the policy
captured at session creation or Reset; keep that preference separate from state-owned terminal flags.
