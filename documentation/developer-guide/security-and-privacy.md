...
Automatic layout, transition routing, Solver inference, and the built-in Server run in dedicated Workers. Routing,
Solver, and Server use application-owned protocols with bounded identifiers, operation-specific payloads, exact keys,
and validated results. Layout uses ELK's Worker API behind `BrowserChartLayoutPort`; that adapter bounds node and edge
counts, enforces a three-second timeout, validates returned state identities and finite coordinates, and disposes the
layout Worker after the job.

Adapters correlate work, ignore stale generations, and suppress late results. Layout, routing, and Server adapters
apply their own timeout bounds; Solver uses explicit Cancel to terminate a long-running job. Cancellation or Worker
failure cannot mutate a draft. Solver candidates are copied and frozen before review; Server revisions and
