...
entire drop. Indicator-attached drafts instead complete through the existing initial-state or terminal-relation planner, or create a direct initial-to-terminal visual relation, without an Event dialog. Initial-to-state completion can change the semantic initial state; terminal and direct indicator relations remain visual only. An incomplete indicator draft focuses its free endpoint when edited. Passive Open, layout, Undo/Redo, and rerenders never trigger restoration or configuration.
