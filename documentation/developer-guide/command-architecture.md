...
its Chart placement, and dependent indicator relations in the same command. Draft state attachments are released at their last effective
coordinates, preserving the unfinished line and any remembered event. Renaming a state updates both draft attachments.
Renaming an event updates draft event memories; deleting the event clears the memory and restoration position while
retaining the line and its surviving connections.
...
| Drop a semantic endpoint on a state | Uses `update_transition` after validating the new deterministic key. |
| Drop a semantic endpoint in free space | `detach_transition_endpoint` replaces the row with a draft retaining its event, other attachment, and table position. |
| Attach a draft endpoint | `update_chart_draft_endpoint` stores the connection; a completed remembered draft restores atomically. |
...
history infrastructure. Eventless state-to-state drafts open configuration only after the second state attachment or explicit editing;
Cancel preserves the attachments. A remembered draft restores without a dialog, inserting at the lesser of its original
row position and the current table length. Duplicate-key, capacity, reference, or stale-revision failures reject the
entire drop. Indicator-attached drafts instead complete through the existing initial-state or terminal-relation planner, or create a direct initial-to-terminal visual relation, without an Event dialog. Initial-to-state completion can change the semantic initial state; state-to-terminal completion sets terminal membership while direct indicator relations remain visual only. An incomplete indicator draft focuses its free endpoint when edited. Passive Open, layout, Undo/Redo, and rerenders never trigger restoration or configuration.
