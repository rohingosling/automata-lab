...
notation synchronized with state-owned terminal flags. Flags participate in the semantic digest and can stop
an execution whose captured policy enables terminal states; Solver observations never infer those flags.
Preparation belongs to atomic domain/application boundaries before display, image export or Print, never rendering.
Client preparation captures current Chart preferences; headless hosting uses central defaults. Existing coherent
geometry is preserved, and capacity or placement failures reject the complete operation.
...
`1.0.0`, `1.1.0`, `1.2.0`, `1.3.0`, and `1.4.0` documents and always writes `1.4.0` with explicit state flags.
...
projection tables label them explicitly. Immediate role reversal uses the endpoint fields introduced in `1.3.0`; current canonical Save still writes `1.4.0`. Passive Open,
