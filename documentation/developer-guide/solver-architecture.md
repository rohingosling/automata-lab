...
Candidate state construction always sets `terminalState: false`. Neither observation inference nor hard-evidence
replay consumes the Simulator enablement preference. Review Summary, Comparison and replacement confirmation
disclose the terminal markings removed by complete model/Chart replacement. Apply remains one atomic command;
Undo restores prior flags and exact notation.
...
With no observations, inference returns a canonical one-state candidate named `state_generated_0001`, with no events,
actions, or transitions. The result includes `NO_OBSERVATIONS` and explicitly reports that no behavioral evidence
constrains it; this is a successful, reviewable result rather than a parsing failure.
...
The application owns at most one active Solver job. Completion, starting a replacement job, choosing Cancel, receiving
an invalid message, or encountering a Worker error disposes of the current Worker generation. A subsequent solve starts
with a fresh instance. The Solver adapter has no automatic elapsed-time timeout; Cancel terminates a long-running job.
