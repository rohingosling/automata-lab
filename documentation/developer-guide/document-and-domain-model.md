...
| States | Ordered unique state entities with required boolean `terminalState`, default false. |
...
A state owns semantic `terminalState`; configured terminal relations provide its synchronized notation.
One atomic domain/application planner updates flags, relations and required geometry before authoring or load
results become visible. True flags have one relation, shared indicators are supported, and false flags have none.
Unfinished drafts, orphan indicators and direct indicator connections remain visual. State-to-terminal completion
sets the flag; initial-to-state completion selects the initial state. Each successful completion removes its draft
in the same command. Solver observations do not infer terminal membership. The Simulator captures a separate
default-false execution policy at session creation or Reset.
...
Ordinary Save changes the baseline after a successful write without incrementing the document revision.
Defensive notation repair, if needed, commits one undoable revision before Save captures its snapshot; cancelling
the destination retains that repair as dirty work. Simulation and other
