...
| Draft transitions | Stable ID, endpoint coordinates, state attachments, and optional remembered event/table position. |
...
configuration or remembered-restoration command atomically adds a transition-table row and removes the draft.
...
deleting the draft. Save/Open and exact Undo/Redo preserve these lifecycle changes. Retained data uses strict file
version `1.2.0` and stays outside the semantic hosted-content digest while the transition is detached.
