...
`simulatorEnableTerminalStates` is a default-false, allowlisted browser preference sourced from the central
compile-time configuration. Settings drafts commit only on Apply. Start Session and Reset copy it into the
execution policy; changing preferences cannot alter an existing execution. It is excluded from model files,
semantic hashes, Solver input, Chart visibility and document-only print content.
...
| `presentation.groupBoxCornerRadiusPixels` | Shared group-box corner radius, currently 6 CSS pixels. |
...
`COMPILE_TIME_CONFIGURATION.presentation.groupBoxCornerRadiusPixels` independently controls the corner radius
of every group box in the workspace and application-owned dialogs. Its default is 6 CSS pixels, exposed by
`Application.tsx` as `--group-box-corner-radius`. It changes the border shape without adding padding or margins.
...
### Console Column Tuning

`shell.consoleColumns` defines widths and minimum/maximum bounds for Time, Severity, Code, Source,
and Message, plus the fixed Goto width. Code defaults to 250 CSS pixels; destination buttons remain
128 pixels wide. Runtime resizing is transient presentation state and is not added to the preference
allowlist. See [Console](./presentation-architecture#console) for keyboard and navigation behavior.

## Application Preferences

`ApplicationPreferences` is an explicit allowlist. It includes:

- theme, independent title-bar/application/Console message button colors, Save Backup, and Console Follow Tail;
- Solver Default Results Tab and Simulator Follow Trace Tail;
...
The Settings dialog works with a complete draft snapshot. Its master list is ordered General, Appearance, Chart,
Server, Solver, Simulator, and Print. All pages contain named group boxes for their controls, even when only one
control is present. General groups Save Backup under File Saving and the existing Console Follow Tail field under
Console; moving the latter does not change its persisted key. Appearance groups its theme selector under Theme.
Editor and Console have no master-list categories. Server groups its URL field and right-aligned Test Server button
inside Built-in Server. Solver uses Results, and Simulator uses Traces.

The Settings detail scroll region uses `--scroll-content-edge-inset` for equal left and right padding. Group-box
outer margins add independently on each side, giving equal clearance to the left content edge and the right pane
edge or vertical scrollbar. The existing top spacing remains separate from these horizontal insets.
Settings checkbox inputs and their labels share a vertically centered row, including narrow layouts; ordinary value
fields retain the common measured label column and responsive stacking.

### Solver and Simulator Preferences

`solverDefaultReviewTab` is an allowlisted candidate-review tab identifier. Its central default is
`applicationSettings.solver.solverDefaultReviewTab`, initially `solver-candidate-summary`. The seven permitted values
correspond to Summary, State Chart, States and Actions, Transition Table, Trace Coverage, Inference Report, and
Comparison. A new candidate initializes its review tab from the committed preference. Changing the preference does
not replace a user's tab selection for an existing candidate and does not change inference or candidate data.

`simulatorFollowTraceTail` is a boolean whose central default is
`applicationSettings.simulator.simulatorFollowTraceTail`, initially `true`. It gates automatic scrolling in both
Simulator trace tables. Each table still tracks whether the user is at the end: scrolling away pauses that table,
and returning to the end resumes it while the preference is enabled. Disabling it prevents new rows from scrolling
either table. Re-enabling it while a table is away from the end preserves its reading position until the user returns
to the end. It does not change trace generation, retention, session state, or the separate Console preference.

Both fields use the existing complete preference transaction and versioned browser envelope. Missing fields use
central defaults so older stored preferences keep their other values; invalid supplied values recover through the
normal per-field corruption handling. Neither preference enters model files, Undo/Redo, semantic revisions, worker
policy, or printed content.
...
`matchConsoleMessageColor`, whose central default is false. The checkbox disables the Console dropdown
...
Application Gray uses #383838 in Dark and #fafbfc in Light. Manual Console Gray uses #383838 in Dark
and #e6e4e1 in Light; Console White uses #e6e4e1 in Dark and #fafbfc in Light. This Light-only neutral
mapping keeps the visible Console shades consistent with their labels without changing other button scopes.
When matching is enabled, all five Console palette tokens are omitted so CSS falls back to row-local
surface, hover, active, text, and icon-filter tokens from `consoleSeverityButtonStyle`.
The helper chooses the shared Green message, Blue warning, and Red error palettes in both themes.
These are palette-family matches, not the exact Severity text RGB values. Matched buttons use white text/icons;
manual choices use their palette foregrounds, including dark text on Yellow/White. No text shadow is applied.
Missing or invalid matching values decode to false; valid saved true/false values remain effective without a
...
Application and Console destination buttons share `1px solid #000000` borders in both themes,
including hover and pressed states. Excluded controls retain their own theme styling.
