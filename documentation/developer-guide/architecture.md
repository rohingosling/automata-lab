...
| Chart-layout Worker | Calculates ELK state positions for an explicit Automatic Layout command. | Disposable ELK Worker. |
| Chart-routing Worker | Calculates derived routes and label positions without mutating the document. | Persistent dedicated Worker. |
...
job and candidate state, server synchronization state, Simulator session reference, bounded Console channel,
and transient entity-reference registry for Console navigation. The registry resolves only current entity lifetimes;
presentation receives a route and optional row-selection request, never a second authoring model.
...
5. Domain execution produces the replacement draft and records complete before/after snapshots for history.
