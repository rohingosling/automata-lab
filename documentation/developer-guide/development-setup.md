...
The default application port is 5173 and the Help preview port is 5174. Both ports are strict: an occupied port stops
startup instead of silently moving a server. Reuse an application instance that is already running, stop the earlier
development command in its terminal, or select two distinct free ports before starting:

```powershell
$env:AUTOMATA_APPLICATION_DEV_PORT = "5183"
$env:AUTOMATA_DOCUMENTATION_DEV_PORT = "5184"
npm --prefix automata-web run dev
```

Follow the new application URL printed by Vite; the Help proxy uses the selected documentation port automatically.
The variables affect that terminal session and can be removed to restore the defaults. Re-run the command after guide
changes when testing integrated Help, because this workflow serves a built documentation snapshot.
