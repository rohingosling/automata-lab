...
The Worker protocol version is `automata-lab-server/2`. Each request has an exact discriminated shape containing the
...
Run. Step consumes at most one buffered event; Run consumes the submitted prefix up to the first enabled terminal destination within protocol limits.
...
Protocol `/2` requires a fixed `executionPolicy` on Start/Reset and echoes it with `executionStatus` in session
snapshots. `/1` is incompatible. The runtime stops only after completing initial or destination entry actions,
preserving the reached state and traces. Initial terminal entry consumes zero events on first Run/Step; Reset
is silent and retains the pinned model while capturing the latest committed preference. A terminated execution
rejects further Run/Step with `SESSION_TERMINATED` until Reset or a new session. Disabled policy preserves
ordinary outgoing/self-transition behavior.

Each accepted execution result carries `consumedEventCount`. The gateway and application validate exact
nested keys, echoed policy, session/revision identity, status consistency and request-relative count bounds.
The client advances from the submitted buffer offset by the actual consumed prefix; late replies cannot
advance a changed buffer. Console emits `TERMINAL_STATE_REACHED` once only after these guards pass.
Flags belong to the semantic hosted revision; geometry and execution policy do not.
