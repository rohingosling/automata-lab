...
black borders, including Console destination buttons. Excluded menus, lists, tabs, Transition Table
dropdowns, Chart Palette tiles, and endpoint grips retain theme borders. Match Console Message Color
uses the shared Green, Blue, and Red palettes for Message, Warning, and Error respectively, while
Console title-bar buttons retain the Application Buttons palette. Console row hover/focus uses the theme control-hover surface to retain severity-text contrast.
...
Every fieldset also uses `--group-box-corner-radius`, supplied from
`COMPILE_TIME_CONFIGURATION.presentation.groupBoxCornerRadiusPixels`, currently 6 CSS pixels. The shared radius
rounds workspace and modal group borders without changing spacing or persisted preferences.
...
All controls on Application Settings pages belong to named group boxes, including groups with a single control.
The detail region keeps equal left/right scroll insets outside the additional group margins. Settings checkboxes
and their labels share a vertically centered row at all widths; labels may wrap without moving the checkbox onto a
separate row. Other value fields retain responsive stacking below their labels.
...
A sticky header names Time, Severity, Code, Source, Message, and Goto. Header and row tracks share CSS
custom properties. `COMPILE_TIME_CONFIGURATION.shell.consoleColumns` owns the defaults and bounds;
Code starts at 250px. The first five headers expose accessible separator grips for pointer dragging,
Left/Right (8px), Shift+Left/Right (32px), Home/End limits, and double-click reset. Message initially
fills available space; resizing fixes its width. Widths are component state, with no preference or model
serialization. Narrow layouts preserve all six columns through horizontal scrolling. Goto buttons use
route names and a fixed 128px width. Grip colors are #404040/#7f7f7f at rest/hover in Dark and their
#bfbfbf/#808080 inverses in Light; forced colors uses system colors.

`ConsoleContext.entityReference` is an optional transient numeric token. The application-owned
`ConsoleEntityNavigation` registry synchronizes current entity lifetimes with the authoring draft.
State/event/action keys use names; transition keys use source state plus event and resolve the current
row index. Unrelated edits and row moves preserve tokens. Rename, deletion, document replacement or
close, and Solver replacement retire affected tokens permanently; recreating a name cannot revive one.
Only diagnostics from the current document and supported failed commands receive captured references;
failed imported documents never bind their paths to unrelated current entities.

The shell resolves a Goto request and passes `rowNavigation` (key plus sequence) through EditorWorkspace
to the shared DataGrid. The grid expands its progressive render range, selects the row, focuses its
first cell, and scrolls it into view. The sequence supports repeated navigation to the same row.
If the token is absent or stale, navigation opens the route without requesting a new selection.
Tokens and selection never enter model files, Undo history, saved preferences, or worker protocols.
