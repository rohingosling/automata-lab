...
an immutable `PrintPageSetup` from the committed `ApplicationPreferences`. Report file version uses the same
`fileVersionForDocument` selector as Save, including `1.1.0` state attachments, `1.2.0` remembered metadata, and
`1.3.0` indicator attachments/direct connections. The Chart Projection lists remembered events and attached state or
