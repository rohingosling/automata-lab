...
an immutable `PrintPageSetup` from the committed `ApplicationPreferences`. Report file version is the canonical `1.4.0`. Each `PrintableState` copies its terminal flag, rendered
as localized Yes/No in the States table. Shared application preparation supplies coherent notation before
Chart rendering or image capture, including Print before any Chart navigation. The Chart Projection lists remembered events and attached state or
indicator names alongside endpoint coordinates, plus a separate Indicator Connections table. Indicator references
and direct-relation records are copied into the immutable snapshot. Capturing or printing a
draft never restores it to the transition table.
