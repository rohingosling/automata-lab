...
Versions `1.0.0`, `1.1.0`, `1.2.0`, `1.3.0`, and `1.4.0` are supported; the canonical writer always emits `1.4.0` with explicit `terminal_state` booleans. A missing or incorrect identity reports `FILE_ID_INVALID`.
An absent, non-string, or unsupported file version reports `FILE_VERSION_UNSUPPORTED`. Either result leaves the active
workspace unchanged.

## JSON Schema

`automata-web/src/infrastructure/files/schema-v1.ts` defines five closed JSON Schemas, one for each supported version.
...
9. Migrate legacy terminal membership or prepare modern notation from explicit flags; validate the complete result.
10. Replace the workspace atomically only after all blocking gates pass.
...
## Terminal Flags and Preparation

File `1.4.0` requires boolean `terminal_state` on every state, mapped explicitly to domain `terminalState`.
Events and Actions remain ordinary named entities. Earlier schemas reject that field. The active
`terminal-state-file-codec.ts` delegates legacy structural decoding, validates original references, migrates
configured state-to-terminal relations to true flags, then invokes the injected preparation port. Modern
flags are authoritative: true/missing notation is created and false/conflicting relations are removed.
Shared indicators and orphan material are retained. Dangling references, capacity failures and impossible
geometry fail atomically. Normalization reports one bounded summary and is idempotent.

Semantic revisions include terminal flags but exclude notation geometry and execution policy. Both strict
hosting and authoring readers use prepared documents; canonical output always writes `1.4.0`. Save commits
any defensive notation repair before confirmation, snapshot capture and destination selection. Cancellation
retains that undoable repair; only the captured revision becomes clean after a successful write.
...
and writes `1.4.0`; state-attachment fields were introduced in `1.1.0`. Even null attachment fields are invalid in `1.0.0`;
...
Canonical Save always chooses `1.4.0`; historical versions remain strict read contracts.
Null optional references are omitted. A `1.0.0`
...
| States | `name`, `description`; optional `terminal_state` | Exports all three columns; synchronizes flags and notation atomically. |
...
For States CSV, an absent `terminal_state` column preserves an overwritten flag and defaults new states false.
A present cell must normalize by trim/case to `true` or `false`; blank or invalid cells reject the entire import.
Preparation, collision confirmation and commit retain the single undoable transaction.
