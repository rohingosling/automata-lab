...
This documentation accompanies Automata Lab 1.7.0, built from the same source revision. This version adds
[Console column resizing and direct entity navigation](./user-guide/console-and-diagnostics), concise Goto buttons,
and consistent button palettes with optional severity matching. Gray is the default Console button color;
existing saved preferences remain in effect. See [Application Settings](./user-guide/application-settings)
for the independent manual-color option. The guides and affected screenshots reflect these changes.
The JSON reader supports strict file versions `1.0.0` through `1.4.0`; every canonical Save writes `1.4.0`
with explicit terminal-state flags. Application version, file-format version, and model metadata version
are separate values.
