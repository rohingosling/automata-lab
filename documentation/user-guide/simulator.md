...
A self-transition uses the same exit-then-entry order. When the buffer is exhausted without terminal stopping, the session remains in its current
...
Enable **Application Settings → Simulator → Model Parameters → Enable Terminal States**, then start or reset
a session. The execution captures that choice; changing Settings later does not alter it. The hosted model
supplies the terminal flags, so push local flag edits before starting a new session.

On terminal entry, the runtime completes all destination entry actions and the transition trace, then stops
before the next event. No terminal exit action runs. The buffer retains its unconsumed suffix and both traces
remain inspectable. Console reports `TERMINAL_STATE_REACHED` once per execution, and the status bar gives the reason.
Run and Step stay disabled until Reset or a new session; selecting another sequence cannot resume it.

If the initial state is terminal, Start Session and Reset remain silent. The first Run or Step emits its entry
actions once and stops with zero events consumed, even without a selected sequence or with an empty buffer.
With the preference disabled, terminal states behave like other states, including outgoing and self-transitions.
Reset captures the latest applied preference but retains the session's pinned model; Close and Start Session
are required to use a newer hosted revision.
...
The table scrolls over retained history and renders only its visible rows, so large traces remain usable.
[Application Settings → Simulator → Traces](./application-settings#simulator-settings) contains **Follow Trace Tail**,
enabled by default, for both trace tables. While enabled, each table follows new rows when at its end. Scrolling
away pauses that table, and returning to the end resumes it. Turning the preference off prevents automatic scrolling;
turning it back on while reading older rows preserves that position until you return to the end. This preference is
independent of Console Follow Tail.

Trace retention is bounded at 50,000 entries, and Console discloses truncation instead of presenting an incomplete
trace as complete. The scrolling preference does not change execution or retention.
