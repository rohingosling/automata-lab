...
The application provides deterministic state transducers with ordered state entry and exit actions, a partial
...
production remote server. Actions are inert names. State terminal flags can stop a Simulator execution after entry actions when its captured policy enables them.
Indicators provide notation; direct indicator links and unfinished drafts do not create terminal membership.
...
These capacities do not guarantee fast routing for every diagram. Automata Lab retains a known performance limitation
for dense Charts; the textual Editor remains available when diagram work becomes slow.
