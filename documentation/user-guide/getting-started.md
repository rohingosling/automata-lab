<script setup>
import guideScreenshot01 from '../../assets/images/screenshots/user-guide/UG-01.png';
</script>
...
Open the [Automata Lab application](https://rohingosling.github.io/automata-lab/) in your browser. The application
requires no account and automatically connects to its built-in server at `builtin://server`. For a local installation,
follow the Developer Guide's [Development Setup](../developer-guide/development-setup) chapter.
...
6. The status bar shows the initial state, model counts, server connection, and status from the active page.
...
Use Figure UG-01 to locate the tree, workspace, Console, and status bar. The screenshots in this guide use the light-switch example in Light theme unless stated otherwise. Open a screenshot to inspect its full-size image.

<figure id="figure-ug-01" class="guide-figure">
    <a :href="guideScreenshot01" aria-label="Open Figure UG-01 at full size">
        <img :src="guideScreenshot01" alt="Editor dashboard with the Model tree, five State Machine Info groups, Console, and status bar." width="1600" height="1200" loading="lazy" decoding="async">
    </a>
    <figcaption>Figure UG-01. The light-switch project in Editor, with all five dashboard groups visible.</figcaption>
</figure>
...
4. Return to **Editor → State Machine** and choose the model's **Initial State**.
...
7. Define state-and-event destinations in **Editor → Transition Table**, or create equivalent semantic transitions in
   **Chart**.
8. Choose **File → Validate State Machine** and review the result on **Editor** (State Machine Info) and in Console.
...
The [incomplete Save example](./files-and-data-exchange#figure-ug-23) shows this warning with the initial state unset.
...
Compare the loaded diagram with the [complete light-switch Chart](./state-chart#figure-ug-07).
...
The [Console example](./console-and-diagnostics#figure-ug-34) shows a Message, Warning, and Error with their codes and context actions.
