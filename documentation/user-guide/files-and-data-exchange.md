...
| `file_version` | Supported file-format version `1.0.0`, `1.1.0`, `1.2.0`, or `1.3.0`. |
...
`file_version` identifies the JSON contract understood by Automata Lab. The application reads strict versions `1.0.0`,
`1.1.0`, `1.2.0`, and `1.3.0`. Save selects the minimum version required by the current Chart data. The first matching row below takes precedence:

| Saved Chart data | File version |
|---|---|
| Draft indicator attachment or direct initial-to-terminal connection | `1.3.0` |
| Detached transition remembering an event or original table position | `1.2.0` |
| Draft state attachments without remembered event data | `1.1.0` |
| Otherwise: free drafts without remembered event data, or no drafts | `1.0.0` |
...
data. Legacy free coordinates remain free when opened. The state-machine execution rules are unchanged.
