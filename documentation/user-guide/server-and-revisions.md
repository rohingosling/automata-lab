<script setup>
import guideScreenshot22 from '../../assets/images/screenshots/user-guide/UG-22.png';
</script>
...
The built-in server uses protocol `automata-lab-server/2`. A future network adapter may implement the same gateway, but
...
Figure UG-22 shows the stale-session warning after a semantic Push.

<figure id="figure-ug-22" class="guide-figure">
    <a :href="guideScreenshot22" aria-label="Open Figure UG-22 at full size">
        <img :src="guideScreenshot22" alt="Simulator retaining its first-step traces while the status bar and Console indicate that the session is stale." width="1600" height="1200" loading="lazy" decoding="async">
    </a>
    <figcaption>Figure UG-22. A newer hosted revision marks the existing session stale but preserves its state and traces. Reset keeps that older pinned revision.</figcaption>
</figure>
