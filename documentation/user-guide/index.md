...
current implemented product scope and uses the labels shown by the application.
...
warning; it does not change the current state or produce transition actions, and a running sequence continues with later
events. Initial entry actions are reported separately, once before the first event after session creation or Reset.
...
Start with the [application overview screenshot](./getting-started#figure-ug-01) to locate these areas. Screenshots throughout the guide use the maintained light-switch example; open an image to read it at full size.

## Browser Requirements

Automata Lab targets current desktop versions of Chrome, Edge, Firefox, and Safari. Automated browser tests exercise
Chromium, Firefox, and WebKit; they do not certify every browser version, operating system, or assistive technology.
