...
mark the state terminal and create its relation. One state may have at most one terminal-indicator relation, while one indicator may receive
relations from several states.

The connection and the state's **Terminal State** checkbox are synchronized atomically. Checking the box creates
missing notation; unchecking removes that state's relation while retaining the indicator. Deleting a relation
clears its state's flag; deleting a shared indicator clears every connected state's flag. Undo restores the
complete flag/notation snapshot. Reconnecting to another indicator retains membership.

Notation stays visible whether **Enable Terminal States** is on or off. The preference controls execution only
and is captured at Start Session or Reset. Orphan indicators, unfinished drafts and direct initial-to-terminal
links remain visual and do not mark states terminal.
...
| State and terminal indicator | Mark the state terminal and create or replace its connection. |
...
| State and Terminal Indicator, in either order | Mark the state terminal and create or replace its relation immediately. |
