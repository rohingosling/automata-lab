...
| About Automata Lab | Shows version, description, licences, and release notes wrapped at 79 characters, with 79-hyphen release separators. The desktop text box fits 80 monospace characters without horizontal scrolling. |
...
Application Colors is available through **File → Application Settings → Appearance**, with independent
...
and `1.2.0`, and requires the top-level members `settings`, `state_machine`, `chart`, `solver`, and `simulator`. Save emits deterministic two-space
JSON with one trailing newline. See [Files and Data Exchange](./files-and-data-exchange) for completeness, canonicalization,
and strict-reader rules. Save chooses the minimum file version needed: `1.2.0` for remembered transition events/order,
...
| Chart draft | Visual transition with coordinates, optional state attachments and event memory; no runtime effect until configured or restored. |
