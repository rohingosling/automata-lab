...
Figure UG-36 illustrates the Console headers, keyboard resize grip, and labeled controls when forced colors replace the application palette.

<figure id="figure-ug-36" class="guide-figure">
    <a :href="guideScreenshot36" aria-label="Open Figure UG-36 at full size">
        <img :src="guideScreenshot36" alt="Console in forced colors with keyboard focus on the Code resize grip, six column headers, severity text, and labeled destination buttons." width="1600" height="196" loading="lazy" decoding="async">
    </a>
    <figcaption>Figure UG-36. The Console with forced colors active and keyboard focus on the Code resize grip. This example uses browser forced-colors emulation; the exact palette follows the user's system.</figcaption>
</figure>
...
<figure id="figure-ug-35" class="guide-figure guide-figure--narrow">
    <a :href="guideScreenshot35" aria-label="Open Figure UG-35 at full size">
        <img :src="guideScreenshot35" alt="390-pixel-wide application with Model, Detail, and Console navigation and the Console selected, showing its first columns with the remaining columns available by horizontal scrolling." width="390" height="900" loading="lazy" decoding="async">
    </a>
    <figcaption>Figure UG-35. At 390 CSS pixels, Console retains all six columns. Scroll horizontally for Source, Message, and Goto, or use Model and Detail to change regions.</figcaption>
</figure>
...
Chart-only coordinates, orphan indicators, direct indicator links, and unconfigured drafts have no runtime semantics.
Configured terminal-state relations and the accessible state checkbox update the same semantic flag. Their
