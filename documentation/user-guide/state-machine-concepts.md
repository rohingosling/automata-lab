...
In both cases, the runtime consumes the event, records a warning, leaves the state unchanged, emits no transition
actions, and continues with later events. The first Run or Step after session creation or Reset still reports pending
initial entry actions before checking that first event.
...
Compare the same state in the [Entry Actions](./editor#figure-ug-03) and [Exit Actions](./editor#figure-ug-04) screenshots.
...
A **Terminal Indicator** is a filled dot enclosed by a ring. A configured state-to-indicator connection marks
that state as terminal. Each terminal state has one such relation, and several states may share an indicator.
The state owns the flag; the indicator supplies visible notation. Orphans, direct initial-to-terminal links,
and unfinished drafts have no terminal membership effect.

New states default to nonterminal. **Terminal State** in the state dialog and Chart connections update the flag
and notation together. **Application Settings → Simulator → Model Parameters → Enable Terminal States** defaults
off. When captured as enabled at Start Session or Reset, execution stops after completing a terminal state's entry
actions, preserving state and traces. An initial terminal state stops on the first Run/Step without consuming an event.
Reset is silent. Solver observations do not classify terminal states.

The [complete light-switch Chart](./state-chart#figure-ug-07) shows the filled initial dot above state_start and the terminal ring below state_fuse_blown.
...
The [first Simulator Step](./simulator#figure-ug-19) illustrates this order with three action rows: initial entry, source exit, then destination entry.
