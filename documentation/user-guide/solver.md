...
Choose the first view for future candidates in [Application Settings → Solver → Results](./application-settings#solver-settings)
using **Default Results Tab**. The default is Summary. You can switch review tabs freely; changing the default does
not move an existing candidate to another tab.
...
Solver candidates set every terminal flag false: observations and the Simulator preference do not infer
terminal membership. Summary, Comparison and Apply confirmation disclose the current terminal markings that
will be removed. Undo restores the previous flags and exact indicator geometry. Figure UG-18 predates this
additional terminal-marking disclosure; its other replacement fields remain applicable.
