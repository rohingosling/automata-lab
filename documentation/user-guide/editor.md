...
**Terminal State** appears in state Add/Edit dialogs and defaults unchecked for a new state. Confirm updates
the flag and its Chart notation in one undoable command. Clearing it removes the relation while retaining the
indicator; shared indicators and other states remain intact. Cancel, Close and Escape discard the draft.
Events and Actions have no terminal checkbox.
