...
uses the shared darker Green message, Blue warning, and Red error palettes for inline buttons. It is
unchecked by default, so Console destination buttons start Gray. Application buttons
...
Group boxes have consistent rounded corners throughout the workspace and application-owned dialogs.
