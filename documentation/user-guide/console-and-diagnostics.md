...
that dropdown to use the shared darker pastel Green palette for messages, Blue for warnings, and Red
for errors, including their hover and pressed shades. Matched buttons use white text and icons in both
themes. This option defaults
checked. While checked, the dropdown is disabled and retains its chosen color for when matching is turned off.
...
theme surface that keeps severity labels readable. In Dark mode, inline button text is white. A subtle
dark text edge keeps it readable on manually selected Yellow/White fills without changing their colors.

Figure UG-34 shows how codes and severity labels distinguish a successful operation, a rejected edit, and a recoverable runtime input.

<figure id="figure-ug-34" class="guide-figure">
    <a :href="guideScreenshot34" aria-label="Open Figure UG-34 at full size">
        <img :src="guideScreenshot34" alt="Enlarged Console with VALIDATION_PASSED, ENTITY_EXISTS, and UNKNOWN_EVENT rows and the severity filters, Follow Tail, and Clear controls." width="1600" height="692" loading="lazy" decoding="async">
    </a>
    <figcaption>Figure UG-34. A successful validation Message, duplicate-name Error, and unknown-event Warning alongside the workflow's lifecycle messages. All severity filters are enabled.</figcaption>
</figure>
...
recorded. Below 768 CSS pixels, the code, source, and inline context columns are hidden; widen the window to see them.
Copying a focused row still includes its code and source.
